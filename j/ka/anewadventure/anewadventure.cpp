...
//inline bool blocked(Node node, const vbi& M)
//{
//	uli j0 = i * A + (j )
//}
...
//	}
//
//	inline bool blocked(const vbi& M) const
//	{
//		if (i == C)
//			return false;
//		uli j0 = j + A - t % A;
//		return M[i * A + j0 % A] || M[i * A + (j0 + 1) % A];
//	}
//
//	inline vector<Node> neighbors(const vbi& M) const
//	{
//		if (i == C)
//			return vector<Node>();
//		vector<Node> ret;
...
//		}
//		{
//			const Node node = { t + 1, i, (j + 1) % A };
//			if (!node.blocked(M))
//				ret.push_back(node);
//		}
//		{
//			const Node node = { t + 1, i, (j + A - 1) % A };
//			if (!node.blocked(M))
//				ret.push_back(node);
//		}
//		return ret;
...
//};
//
//inline constexpr bool operator==(const Node& lhs, const Node& rhs)
//{
//	return lhs.t == rhs.t && (lhs.done() && rhs.done() || lhs.i == rhs.i && lhs.j == rhs.j);
//}
//
//template<>
//struct std::hash<Node>
//{
//	ulli operator()(const Node& node) const
//	{
//		ulli ret = ulli(node.t) << 32;
//		ret += node.i * A;
//		if (!node.done())
//			ret += node.j;
//		return ret;
...
//inline constexpr void clear(priority_queue<Args...>& q)
//{
//
//}

//unordered_map<Node, ulli> gScore, fScore;
...
//		for (Node next : current.neighbors(M))
//		{
//			ulli tentative = gScore[current] + 1;
//			auto iter = gScore.find(next);
//			if (iter == gScore.end())
//			{
//				iter = gScore.emplace(next, MAX(ulli)).first;
//			}
//			if (tentative < iter->second)
//			{
//				iter->second = tentative;
//				fScore[next] = tentative + next.h();
//				if (openSet.insert(next).second)
//					openQueue.push(next);
//			}
//		}
//	}
...
inline vuli neighbors(uli u, const vbi& M)
{
	//if (u > A * C)
	//{
	//	cout << "eqhifojqweoigjeijoewfjqowejf" << endl;
	//	throw nullptr;
	//}
	uli i = u / A, j = u % A;
	vuli ret;
