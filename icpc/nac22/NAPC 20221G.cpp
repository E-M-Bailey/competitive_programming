...
void go()
{
    ...
    int dt = int(round(m * 10));

    vvi32 I(G, vi32(W, MAX<int>)), O(G, vi32(W, MAX<int>);

    const auto dryT = [&](pi32 p)
    {
        int h = G[p.first][p.second];
        long double t = 1.5 / atanl(1) * acosl(2 * h / a - 1);
        return make_pair(t, 8 * atanl(1) - t);
    };

    uspi32 F, F2;
    F.emplace(X, Y);
    for (int i = 0; i <= MAXT; i += dt)
    {
        for (pi32 f : F)
        {
