...
struct rarray
{
	...
	array_type a;
	constexpr [[nodiscard]] auto& at(size_t pos) { return a.at(pos); }
	constexpr [[nodiscard]] const auto& at(size_t pos) const { return a.at(pos); }
	constexpr [[nodiscard]] auto& operator[](size_t pos) { return a[pos]; }
	constexpr [[nodiscard]] const auto& operator[](size_t pos) const { return a[pos]; }
	constexpr [[nodiscard]] auto& front() { return a.front(); }
	constexpr [[nodiscard]] const auto& front() const { return a.front(); }
	constexpr [[nodiscard]] auto& back() { return a.back(); }
	constexpr [[nodiscard]] const auto& back() const { return a.back(); }
	constexpr [[nodiscard]] auto data() { return a.data(); }
	constexpr [[nodiscard]] auto data() const { return a.data(); }
	constexpr [[nodiscard]] auto begin() noexcept { return a.begin(); }
	constexpr [[nodiscard]] auto begin() const noexcept { return a.begin(); }
	constexpr [[nodiscard]] auto cbegin() const noexcept { return a.cbegin(); }
	constexpr [[nodiscard]] auto end() noexcept { return a.end(); }
	constexpr [[nodiscard]] auto end() const noexcept { return a.end(); }
	constexpr [[nodiscard]] auto cend() const noexcept { return a.cend(); }
	constexpr [[nodiscard]] auto rbegin() noexcept { return a.rbegin(); }
	constexpr [[nodiscard]] auto rbegin() const noexcept { return a.rbegin(); }
	constexpr [[nodiscard]] auto crbegin() const noexcept { return a.crbegin(); }
	constexpr [[nodiscard]] auto rend() noexcept { return a.rend(); }
	constexpr [[nodiscard]] auto rend() const noexcept { return a.rend(); }
	constexpr [[nodiscard]] auto crend() const noexcept { return a.crend(); }
	constexpr [[nodiscard]] auto size() const noexcept { return a.size(); }
	constexpr [[nodiscard]] auto empty() const noexcept { return a.empty(); }
	...
	constexpr [[nodiscard]] rarray() = default;
	constexpr [[nodiscard]] rarray(array_type&& a) noexcept: a(forward<array_type>(a)) {}
	template<input_iterator It> constexpr [[nodiscard]] rarray(It first, It last)
	{
...
template<class R, class C> constexpr [[nodiscard]] auto container_inserter(C& c)
{
...
template<class C, ranges::input_range R, class... Args> requires (!ranges::view<C>) constexpr C to(R&& r, Args&&... args)
{
	if constexpr (!ranges::input_range<C> || convertible_to<ranges::range_reference_t<R>, ranges::range_value_t<C>>)
	{
		...
		else
		{
			...
			auto last = ranges::copy(r, container_inserter<ranges::range_reference_t<R>>(c)).out;
			if constexpr (is_std_array<C>)
			{
...
template<size_t N, ranges::input_range R> using to_array_t = array<ranges::range_value_t<R>, N>;
template<size_t N, ranges::input_range R> constexpr [[nodiscard]] auto to(R&& r) // Nonstandard
{
	return to<to_array_t<N, R>>(forward<R>(r));
}
template<template<class...> class C, ranges::input_range R, class... Args> constexpr [[nodiscard]] auto to(R&& r, Args&&... args)
{
...
template<template<class...> class C, ranges::input_range R, class... Args> using to_deduce_t = invoke_result_t<to<C, R, Args...>, R&&, Args&&...>;
namespace impl
{
...
{
private:
	template<class S, ranges::range R> static constexpr to_array_t<N, R> call(S&& s, R&& r) { return to<N>(forward<R>(r)); }
public:
	template<ranges::range R> constexpr to_array_t<N, R> operator()(R&& r)& { return call(*this, forward<R>(r)); }
	template<ranges::range R> constexpr to_array_t<N, R> operator()(R&& r) const& { return call(*this, forward<R>(r)); }
	template<ranges::range R> constexpr to_array_t<N, R> operator()(R&& r)&& { return call(move(*this), forward<R>(r)); }
	template<ranges::range R> constexpr to_array_t<N, R> operator()(R&& r) const&& { return call(move(*this), forward<R>(r)); }
};
...
template<class C, class... Args> requires (!ranges::view<C>) constexpr impl::ToExplicit<C, Args...> to(Args&&... args) { return impl::ToExplicit<C, Args...>(0, forward<Args>(args)...); }
...
private:
	...
	constexpr T a_lo() { return a & (a - 1); }
	constexpr T b_hi() { return (T)1 << hi_idx(b); }
...
public:
	...
	constexpr biterator& operator++() noexcept { dassert(a); sw(a_lo()); }
	constexpr biterator& operator--() noexcept { dassert(b); sw(b_hi()); }
	constexpr [[nodiscard]] biterator operator++(int) noexcept { auto it = *this; ++* this; return it; }
	constexpr [[nodiscard]] biterator operator--(int) noexcept { auto it = *this; --* this; return it; }
	constexpr [[nodiscard]] T bit() const noexcept { return a_lo(); }
	constexpr [[nodiscard]] int operator*() const noexcept { dassert(a); return lo_idx(a); }
	constexpr [[nodiscard]] bool operator==(const biterator& it) const noexcept { return cmp<equal_to>(it); }
	constexpr [[nodiscard]] bool operator!=(const biterator& it) const noexcept { return cmp<not_equal_to>(it); }
	constexpr [[nodiscard]] bool operator<(const biterator& it) const noexcept { return cmp<less>(it); }
	constexpr [[nodiscard]] bool operator<=(const biterator& it) const noexcept { return cmp<less_equal>(it); }
	constexpr [[nodiscard]] bool operator>(const biterator& it) const noexcept { return cmp<greater>(it); }
	constexpr [[nodiscard]] bool operator>=(const biterator& it) const noexcept { return cmp<greater_equal>(it); }
	constexpr [[nodiscard]] strong_ordering operator<=>(const biterator& it) const noexcept { return cmp<compare_three_way>(it); }
};

template<integral T> struct bits: ranges::view_interface<bits>
{
	T val;
	constexpr [[nodiscard]] bits(T val = 0): val(val) {}
	constexpr [[nodiscard]] biterator<T> begin() { return biterator(val); }
	constexpr [[nodiscard]] biterator<T> end() { return biterator<T>(); }
};
...
template<unsigned_integral T> constexpr [[nodiscard]] T isqrtc(T x)
{
	T ub = ((T)1 << (8 * sizeof(x) - countl_zero(x) + 1) / 2) + 1;
	T r = *ranges::lower_bound(views::iota((T)0, ub), x, {}, [](T r) { return r * r; });
	return r;
}
template<unsigned_integral T> constexpr [[nodiscard]] T isqrtf(T x)
{
	T r = isqrtc(x);
	return r - (r * r != x);
}
...
constexpr [[nodiscard]] uint32_t phi(uint32_t a)
{
	uint32_t phi = a;
...
template<uint32_t M> struct mint
{
	...
	constexpr [[nodiscard]] uint32_t val() const { return v; }
	constexpr [[nodiscard]] friend mint operator+(mint a) { return a; }
	constexpr [[nodiscard]] friend mint operator-(mint a) { if (a.v) a.v = M - a.v; return a; }
	constexpr mint& operator+=(mint b) { if ((v += b.v) >= M) v -= M; return *this; }
	constexpr [[nodiscard]] friend mint operator+(mint a, mint b) { return a += b; }
	...
	constexpr [[nodiscard]] friend mint operator*(mint a, mint b) { return a *= b; }
	constexpr [[nodiscard]] friend mint mpow(mint a, pow_t b)
	{
		mint p = 1;
		for (; b.v; b.v /= 2, a *= a) if (b.v % 2) p *= a;
		return p;
	}
	constexpr [[nodiscard]] mint mpow(pow_t b) { return mpow(*this, b); }
	constexpr [[nodiscard]] mint inv(mint a) const requires PRIME
	{
		dassert(a);
		return mpow(*this, INV_POW);
	}
	constexpr [[nodiscard]] friend mint inv(mint a) { return a.inv(); }
	constexpr mint& operator/=(mint b) { return *this *= b.inv(); }
	constexpr [[nodiscard]] friend mint operator/(mint a, mint b) { return a /= b; }
	constexpr [[nodiscard]] friend bool operator==(mint a, mint b) { return a.v == b.v; }
	constexpr [[nodiscard]] friend bool operator!=(mint a, mint b) { return a.v != b.v; }
	constexpr [[nodiscard]] friend bool operator<(mint a, mint b) { return a.v < b.v; }
	constexpr [[nodiscard]] friend bool operator<=(mint a, mint b) { return a.v <= b.v; }
	constexpr [[nodiscard]] friend bool operator>(mint a, mint b) { return a.v > b.v; }
	constexpr [[nodiscard]] friend bool operator>=(mint a, mint b) { return a.v >= b.v; }
	constexpr [[nodiscard]] operator uint32_t() const { return v; }
