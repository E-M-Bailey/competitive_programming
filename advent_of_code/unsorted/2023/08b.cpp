...
{
	string lr, token;
	unordered_map<string, a2string> M;
	cin >> lr;
	while (cin >> token)
	{
		auto [it, ins] = M.emplace(token, a2string{ "", "" });
		cin >> token >> it->second;
		it->second[0].erase(begin(it->second[0]));
		it->second[0].pop_back();
		it->second[1].pop_back();
	}

	vector<vector<pair<string, int>>> P;
	vector<unordered_set<string>> C;
	for (auto [cur, _] : M)
	{
		C.clear();
		P.emplace_back();
		token = cur;
		while (!C.contains(token))
		{
			for (char c : lr)
			{
				ans++;
