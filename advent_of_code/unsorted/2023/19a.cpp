...
void go()
{
	...
	{
		auto ix = line.find("x="), im = line.find("m=", ix), ia = line.find("a=", im), is = line.find("s=", ia);
		auto it = begin(line);
		int x = stoi(string(it + ix + 2, it + im - 1)),
			m = stoi(string(it + im + 2, it + ia - 1)),
			a = stoi(string(it + ia + 2, it + is - 1)),
			s = stoi(string(it + is + 2, end(line) - 1));
		string cur = "in";
		while (cur != "A" && cur != "R")
		{
			cerr << x << cur << endl;
			...
			{
				cerr << c << ' ' << gt << ' ' << lim << ' ' << dest << endl;
				int val = c == 'x' ? x : c == 'm' ? m : c == 'a' ? a : s;
				if (gt ? val > lim : val < lim)
				{
					cur = dest;
					break;
				}
