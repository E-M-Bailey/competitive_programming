...
unordered_map<string, vector<tuple<char, bool, int, string>>> D;

long long solve(int lx = 1, int rx = 4000, int lm = 1, int rm = 4000, int la = 1, int ra = 4000, int ls = 1, int rs = 4000, string const &cur = "in")
{
	if (cur == "R")
		return 0;
	if (cur == "A")
		return (long long)(rx - lx + 1) * (rm - lm + 1) * (ra - la + 1) * (rs - ls + 1);
	long long acc = 0;
	for (auto const &[c, gt, lim, dest] : D[cur])
	{
		if (lx > rx || lm > rm || la > ra || ls > rs)
			return acc;
		int &l = c == 'x' ? lx : c == 'm' ? lm : c == 'a' ? la : ls;
		int &r = c == 'x' ? rx : c == 'm' ? rm : c == 'a' ? ra : rs;
		if (gt && r > lim)
		{
			int tmp = l;
			l = max(l, lim + 1);
			acc += solve(lx, rx, lm, rm, la, ra, ls, rs, dest);
			l = tmp;
			r = lim;
		}
		else if (!gt && l < lim)
...
{
	{
		...
		while (getline(cin, line) && !line.empty())
		{
			...
			string name = line.substr(0, l), rules = line.substr(l + 1, r - (l + 1));
			istringstream is(rules);
			string rule;
			auto &DD = D[name];
			//cerr << name << "->" << rules;
			while (getline(is, rule, ','))
			{
				auto pos = rule.find(':');
				if (pos == string::npos)
					DD.emplace_back('x', true, -1, rule);
				else
					DD.emplace_back(rule[0], rule[1] == '>', stoi(rule.substr(2, pos - 2)), rule.substr(pos + 1));
			}
		}
	}
