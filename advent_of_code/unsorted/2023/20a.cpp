...
unordered_map<string, char> T;
unordered_map<string, vector<string>> G, H;
unordered_map<string, bool> off;
unordered_map<string, unordered_set<string>> last_off;

void go()
{
	{
		string src, dummy, dests;
		while (getline(cin >> src >> dummy, dests))
		{
			//cerr << ' ' << src << ' ' << dests << endl;
			char tp = src[0];
			if (tp != 'b')
			...
			while (is >> dest)
			{
				...
				H[dest].push_back(src);
			}
		}
	}

	for (auto const &[dest, tp] : T)
	{
		if (tp == '%')
			off[dest] = true;
		else if (tp == '&')
			last_off[dest] = unordered_set(begin(H[dest]), end(H[dest]));
	}
	...
	rep(i, 0, 1000)
	{
		Q.emplace("button", "broadcaster", true);
		while (!Q.empty())
		{
			auto [u, v, l] = Q.front();
			Q.pop();
			...
			else
			{
				if (l)
					...
					last_off[v].erase(u);
				for (auto const &w : G[v])
					Q.emplace(v, w, last_off[v].empty());
			}
		}
	}
