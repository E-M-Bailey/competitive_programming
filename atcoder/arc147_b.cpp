...
template<class Op>
void sort2(vi32& P, const Op& op, stringstream& ans) {
    int n = P.size();
	for (int i = n - 1; i >= 0; i--)
		for (int j = i % 2; j + 2 <= i; j += 2)
            if (op(P[j + 2], P[j], i % 2)) {
                ans << "B " << j + 1 << endl;
                ln++;
                swap(P[j], P[j + 2]);
...
void go() {
	...
	const auto cmp1 = [&](int l, int r, int o) {
        int lo = l % 2, ro = r % 2;
        return o ? ro > lo : ro < lo;
	};
	sort2(P, cmp1, ans);
	...
	sort2(P, [&](int l, int r, int o) {return l < r;}, ans);
	cout << ln << endl << ans.str();
}
