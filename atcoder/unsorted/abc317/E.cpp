...
void go()
{
	...
	{
		if (GR[i][j] == '>')
			for (int k = j; ++k < w && (GR[i][k] == '.' || GR[i][k] == '$');)
				GR[i][k] = '$';
		else if (GR[i][j] == '<')
			for (int k = j; --k >= 0 && (GR[i][k] == '.' || GR[i][k] == '$');)
				GR[i][k] = '$';
		else if (GR[i][j] == 'v')
			for (int k = i; ++k < h && (GR[k][j] == '.' || GR[k][j] == '$');)
				GR[k][j] = '$';
		else if (GR[i][j] == '^')
			for (int k = i; --k >= 0 && (GR[k][j] == '.' || GR[k][j] == '$');)
				GR[k][j] = '$';
		else if (GR[i][j] == 'S')
		{
			tie(si, sj) = pair(i, j);
	...
	while (!Q.empty())
	{
		...
		{
			int i2 = i + di, j2 = j + dj;
			if (i2 < 0 || i2 >= h || j2 < 0 || j2 >= h || V[i2][j2] || GR[i2][j2] != '.')
				continue;
			Q.emplace(d + 1, i2, j2);
		}
	}
