...
void go() {
    ...
    vi64 CC(M), LC(M), LD(M), LS(M), RC(M), RD(M), RS(M);
    for (int i = 1; i < M; i++) {
        LS[i] = LS[i - 1] + LD[i - 1] * R[i - 1];
        LC[i] = LC[i - 1] + R[i - 1];
        CC[i] = CC[i - 1] + L[i] - R[i - 1];
        LD[i] = LD[i - 1] + LC[i];
    }
    for (int i = M - 2; i >= 0; i--) {
        RS[i] = RS[i + 1] + RD[i + 1] * L[i + 1];
        RC[i] = RC[i + 1] + L[i + 1];
        RD[i] = RD[i + 1] + RC[i];
    }

    //for (int i = 0; i < 50; i++)
    //    cerr << i << ' ' << L[i] << ' ' << R[i] << ' ' << CC[i] << ' ' << LC[i] << ' ' << LD[i] << ' ' << LS[i] << ' ' << RC[i] << ' ' << RD[i] << ' ' << RS[i] << ' ' << LS[i] + RS[i] + LD[i] * (RC[i] + CC[i]) + RD[i] * (LC[i] + CC[i]) << endl;

    i64 best = MAX<i64>;
    for (int i = 0; i < M; i++) {
        i64 d = LS[i] + RS[i] + LD[i] * (RC[i] + CC[i]) + RD[i] * (LC[i] + CC[i]);
        best = min(d, best);
    }
    cout << best;
}
