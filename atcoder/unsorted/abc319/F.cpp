...
auto const cmp = [](int i, int j)
	{
		return get<1>(A[i]) > get<1>(A[j]);
	};
typedef PQi32<decltype(cmp)> front;

pair<int, front> frontier(int mask, int st)
{
	auto ans = make_pair(0, front(cmp));
	auto &[fm, F] = ans;
	F.push(0);
	while (!F.empty() && get<1>(A[F.top()]) <= st)
	{
		int v = F.top();
		F.pop();
		for (int w : T[v])
...
void complete(front &F, int &st)
{
	while (!F.empty() && get<1>(A[F.top()]) <= st)
	{
		int v = F.top();
		F.pop();
		st = min(st + get<2>(A[v]), 1000000000);
		for (int w : T[v])
		{
			if (MI[w] < 0)
				F.push(w);
...
void go()
{
	...
	while (!PQ.empty())
	{
		...
		if (DP[mask] > st)
			continue;
		auto [fm, F] = frontier(mask, st);
		while (fm)
		{
			auto F2 = F;
			int i = __builtin_ctz(fm);
			fm &= ~(1 << i);
			int st2 = (int)min(st * (i64)get<2>(A[M[i]]), (i64)1000000000);
