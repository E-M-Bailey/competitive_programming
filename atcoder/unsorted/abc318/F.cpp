...
void go()
{
	...
	i64 x = -4'000'000'000'000'000'000, ans = 0;
	while (x < LLONG_MAX)
	{
		int ct = 0;
		i64 nxt = P.empty() ? LLONG_MAX : X[P.back()];
		while (!P.empty() && X[P.back()] == x)
		{
			N.push_back(P.back());
			P.pop_back();
		}
		int pi = (int)P.size() - 1, ni = (int)N.size() - 1, li = 0;
		while (pi >= 0 && ni >= 0)
		{
			i64 pd = X[P[pi]] - x, nd = x - X[N[ni]], l = L[li++];
			if (pd <= nd)
			{
				if (pd <= l)
					ct++;
			...
			else
			{
				...
				{
					ct++;
					nxt = min(nxt, x + l - nd + 1);
				}
				ni--;
				nxt = min(nxt, x + (pd - nd + 1) / 2);
			}
		}
		while (pi >= 0)
		{
			i64 pd = X[P[pi]] - x, l = L[li++];
			if (pd <= l)
				ct++;
			else
			{
