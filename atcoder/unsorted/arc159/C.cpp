...
void go()
{
	...
	int avg = accumulate(ALL(A), 0) / n;
	for (int& a : A)
		a -= avg;

	vi32::iterator m, M;
	while ((tie(m, M) = minmax_element(ALL(A))), *m != *M)
	{
