...
// source: https://cses.fi/paste/7b5c0925b9926d5f640805/

struct Node
{
	struct T { int l0, r0, m0, l1, r1, m1, s; }; // data type
	typedef bool L; // lazy type
	const T tneut{ 0, 0, 0, 0, 0, 0, 0 };     // neutral elements
	const L lneut = false;
	T f(T a, T b)
	{
		return {
			a.l0 == a.s ? a.l0 + b.l0 : a.l0,
			b.r0 == b.s ? a.r0 + b.r0 : b.r0,
			max({a.m0, b.m0, a.r0 + b.l0}),
			a.l1 == a.s ? a.l1 + b.l1 : a.l1,
			b.r1 == b.s ? a.r1 + b.r1 : b.r1,
			max({a.m1, b.m1, a.r1 + b.l1}),
			a.s + b.s };
	} // (any associative fn)
	T apply(T a, L b)
	{
...
void go()
{
	...
	rep(i, 0, n)
	{
		S[i].l0 = S[i].r0 = S[i].m0 = s[i] == '0';
		S[i].l1 = S[i].r1 = S[i].m1 = s[i] == '1';
		S[i].s = 1;
	}
	Node *root = new Node(S, 0, n);
