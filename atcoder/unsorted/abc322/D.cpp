...
bitset<16> constexpr t(0xf), l(0x1111), b(0xf000), r(0x8888);
...
vector<bitset<16>> tr(bitset<16> s)
{
...
void dbg(bitset<16> s)
{
	rep(i, 0, 4)
		cerr << bitset<4>((s >> (4 * i)).to_ulong()) << endl;
}

void go()
{
	bitset<16> A = rd(), B = rd(), C = rd();
	for (auto aa : tr(A))
	{
		rep(i, 0, 4)
		{
			for (auto bb : tr(B))
			{
				rep(j, 0, 4)
				{
					for (auto cc : tr(C))
					{
						if ((aa & bb).none() && (aa & cc).none() && (bb & cc).none() && (aa | bb | cc).all())
						{
							cout << "Yes";
							return;
						}
