...
void go() {
    ...
    for (auto& [a, b] : AB) {
        ...
        /*
    vector<vector<pair<int, bool>>> E(m);
    for (int i = 0; i < n; i++) {
        E[A[i]].emplace_back(i, false);
        E[B[i]].emplace_back(i, true);
    }*/
    vvi32 E(m);
    map<int, int> S;
    int maxA = 0;
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (i >= maxA) {
            int lo = S.begin()->first, hi = i;
            incr(hi - lo, hi);
            //cerr << i << ' ' << hi - lo << ' ' << hi << endl;
        }
    }
