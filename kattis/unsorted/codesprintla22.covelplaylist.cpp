...
bool has(const string& str, int pos, int nd = 0)
{
    if (pos == str.size())
        return true;
    if (t[pos].next.find(str[0]) == t[pos].next.end())
        return false;
    int next = t[pos].next[str[0]];
    return has(str, pos + t[next].len(), next);
}
