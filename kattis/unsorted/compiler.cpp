...
vi32 costs2(int lim) {
    ...
    for (int n = 0; n < lim; n++) {
        if (n <= 1)
            ...
            ans[n] = 4 + ans[n - 1];
            for (int d : pdivs(n))
                ans[n] = min(ans[n], ans[d] + 2 * n / d);
        }
    }
    return ans;
}

//const vi32 C1 = costs1(256);

/*
vi32 costs2(int lim, int mid) {
    vi32 ans(lim);
    for (int n = 0; n < lim; n++) {
        if (n <= 1)
            ans[n] = C1[mid];
