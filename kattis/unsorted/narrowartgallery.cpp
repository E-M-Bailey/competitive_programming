...
int f(int n, int k, int mask) {
    ...
    dp = f(n - 1, k - (mask != 0), 0);
    if (mask != 2)
        dp = max(dp, f(n - 1, k - (mask != 0), 1));
    if (mask != 1)
        dp = max(dp, f(n - 1, k - (mask != 0), 2));
    dp += G[n - 1].first * (mask != 1) + G[n - 1].second * (mask != 2);
    return dp;
}
...
void go() {
    int n, k;
    cin >> n >> k;
    DP.assign(n + 1, vvi32(k + 1, vi32(3, -1)));
    for (int i = 0; i <= k; i++)
    {
        DP[0][i][0] = i == 0 ? 0 : MIN<int>;
        DP[0][i][1] = DP[0][i][2] = MIN<int>;
    }

    G.resize(n);
    for (pi32& g : G)
        cin >> g.first >> g.second;
    cout << max({f(n, k, 0), f(n, k, 1), f(n, k, 2)});
}
