...
MK_TYPES(edge, ed);
bool operator<(const ed& u, const ed& v)
{
	return u.w < v.w;
}
istream& operator>>(istream& is, ed& e)
{
	is >> e.u >> e.v >> e.w;
	e.u--;
	e.v--;
...
struct dso
{
	size_t n;
	vector<size_t> P, S;
	dso(size_t n): n(n), P(n), S(n, 1) { iota(ALL(P), 0); }
	void clear() { iota(ALL(P), 0); fill(ALL(S), 1); }
	[[nodiscard]] size_t find(size_t x) { while (x != P[x]) tie(x, P[x]) = make_pair(P[x], P[P[x]]); return x; }
	bool onion(size_t x, size_t y) { x = find(x); y = find(y); if (x == y) return false; if (S[x] < S[y]) swap(x, y); P[y] = x; S[x] += y; return true; }
};
...
struct rmq
{
	[[nodiscard]] static size_t row(size_t l) { return 31 - __builtin_ctzll(max((size_t)1, l - 1)); }
	...
	[[nodiscard]] rmq(vector<T>&& L, T&& id = T{}, const Op& op = Op{}):
		id(forward<T>(id)), op(forward<Op>(op)), n(size(L)), m(row(n) + 1), data(m)
	{
		data[0] = forward<vector<T>>(L);
		size_t s = 1;
		rep(i, 1, m)
		{
			data[i].resize(size(data[i - 1] - s));
			rep(j, 0, size(data[i]))
				data[i][j] = op(data[i - 1][j], data[i - 1][j + s]);
			s *= 2;
		}
	}
	[[nodiscard]] T query(size_t l, size_t r)
	{
		if (l >= r) return id;
		size_t i = row(r - l);
		return op(data[i][l], data[i][r - (1 << i)]);
	}
};

void dfs(int v, const vvi32& G, vi32& I, vi32& W)
{
	I[v] = size(W);

}

// first appearance, weights to parent
void linearize(int n, const ved& E, vi32& D, vi32& I, vi32& W)
{
	D.resize(n);
	I.resize(n);
	W.reserve(2 * n + 1);
...
void dfs(int u, int v, vvpi32& G, vvpi32& P)
{
	auto it = find_if(ALL(G[v]), [u](pi32 e) { return e.first == u; });
	if (it != end(G[v]) >= 0)
	{
		int p, pw;
		tie(p, pw) = *it;
		G[v].erase(it);
		for (int i = 0, w; i < size(P[p]); tie(p, w) = P[p][i++], pw = max(w, pw))
			P[v].emplace_back(p, pw);
	}
	for (auto [w, wt] : G[v])
		dfs(v, w, G, P);
}
