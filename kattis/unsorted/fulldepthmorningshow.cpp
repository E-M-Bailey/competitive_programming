...
i64 R, S;
vi64 r, s, t, da, db, ua, ub;
vvpi64 G;

void dfs1(i64 u, i64 v) {
    r[v] = 1;
    s[v] = t[v];
    for (auto [w, wt] : G[v]) {
        if (w == u)
...
void dfs2(i64 u, i64 v, i64 wt) {
    if (u != v) {
        ua[v] = da[u] + ua[u] - da[v] + wt * (S - 2 * s[v]);
        ub[v] = db[u] + ub[u] - db[v] + wt * (R - 2 * r[v]);
    }
    for (auto [w, wt] : G[v]) {
        if (w != u)
            dfs2(v, w, wt);
...
void go() {
    ...
    dfs2(0, 0, 0);
    for (i64 i = 0; i < R; i++) {
        cout << da[i] + ua[i] + (db[i] + ub[i]) * t[i] << endl;
    }
}
