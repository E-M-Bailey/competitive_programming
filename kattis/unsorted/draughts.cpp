...
bool good(int n, char c = '#'){
    ...
    return (grid[n/10][n%10] == c);
}

int dfs(int n){
    int ans = 0;
    int dir = 22;
    if(n+dir < 100 && path[n][n+dir]){
        path[n][n+dir] = path[n+dir][n] = 0;
        ans = max(ans, 1+dfs(n+dir));
        path[n][n+dir] = path[n+dir][n] = 1;
    }
    if(n-dir >= 0 && path[n][n-dir]){
        path[n][n-dir] = path[n-dir][n] = 0;
        ans = max(ans, 1+dfs(n-dir));
        path[n][n-dir] = path[n-dir][n] = 1;
    }
...
void go(){
    ...
    for(int i = 0; i < 10; i++){
        for(int e = 0; e < 10; e++){
            int val = i*10 + e;
            if(grid[i][e] == 'B'){
                if(good(val-11) && good(val+11)){
                    path[val-11][val+11] = true;
                    path[val+11][val-11] = true;
                }
                if(good(val-9) && good(val+9)){
                    path[val-9][val+9] = true;
                    path[val+9][val-9] = true;
                }
            }
        }
    ...
    for(int i = 0; i < 10; i++){
        for(int e = 0; e < 10; e++){
            int val = i*10 + e;
            if(grid[i][e] == 'W'){
                if(good(val+11, 'B') && good(val+22)){
                    path[val][val+22] = true;
                    path[val+22][val] = true;
                }
                if(good(val-11, 'B') && good(val-22)){
                    path[val][val-22] = true;
                    path[val-22][val] = true;
                }
                if(good(val-9, 'B') && good(val-18)){
                    path[val][val-18] = true;
                    path[val-18][val] = true;
                }
                if(good(val+9, 'B') && good(val+18)){
                    path[val][val+18] = true;
                    path[val+18][val] = true;
                ...
                ans = max(ans, dfs(val));
                if(good(val+11, 'B') && good(val+22)){
                    path[val][val+22] = false;
                    path[val+22][val] = false;
                }
                if(good(val-11, 'B') && good(val-22)){
                    path[val][val-22] = false;
                    path[val-22][val] = false;
                ...
                if(good(val-9, 'B') && good(val-18)){
                    path[val][val-18] = false;
                    path[val-18][val] = false;
                }
                if(good(val+9, 'B') && good(val+18)){
                    path[val][val+18] = false;
                    path[val+18][val] = false;
