...
void go() {
	...
	const auto ndiv = [&](int i, int p) {
		...
		//cerr << "ndiv(" << i << ", " << p << ") = " << lo << endl;
		return lo;
	};

	vbl VS(maxv + 1, false);

	const auto fac = [&](int x) {
		vi32 ans;
		int hp;
		while (hp = HP[x]) {
            if (!VS[hp])
	...
	vvi32 DP(n + 1, vi32(k + 1, 0));
	fill(ALL(DP[n]), MAX<int>);
	//DP[n][0] = MAX<int>;
	for (int i = n - 1; i >= 0; i--) {
        //for (int l = 1; i <= k; l++)
            //DP[i][l] = max(DP[i][l], HP[V[i]])
		for (int p : fac(V[i])) {
			int nd = ndiv(i, p);
			for (int l = 1; l <= k; l++)
				DP[i][l] = max(DP[i][l], min(p, DP[nd][l - 1]));
		}
	}
	cout << DP[0][k];
}
