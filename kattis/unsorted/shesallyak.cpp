...
pi32 combine(pi32 l, pi32 r) {
	auto [la, lb] = l;
	auto [ra, rb] = r;
	if (la == -1 && ra == -1)
        return make_pair(-1, -1);
	array<int, 4> A{la, lb, ra, rb};
	sort(ALL(A), [](int a, int b) {return dp(a) > dp(b);});
	for (int i = 1; i < 4; i++)
		if (cc(A[i]) != cc(A[0]))
			return make_pair(A[0], A[i]);
}

struct Tree {
	typedef pi32 T;
	static constexpr T unit{-1, -1};
	T f(T a, T b) { return combine(a, b); } // (any associative fn)
	vector<T> s; int n;
...
void go() {
	...
	for (int i = 0; i < n; i++) {
		int c = C[i], s = S[i];
		auto [a, b] = ST.query(0, s);
        if (cc(a) != c)
            PR[i] = a;
        else
            PR[i] = b;
        DP[i] = dp(PR[i]) + 1;
        pi32 old = ST.query(s, s + 1);
        ST.update(s, combine(old, make_pair(i, -1)));
	}
