...
array<pi32, 4> DIR{pi32{-1, 0}, pi32{0, 1}, pi32{1, 0}, pi32{0, -1}};
unordered_map<char, int> CDIR{{'U', 0}, {'R', 1}, {'D', 2}, {'L', 3}};
...
void go() {
	...
	const auto idx1 = [r, c] (int i, int y, int x) {
		return (i * r + y) * c + x;
	};

	const auto idx3 = [r, c] (int i) {
		return make_tuple(i / (r * c), i / c % r, i % c);
	};

	const auto dest = [&](int y, int x, int d) {
		auto [dy, dx] = DIR[d];
		int y2 = y + dy, x2 = x + dx;
		if (GR[y2][x2])
			return make_pair(y, x);
		else
			return make_pair(y2, x2);
	};
	...
	for (int i = 0; i < n; i++) {
		for (int y = 1; y < r - 1; y++)
			for (int x = 1; x < c - 1; x++) {
				...
				int v = idx1(i, y, x);
				int d0 = CDIR[S[i]];
				auto [y0, x0] = dest(y, x, d0);
				int w0 = idx1(i + 1, y0, x0);
				G[v].emplace_back(w0, 0);
				if (tie(y0, x0) != tie(y, x))
					G[v].emplace_back(idx1(i + 1, y, x), 1);
			}
	}
	...
	for (int i = 0; i <= n; i++)
		for (int y = 1; y < r - 1; y++)
			for (int x = 1; x < c - 1; x++) {
				...
				int v = idx1(i, y, x);
				for (int d = 0; d < 4; d++) {
					auto [y2, x2] = dest(y, x, d);
					if (tie(y2, x2) != tie(y, x))
						G[v].emplace_back(idx1(i, y2, x2), 1);
				}
			}

	int s = idx1(0, sy, sx);//, t = idx1(n, ty, tx);
	...
	while (!DQ.empty()) {
		...
		for (auto [w, wt] : G[v]) {
			if (D[w] >= 0)
				continue;
			if (wt)
				DQ.emplace_back(w, d + 1);
			else
				DQ.emplace_front(w, d);
		}
	}
	...
	for (int i = 0; i <= n; i++) {
		...
		for (int y = 0; y < r; y++) {
			for (int x = 0; x < c; x++)
				cerr << (GR[y][x] ? '#' : char(D[idx1(i, y, x)] + '0'));
			cerr << endl;
		}
        cerr << endl;
	}
