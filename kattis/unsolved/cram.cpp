// Let lcp(i, j) be the length of the longest common prefix of the suffixes s[i...n-1] and s[j...n-1]. For example, the
// longest common prefix of bcabcd and bcd is bc with length 2, so if s = abcabcd, then lcp(1, 4) = 2.
//
// Let dp(i) be the minimum cost of s[0...i-1]. As a base case, s[0...-1] is the empty string, so its cost is dp(0) = 0.
// Otherwise, at position i, you can either use one character with a cost of 1 or use a backreference with a cost of 3.
// From position i, using a backreference to position j < i lets you skip any common prefix of s[j...n-1] and s[i...n-1],
// so you can skip up to lcp(j, i) characters.
// Therefore, for i < n, you can transition to i + 1 with a cost of 1 and to i + max[j < i](lcp(j, i)) with a cost of 3.
// If you can somehow precompute max[j < i](lcp(j, i)) for all i, this yields the answer in linear time.
//
// First, we'll precompute the suffix and lcp arrays for s. The suffix array A is the permutation of indices sorted by their
// suffixes. The lcp array L stores the lcp of adjacent pairs of suffixes in the suffix array. For s = abcabcd:
//
// i   A[i]   s[A[i]...n-1]   L[i]   LCP of s[A[i-1]...n-1], s[A[i]...n-1]   lcp(A[i], 4)
// 0   0      abcabcd         0                                              0
// 1   3      abcd            3      abc                                     0
// 2   1      bcabcd          0                                              2
// 3   4      bcd             2      bc                                      3
...
// 6   6      d               0                                              0
//
// It is possible to construct the suffix and lcp arrays in O(n). There are simpler O(n log(n)) and O(n log^2(n)) algorithms.
//
// The length of the LCP of two distinct suffixes is the minimum value of L between the two suffixes in the table. For
// example, the values of LCP between abcabcd and cabcd are 0, 3, 0, and 2, so their LCP's length is min(0, 3, 0, 2) = 0.
// This means lcp(A[j], A[i]) is nondecreasing as j moves towards i from either direction. Thus, the maximum value of
// lcp(A[j], A[i]) for j in some set will be achieved by the closest value of A[j] to A[i] in either direction. This
// means that to calculate lcp(A[i], A[j]), we can just build a structure which supports range minimum queries on L (such
// as a segment tree or a sparse table), and query it for the minimum value in L[min(i,j)...max(i,j)-1].
//
// First, compute the inverse permutation B of A ([1, 3, 5, 2, 4, 6, 7, 0] in the above example) let i range over its
// elements in order. This will iterate over indices in increasing order of A[i], so it will consider the suffixes in
// order of abcabcd, bcabcd, cabcd, ..., cd, d, and the empty string. As we iterate, we'll maintain some data structure
// which represents the suffixes starting before A[i], i.e. the rows j in the table such that A[j] < A[i]. First, we'll use
// the data structure to find the values of j such that A[j] is as close as possible to A[i] in either direction. The max
// of lcp(A[i], A[j]) is achieved at one of these two values of j. Then, we'll update the structure to include the suffix
// starting at A[i] to prepare for the next iteration. These operations can be handled in O(log(n)) time each using a simple
// ordered set (i.e. std::set, TreeSet, etc.), leading to a total time complexity of O(n log(n)).
//
// Challenge: This can be improved to O(n log(log(n))) by modifying only the function called "nearest". How?
//
// Challenge 2: This can be further improved to O(n a(n)), where a is the inverse Ackermann function, also by
//              modifying only the function called "nearest". How?
...
{
private:
	static int constexpr MAXN = 100'010;
	static int C[MAXN];
...
struct dso
{
	// left endpoint, right endpoint, parent, rank
	vector<int> L, R, P, Q;
	dso(int n) : L(n), R(n), P(n), Q(n)
	{
...
// Simulates the results of the following process:
// Start with the set {-1, n}, where each element of A is in [0, n). For each element a of A in order:
//     Add a to the set.
//     Report a's predecessor and successor, replacing nonexistent values with -1.
vector<pair<int, int>> nearest(int n, vector<int> const &A)
{
	...
	for (int a : A)
	{
		...
		ans.emplace_back(*prev(it), *next(it));
	}
	return ans;
}
...
int main()
{
	...
	getline(cin, str);
	int n = (int)str.size();
	auto [A, B, L] = suffix_array(begin(str), end(str));

	auto R = rmq(begin(L) + 1, end(L));
	auto N = nearest(n, B);
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (l >= 0)
			step = R.query(l, b);
		if (r < n)
			step = max(step, R.query(b, r));
		DP[i + step] = min(DP[i + step], DP[i] + 3);
	}
	cout << DP.back();
}
