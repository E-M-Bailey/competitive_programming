...
int cmp(u64 s, u64 x, u64 y, int d) {
	if (s == 0)
		return d;
	u64 a = x / s, b = y / s;
	//cerr << s << ' ' << x << ' ' << y << ' ' << a << ' ' << b << ' ' << flush << TBL[b][a] << endl;
	switch (TBL[b][a]) {
	case 0:
		break;
	case 1:
        tie(x, y) = make_pair(x - s, y - s);
		break;
    case 2:
        tie(x, y) = make_pair(y, 4 * s - 1 - x);
        d += 3;
        break;
    case 3:
        tie(x, y) = make_pair(4 * s - 1 - y, x);
        d += 1;
        break;
	}
	return cmp(s / 2, x, y, d);
}
...
void go() {
	...
	u64 s = 1, a = x / s, b = y / s;
	while (b >= TBL.size() || a >= TBL[b].size())
    {
        s *= 2;
        a = x / s;
        b = y / s;
    }
	cout << cmp(s, x, y, 0) % 4 << endl;
}
