...
bool const TEST_CASES = 0;
bool const DEBUG_FILE = 1;
bool const TC_NUMBERS = 0;
...
i64 m, S[2], s;
unique_ptr<node> ed;
i64 steps(node const *a, node const *b);

pair<i64, i64> repr(bool side, i64 pos)
{
	pos += side * S[0];
	return { pos % s, pos / s * 2 - side };
}
pair<bool, i64> rpos(i64 bck, i64 bpos)
{
	bool side = bpos % 2;
...
struct node
{
	...
	i64 bck, bpos;
	i64 haz = 0;
	// friend ostream &operator<<(ostream &os, node const &nd)
	// {
	// 	if (&nd == ed.get())
	// 		return os << "end";
	// 	auto [side, pos] = rpos(nd.bck, nd.bpos);
	// 	return os << side << ':' << pos << '[' << nd.bck << ',' << nd.bpos << ']' << '(' << nd.haz << ' ' << nd.val << ')';
	// }
	node(i64 b = 0, i64 bp = 0) : bck(b), bpos(bp)
	{
		fix();
		// cerr << " Constructing " << *this << endl;
	}
	void fix()
	{
...
i64 steps(node const *a, node const *b)
{
	...
	if (b == ed.get())
		bp = m;
	i64 pcur = ap + a->haz, popp = pcur + S[!as];
	if (popp >= bp)
		return 1;
	i64 scur = (bp - pcur + s - 1) / s * 2, sopp = (bp - popp + s - 1) / s * 2 + 1;
	// cerr << "  steps(" << *a << ", " << *b << "): " << bp << ' ' << pcur << ' ' << popp << ' ' << scur << ' ' << sopp << endl;
	return min(scur, sopp);
}
void link(node *u, node *v)
{
	// cerr << "  note: " << *u << ' ' << *v << endl;
	dassert(!connected(u, v));
	make_root(u);
	u->pp = v;
	u->par = v;
	u->fix();
	// cerr << " Linking " << *u << " -> " << *v << endl;
}
void cut(node *u, node *v)
{
	make_root(v); u->splay();
	dassert(v == (u->pp ? : u->c[0]));
	dassert(v == u->par);
	u->par = 0;
	if (u->pp)
		u->pp = 0;
...
unordered_map<i64, map<i64, unique_ptr<node>>> E;

// Add cloudless node if no node already present
node *node_at(bool side, i64 pos)
{
...
void add_cloud(bool side, i64 pos, i64 haz)
{
	// cerr << "Adding cloud " << side << ':' << pos << '(' << haz << ')' << endl;
	node *cur = node_at(side, pos), *nxt = node_at(!side, pos + haz + S[!side]);
	cur->haz = haz;

	relink(cur, nxt);
	// cerr << "Cloud added" << endl;
}
void del_cloud(bool side, i64 pos)
{
	// cerr << "Deleting cloud " << side << ':' << pos << endl;
	auto [bck, bpos] = repr(side, pos);
	auto &EE = E[bck];
	auto itnxt = EE.upper_bound(bpos);
