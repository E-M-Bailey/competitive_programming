...
EXPLANATION:
Let n <= 10000 be the size of the input and m = 10000 be the bound on size of the output excluding new lines in both
cases. One solution is to eagerly evaluate every string when reading the input.
...
to compute it. Thus, we can optimize the above algorithm by not parsing any strings which contain more than m
characters.

A expression for a variable can be evaluated in time proportional to that variable's length. By additionally stopping
early if and when that length exceeds m, the time complexity of evaluating a variable's expression is proportional to
the minimum of its length and m, which is O(m). Thus, the complexity of evaluating all m variables is O(m n), which is
fast enough.
*/
...
bool parse_expr(string &val, string::const_iterator &it, string::const_iterator last)
{
	...
	else // variable name
	{
		...
		// if the variable's value is longer than m, or the length of the new combined value is longer than m, return
		// false
