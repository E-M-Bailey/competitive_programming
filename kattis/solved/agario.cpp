...
finline NODISCARD static constexpr size_ref<C> sizer(C &container, Args &&... args) noexcept
{
	return size_ref<C>(container, static_cast<Args &&>(args)...);
}
...
struct p_dso
{
	...
	{
		return *prev(partition_point(ALL(N[i]), [t](node nd) { return nd.t <= t; }));
	}
	node &get_mut(int i)
	{
...
static inline void go(size_t)
{
	...
	rep(i, 0, n)
	{
		...
		{
			prv = sum;
			int t = prev(partition_point(ALL(TS), [sum](pi32 pr) { return pr.first <= sum; }))->second;
