...
vi32 BPADIR{6, 7};
vi32 BPDIR{2};
vi32 NDIR{8, 9, 10, 11, 12, 13, 14, 15};
vi32 BDIR{4, 5, 6, 7};
vi32 RDIR{0, 1, 2, 3};
vi32 QDIR{0, 1, 2, 3, 4, 5, 6, 7};
vi32 KDIR{0, 1, 2, 3, 4, 5, 6, 7};

struct pce {
	static unordered_map<char, Type> TYPES;
	Type type;
	bool black;
	friend istream& operator>>(istream& strm, pce& p) {
		char c;
		strm >> c;
		p.black = islower(c);
		c = toupper(c);
...
unordered_map<char, Type> pce::TYPES{
	{'.', BLANK},
	{'P', PAWN},
	{'N', KNIGHT},
	{'B', BISHOP},
...
typedef vector<pce> vpce;
typedef vector<vpce> vvpce;

vvpce board;

struct mve {
    pi32 A, B;
    friend ostream& operator<<(ostream& strm, mve m) {
        auto [i1, j1] = m.A;
        auto [i2, j2] = m.B;
        return strm << char(j1 + 'a') << char(8 - i1 + '0') << char(j2 + 'a') << char(8 - i2 + '0');
    }
};

constexpr bool oob(int i, int j) {
    return i < 0 || i >= 8 || j < 0 || j >= 8;
}

bool hasColor(const vvpce& board, int i, int j, bool black) {
    pce p = board[i][j];
    if (p.type == BLANK)
        return false;
    else
...
vpi32 attacking(const vvpce& board, int i, int j) {
    pce ap = board[i][j];
    vpi32 ans;
    switch (ap.type) {
    case PAWN:
        for (int d : (ap.black ? BPADIR : WPADIR)) {
            auto [di, dj] = DIR[d];
            int i2 = i + di, j2 = j + dj;
            if (!oob(i2, j2) && hasColor(board, i2, j2, !ap.black))
...
bool check(const vvpce& board, bool black) {

}

void go() {
	board.assign(8, vpce(8));
	for (vpce& row : board)
		for (pce& p : row)
			cin >> p;
}
