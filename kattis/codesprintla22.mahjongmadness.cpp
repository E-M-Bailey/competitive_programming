...
typedef array<int, 9> freq;

bool complete(freq H)
{
	freq H2;
	const auto rem = [&](int i, bool st)
	{
        if (st)
        {
            H2[i]--;
	...
	for (int i = 0; i < 9; i++)
	{
		...
		for (int mask = 0; mask < 16; mask++)
			for (int a = 0; a < (mask & 1 ? 7 : 9); a++)
				for (int b = a; b < (mask & 2 ? 7 : 9); b++)
					for (int c = b; c < (mask & 4 ? 7 : 9); c++)
						for (int d = c; d < (mask & 8 ? 7 : 9); d++)
						{
							H2 = H;
							rem(a, mask & 1);
							rem(b, mask & 2);
...
int eval(freq H, int x)
{
    return complete(H) ? 1 + H[x] : 0;
}
...
void go()
{
    ...
    cout << fixed << setprecision(7) << (long double)ans / 22;
}
