...
void go()
{
	...
	const auto d2 = [&](pi32 ab)
	{
		auto [ax, ay] = XY[ab.first];
		auto [bx, by] = XY[ab.second];
	...
	vpi32 C;
	unordered_map<pi32, int> CI;
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
		{
			CI[make_pair(i, j)] = C.size();
			C.emplace_back(i, j);
		}
	...
	for (int i = 0; i < n * n; i++)
	{
		...
		for (int a2 : G1[a])
		{
			int j = CI[make_pair(a2, b)];
			G[i].push_back(j);
		}
		for (int b2 : G1[b])
		{
			int j = CI[make_pair(a, b2)];
			G[i].push_back(j);
		}
	}

	const auto cmp = [&](int l, int r)
	{
		return d2(C[l]) > d2(C[r]);
	};
	...
	{
		auto [a, b] = C[i];
		I[i] = CI[make_pair(b, a)];
	}

	int l;
	cin >> l;
	vi32 Q(l);
	unordered_map<int, usi32> L;
	for (int i = 0; i < l; i++)
	{
		int a, b;
		cin >> a >> b;
		a--;
		b--;
		int idx = CI[make_pair(a, b)];
		Q[i] = idx;
		L[idx].insert(idx);
		L[I[idx]].insert(I[idx]);
	}

	unordered_map<int, long double> ans;
...
//		cerr << endl;
	...
	for (int q : Q)
		cout << fixed << setprecision(7) << ans[q] << '\n';
}
