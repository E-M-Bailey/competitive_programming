...
int ct02(i64 d1, i64 d2, i64 l) {
    ...
    vi32 pf2 = pfac(d2);
    usi32 pf2s(ALL(pf2));
    auto it = remove_if(ALL(pf), [&](int p) {return pf2s.find(p) == pf2s.end();});
    pf.resize(it - pf.begin());
    int m = pf.size();
    int ans = 0;
    for (int mask = 1; mask < (1 << m); mask++) {
        int d = 1;
...
array<i64, 3> cmp(i64 da, i64 db, i64 l) {
    i64 ac = da == 0 ? l - 1 : da == 1 ? 0 : ct0(da, l);
    i64 bc = db == 0 ? l - 1 : db == 1 ? 0 : ct0(db, l);
    i64 uc = da == 0 ? bc : db == 0 ? ac : (da == 1 || db == 1) ? 0 : ct02(da, db, l);
    if (da > 1 && db > 1 && ct02(da, db, l) != ct02(db, da, l))
        cerr << "help! " << da << ' ' << db << ' ' << l << endl;
    i64 a = l - ac, b = l - bc, u = l - uc;
    i64 i = a + b - u;
    cerr << da << ' ' << db << ' ' << l << flush;
    cerr << ' ' << ac << ' ' << bc << ' ' << uc << ' ' << l - uc - i << ' ' << i << endl;
    return {uc, l - uc - i, i};
}
