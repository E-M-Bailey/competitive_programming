...
template<bool Rec>
bool eval(map<int, int> P, int d) {
    vi32 A;
    int o = 0;
    //cerr << d << ":\n";
    while (!P.empty()) {
        auto it = P.upper_bound(abs(o) + d);
        //cerr << o << ' ' << it->first << endl;
        if (it == P.begin())
            return false;
        it--;
        int a = o > 0 ? -it->first : it->first;
        if (--it->second == 0)
            P.erase(it);
        o += a;
        if (Rec)
...
void go() {
    ...
    while ((cin >> n), n) {
        ...
        for (int& p : P)
            cin >> p;
        sort(ALL(P));
        int lo = max(P[0], (P[n-1] + 1) / 2), hi = MAX<int> / 2;
        ans.reserve(n);
        map<int, int> PM;
        for (int p : P)
            PM[p]++;
