...
pi64 dfs(const vvi32& C, const vi64& F, int i)
{
    int M = C[i].size();
    if (M == 0)
        return make_pair(F[i], F[i]);
    vi64 CV(M), CP(M);
    i64 v = 0;
    for (int j = 0; j < M; j++)
    {
        int c = C[i][j];
        tie(CV[j], CP[j]) = dfs(C, F, c);
        v += CV[j];
    }
    i64 mp = *min_element(ALL(CP));
    i64 p = max(mp, F[i]);
    v = v - mp + p;
...
void go()
{
    ...
    vi32 P(N + 1);
    F.front() = 0;
    copy_n(istream_iterator<i64>(cin), N, F.begin() + 1);
    copy_n(istream_iterator<int>(cin), N, P.begin() + 1);
    vvi32 C(N + 1);
    for (int i = 1; i <= N; i++)
        C[P[i]].push_back(i);

    cout << dfs(C, F, 0).first << endl;
}
