...
void go()
{
	...
	G[0][0] = 0;
	{
		...
		for (int x = 1; x < n * n; x++)
		{
			int i2 = i + DIR[d].first, j2 = j + DIR[d].second;
			if (i2 >= n || i2 < 0 || j2 >= n || j2 < 0 || G[i2][j2] != -1)
			{
				d = (d + 1) % 4;
				i2 = i + DIR[d].first, j2 = j + DIR[d].second;
			}
			i = i2;
			j = j2;
			G[i][j] = x;
	...
	if (k % 2 == 1 || k < DP[0].first || k > DP[0].second)
	{
	...
	for (int i = k - 1; i >= 0; i--)
	{
		for (int j : next[path.back()])
			if (DP[j].first <= i && i <= DP[j].second)
			{
				if (j > path.back() + 1)
					SC.emplace_back(path.back(), j);
				path.push_back(j);
				break;
			}
	}
	cout << SC.size() << endl;
	for (pi32 sc : SC)
		cout << sc.first + 1 << ' ' << sc.second + 1 << endl;
}
