...
int d2(pi32 a, pi32 b)
{
	pi32 delta(a.first - b.first, a.second - b.second);
	return delta.first * delta.first + delta.second * delta.second;
}
...
void go()
{
	...
	do
	{
		...
		if (found)
		{
			cout << "POSSIBLE" << endl;
            for (int i = 0; i < n; i++)
                cout << P[i] + 1 << ' ' << O[i] + 1 << endl;
		}
	}
	while (!found && next_permutation(ALL(P)));
	if (!found)
	{
