...
struct tree
{
	...
	void dfs2(int v, int sup, i64 dup)
	{
		for (int w : C[v])
			dfs2(w, sup + S[v] - S[w], dup + sup + D[v] - D[w] + S[v] - 2 * S[w]);
		D[v] += dup;
	}

	tree(int N): n(N), P(n, -1), C(n), S(n, 1), D(n)
	{
		rep(i, 0, n - 1)
		{
			cin >> P[i];
...
void go()
{
	...
	while (c--)
	{
		...
		a--;
		b--;
		long double ans = (W.td + E.td + w * e + W.D[a] * e + E.D[b] * w) * fac;
