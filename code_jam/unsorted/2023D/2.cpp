...
template<class T> [[nodiscard]] constexpr T rup2(T x) noexcept
{
	if (x-- == 0) return 1;
...
pair<vector<int>, vector<int>> suffix_array_construction(const string& a, const string& b)
{
	string s = a + '!' + b + '$';
	vector<int> sorted_shifts = sort_cyclic_shifts(s);
	return make_pair(sorted_shifts, lcp_construction(s, sorted_shifts));
}
...
struct tree
{
	...
	int n; vector<T> s;
	tree(int N): n(rup2(N)), s(2 * n, unit) {}
	void upd(int i, int t)
	{
		//cerr << "updating " << i << ' ' << t << endl;
		for (s[i += n] = t; i /= 2;)
			s[i] = f(s[i * 2], s[i * 2 + 1]);
	}
	int first(int l, int r, int lim, int i, int nl, int nr)
	{
		l = max(l, nl);
		r = min(r, nr);
		if (l >= r || s[i] >= lim)
		...
		if (nl + 1 == nr)
		{
			//cerr << "found " << l << ' ' << r << ' ' << lim << ' ' << i << ' ' << s[i] << ' ' << n << endl;
			return l;
		}
		int nm = (nl + nr) / 2;
		int ans = first(l, r, lim, i * 2, nl, nm);
		if (ans < 0)
			ans = first(l, r, lim, i * 2 + 1, nm, nr);
		return ans;
	}
	int first(int l, int r, int lim)
	{
		int ans = first(l, r, lim, 1, 0, n);
		//cerr << "first(" << l << ',' << r << ',' << lim << ") = " << ans << endl;
		return ans;
	}
	int last(int l, int r, int lim, int i, int nl, int nr)
	{
		l = max(l, nl);
		r = min(r, nr);
		if (l >= r || s[i] >= lim)
		...
		if (nl + 1 == nr)
			return l;
		int nm = (nl + nr) / 2;
		int ans = last(l, r, lim, i * 2 + 1, nm, nr);
		if (ans < 0)
			ans = last(l, r, lim, i * 2, nl, nm);
		return ans;
	}
	int last(int l, int r, int lim)
	{
		return last(l, r, lim, 1, 0, n);
	}
};
...
void go()
{
	...
	cin >> A >> B >> m;
	vi32 p, lcp;
	tie(p, lcp) = suffix_array_construction(A, B);
	int n = p.size();
	vi32 q(n);
	rep(i, 0, n)
		q[p[i]] = i;
	...
	while (m--)
	{
		...
		{
			int k = (lo + hi + 1) / 2;
			int x = T.last(0, q[b], a - k + 1);
			int y = T.first(q[b], n, a - k + 1);
			int l = 0;
			//cerr << "query " << x << ' ' << q[b] << ' ' << y << endl;
			if (x >= 0)
				l = max(l, rmq.query(x, q[b]));
			if (y >= 0)
				l = max(l, rmq.query(q[b], y));
			//cerr << " len " << l << endl;
			if (l >= k)
				lo = k;
			else
