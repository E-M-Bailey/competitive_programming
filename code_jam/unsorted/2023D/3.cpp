...
void go()
{
	...
	cin >> r >> c;
	array<pi32, 26> S, T;
	fill(ALL(S), make_pair(-1, -1));
	fill(ALL(T), make_pair(-1, -1));
	vstr8 grid(r);
	rep(i, 0, r)
	{
		cin >> grid[i];
	...
	const auto wall = [&](int i, int j) -> bool
	{
		return i < 0 || i >= r || j < 0 || j >= c || grid[i][j] == '#';
	};

	const auto dir = [&](int i, int j)->array<pi32, 4>
	{
		return array<pi32, 4>
		{
			wall(i + 1, j) ? pi32(i + 1, j) : pi32(i, j),
				wall(i - 1, j) ? pi32(i - 1, j) : pi32(i, j),
				wall(i, j + 1) ? pi32(i, j + 1) : pi32(i, j),
				wall(i, j - 1) ? pi32(i, j - 1) : pi32(i, j),
		};
	};
	...
	for (auto [ti, tj] : T)
	{
		if (ti < 0)
			continue;
		vvbl UD(r, vbl(c, true));
		vvbl LR(r, vbl(c, true));
