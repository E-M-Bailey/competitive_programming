...
constexpr mnt zero = 0, one = 1, two = 2, rat = 4;
struct ent;
TDEFS1(ent);
TDEFS2(ent*, hent);
struct ent
{
	...
	int s;
	bool valid;
	constexpr static mnt H(const ent* e) { return e ? e->h : zero; }
	constexpr static mnt P(const ent* e) { return e ? e->p : one; }
	constexpr static int S(const ent* e) { return e ? e->s : 0; }
	constexpr ent(ent* l, ent* r):
		l(l), r(r)
	{
		mnt hl = H(l);
		mnt hr = (H(r) + P(r)) * two;
		//cerr << hex << H(l) << ' ' << H(r) << ' ' << P(r) << ' ' << hl * P(r) * rat + hr << dec << endl;
		if (valid = make_pair(hl, S(l)) <= make_pair(hr, S(r) + 1))
		{
			h = hl * P(r) * rat + hr;
			p = P(l) * P(r) * rat;
			s = S(l) + S(r) + 1;
	...
	/*
	void hashes(ostream& os, int& t) const
	{
		int x = t;
		for (const ent* ptr = this; ptr; ptr = ptr->l)
		{
			int y = ++t;
			if (ptr->r)
				ptr->r->hashes(os, t);
	...
	void print(ostream& os, int& t) const
	{
		int x = t;
		for (const ent* ptr = this; ptr; ptr = ptr->l)
		{
...
constexpr auto cmp = [](const ent& l, const ent& r)
{
	return make_pair(l.h, l.s) < make_pair(r.h, r.s);
};
dent AL;
vvhent H;
vhent S;
void go()
{
	...
	for (int i = 1; i < 100; i++)
	{
		...
		cerr << i << endl;
		vhent N;
		// D -> D(D) is unambiguous
		for (int j = 0, k = i - 1; j < i; j++, k--)
		{
			rep(a, 0, size(H[j])) rep(b, 0, size(H[k]))
			{
				AL.emplace_back(H[j][a], H[k][b]);
				hent e = &AL.back();
				if (e->valid)
				{
					N.push_back(e);
		...
		ranges::sort(ALL(S), cmp, [](const ent* e) -> const ent& { return *e; });

		total += cur;
		cerr << "Checked size " << i << ": " << cur << " this time, " << total << " total." << endl;

		rep(j, 1, size(S))
		{
			hent a = S[j - 1], b = S[j];
			if (!a || a->h != b->h || a->s != b->s) continue;
			cerr << "Collision with hash " << hex << b->h << dec << '!' << endl;
			ent L(nullptr, a);
			ent T(&L, b);
