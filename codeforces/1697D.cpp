...
void go() {
    ...
    for (int i = 0; i < n; i++) {
        int total = getUnique(0, i);
        //cerr << "IL: " << IL.size() << endl;
        if (total > IL.size()) {
            char ch = ans[i] = getChar(i);
            L.emplace(ch, i);
            IL.emplace(i, ch);
        }
        else {
            vector<pair<int, char>> ILV(ALL(IL));
            int lo = 0, hi = ILV.size() - 1;
            while (lo < hi) {
                int mid = (lo + hi + 1) / 2;
                int ct = ILV.size() - mid;
                int u = getUnique(ILV[mid].first, i);
                if (ct == u)
                    lo = mid;
                else
                    hi = mid - 1;
            }
            char ch = ans[i] = ILV[lo].second;
            IL.erase(L[ch]);
            L[ch] = i;
            IL[i] = ch;
        }
