...
void go() {
    ...
    for (i64& a : A)
        cin >> a;
    const auto eval = [&](int l, int r) {
        if (l == 0 && r == n - 1 || l > r) return MIN<i64>;
        i64 m1 = MAX<i64>, M1 = MIN<i64>;
        if (l > 0) {
            m1 = min(m1, *min_element(A.begin(), A.begin() + l));
            M1 = max(M1, *max_element(A.begin(), A.begin() + l));
        }
        if (r < n - 1) {
            m1 = min(m1, *min_element(A.begin() + r + 1, A.end()));
            M1 = max(M1, *max_element(A.begin() + r + 1, A.end()));
        }
        i64 m2 = *min_element(A.begin() + l, A.begin() + r + 1);
        i64 M2 = *max_element(A.begin() + l, A.begin() + r + 1);
        i64 ans = M1 - m1 + M2 - m2;
        //cerr << l << ' ' << r << ": " << M1 - m1 << ' ' << M2 - m2 << endl;
        return ans;
    };
    i64 a = min_element(ALL(A)) - A.begin(), b = max_element(ALL(A)) - A.begin();
    if (a > b)
        swap(a, b);
    cout << max({eval(a, b), eval(a + 1, b), eval(a, b - 1), eval(a + 1, b - 1)}) << endl;
}
