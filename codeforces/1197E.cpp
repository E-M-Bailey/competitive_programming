...
struct bezout {
	...
	inline bezout(const T& l, const T& r) {
		T a = abs(l), b = abs(r);
		vector<pair<T, T>> S;
		while (b != 0) {
			S.emplace_back(a, b);
...
template<typename T>
constexpr T pos_mod(const T& n, const T& mod) {
	return is_unsigned_v<T> ? n % mod : (n % mod + mod) % mod;
}
...
struct mod_int {
	...
	constexpr mod_int(const T& value) :
		value(pos_mod(value, MOD))
	{}

	inline mod_int& operator=(const T& value) {
		this->value = pos_mod(value, MOD);
		return value;
	}

	inline mod_int& operator=(T&& value) noexcept {
		this->value = pos_mod(value, MOD);
		return *this;
	}
...
struct mod_int<T, 0> {
	...
	explicit
	inline mod_int(const T& mod) :
		MOD(mod),
		value(0)
	{}

	constexpr mod_int(const T& mod, const T& value) :
		MOD(mod),
...
template<typename T, uintmax_t M>
constexpr mod_int<T, M> operator+(const mod_int<T, M>& l) {
	return l;
}

template<typename T, uintmax_t M>
constexpr mod_int<T, M> operator-(mod_int<T, M> l) {
	l.value = l.mod() - l.value;
	return l;
}
...
template<typename T, uintmax_t M>
constexpr mod_int<T, M>& operator+=(mod_int<T, M>& l, const T& r) {
	if constexpr (M > 0)
...
template<typename T, uintmax_t M>
constexpr mod_int<T, M> operator+(const T& l, mod_int<T, M> r) {
	r += l;
	return r;
}
...
template<typename T, uintmax_t M>
constexpr mod_int<T, M> operator/(const T& l, const mod_int<T, M>& r) {
	return l * inv(r);
}
...
inline istream& operator>>(istream& strm, mod_int<T, M>& n) {
	strm >> n.value;
	n.value = pos_mod(n.value, n.mod() + n.mod()) % n.mod();
	return strm;
}
...
typedef mod_int<i64, MOD> mint;
typedef vector<mint> vmint;

void go() {
	const int MASK = uniform_int_distribution<int>(MIN<int>, MAX<int>)(randy);
	...
	int maxin = 0;
	for (pi32& a : A) {
		a.first = CC[a.first ^ MASK];
		a.second = CC[a.second ^ MASK];
		G[a.first].emplace_back(a.second, 0);
		maxin = max(maxin, a.first);
	}
	for (int i = maxin + 1; i < m; i++)
        G[i].emplace_back(m, 0);
