...
st dfs(int u, int v) {
    ...
    vmint pNM(n + 1, 1), sNM(n + 1, 1), pL(n + 1, 1), sL(n + 1, 1);
    //mint PL = 1;
    mint allNM = 1, allMLNM = 1;
    for (int i = 0; i < n; i++) {
        pNM[i + 1] = pNM[i] * C[i].nm;
        //PL *= C[i].l();
        pL[i + 1] = pL[i] * C[i].l();
    }
    for (int i = n - 1; i >= 0; i--) {
        sNM[i] = sNM[i + 1] * C[i].nm;
        sL[i] = sL[i + 1] * C[i].l();
    }
    for (auto [ml, mi, nm] : C) {
        allNM *= nm;
    ...
    mint ML = 0, MI = 0, NM = 0, x = 0;
    for (int i = 0; i < n; i++) {
        auto [ml, mi, nm] = C[i];
        auto m = C[i].m(), l = C[i].l();
        //if (l.value == 0)
        //    exit(-1);
        NM += x * m * sL[i + 1];
        x = x * l + m * pL[i];
    ...
    MI -= ML;
    ML += pNM[n];
    //cerr << "dfs(" << u + 1 << ", " << v + 1 << ") = {" << ML << ", " << MI << ", " << NM << "}" << endl;
    return {ML, MI, NM};
}
