...
int a(int i) {
	return i < 0 ? INT_MAX : A[i];
}

struct LIS {
	int j = -1;
	int ai = -1, bi = -1; // Where are A[i], B[j] used?
	vpi32 S;

	pi32 get(int l) const {
		return l < sz(S) ? S[l] : pi32(-1, -1);
	}
	void set(int l, int i, int j) {
		while (l >= sz(S))
			S.emplace_back(-1, -1);
		S[l] = pi32(i, j);
	}
	...
	void upd(int i, const LIS& L) {
		...
		} else if (L.ai >= 0 && A[i] < a(get(L.ai).first)) {
			ai = L.ai;
			if (ai == bi)
				bi = -1;
			auto [li, lj] = L.get(ai);
...
void go() {
	...
	auto [i, j] = DP.back().S.back();
	vi32 ans;
	ans.reserve(sz(DP.back().S));
	while (i >= 0) {
		ans.push_back(A[i]);
		tie(i, j) = P[i][j];
	}
	cout << sz(ans) << endl;
	for (; sz(ans); ans.pop_back())
		cout << ans.back() << ' ';
}
