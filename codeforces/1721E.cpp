...
template<i64 M>
struct hashDeque {
	typedef mod_int<i64, M> mint;
	dc8 s;
	mint v = 0, p26 = 1, i26 = mint(1) / mint(26);
	...
	friend bool operator==(const hashDeque& l, const hashDeque& r) {
		return l.size() == r.size() && l.v == r.v;
	}
	friend bool operator!=(const hashDeque& l, const hashDeque& r) {
		return l.size() != r.size() || l.v != r.v;
	}
...
void go() {
	...
	hashDeque<1000000007ll> FA, BA;
	hashDeque<998244353ll> FB, BB;
	for (int k = 1; k < n; k++) {
		FA.push_back(s[k - 1]);
		FB.push_back(s[k - 1]);
		BA.push_front(s[n - k]);
		BB.push_front(s[n - k]);
		if (FA != BA || FB != BB) {
            cerr << s.substr(0, k) << ' ' << s.substr(n - k, k) << endl;
			continue;
		}
		for (int l = 1; l <= 10 && k + l <= n; l++) {
            if (k + l <= n)
                M[s.substr(k, l)][0] = k + l;
            else
                M[s.substr(k)][n - k] = k + l;
		}
	}

	const auto query = [&](string t) {
        int m = t.size();
        int ans = 0;
        for (int o = 0; o < m; o++) {
            string pr = t.substr(0, m - o), su = t.substr(o);
            if (pr == su)
                ans = max(ans, M[t.substr(0, o)][o]);
        }
        return ans;
	};
