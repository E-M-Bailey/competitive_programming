...
i64 cmp(i64 sum, di64& A) {
    vi64 L, LZ, R, RZ;
    i64 ls = 0, rs = 0, lz = 0, rz = 0;
    do {
        i64 l = A.front(), r = A.back();
        if (ls + l <= rs + r) {
            A.pop_front();
            ls += l;
            lz = 0;
...
void go() {
    ...
    while (L.size() > M.back().first) {
        A.push_front(L.back());
        L.pop_back();
        LZ.pop_back();
    }
    while (R.size() > M.back().second) {
        A.push_back(R.back());
        R.pop_back();
        RZ.pop_back();
    }
    i64 c;
    if (A.empty()) {
        i64 m = LZ.back() + RZ.back();
        c = (m + 2) * (m + 1) / 2;
    }
    else
        c = (LZ.back() + 1) * (RZ.back() + 1);
    c %= MOD;
    i64 ans = c;
    //LZ.pop_back();
    //RZ.pop_back();
    ...
    while (!M.empty()) {
        ...
        c = (LZ.back() + 1) * (RZ.back() + 1) % MOD;
        c++;
        cerr << i << ' ' << j << ' ' << L.size() << ' ' << R.size() << ' ' << LZ.size() << ' ' << RZ.size() << ' ' << c << endl;
        //if (i != 0)
        //    c++;
        ans = c * ans % MOD;
    }
