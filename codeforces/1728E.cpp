...
i64 cdiv(i64 a, i64 b) {
	return a >= 0 ? (a + b - 1) / b : -fdiv(-a, b);
}
i64 fdiv(i64 a, i64 b) {
    return a >= 0 ? a / b : -cdiv(-a, b);
}
...
void go() {
	...
	while (m--) {
        ...
        //cerr << endl;
        while (kLo < kHi) {
            i64 k1 = fdiv(kLo + kHi, 2), k2 = k1 + 1;
            i64 x1 = (a + k1 * y) * xg, x2 = (a + k2 * y) * xg;
            //cerr << ' ' << kLo << ' ' << kHi << ' ' << k1 << ' ' << k2 << endl;
            //if (min(x1, x2) < 0 || max(x1, x2) > n)
            //    cerr << ' ' << kLo << ' ' << kHi << endl;
            i64 f1 = P[x1], f2 = P[x2];
            if (f1 <= f2)
                kLo = k2;
        ...
        //cerr << kLo << ' ' << (a + kLo * y) * x << ' ' << (b - kLo * x) * y << endl;
        i64 xf = (a + kLo * y) * xg;
        cout << B + P[xf] << '\n';
	}
}
