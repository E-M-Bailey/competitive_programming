...
void go()
{
	...
	if (*max_element(begin(A), end(A)) == 1)
	{
		cout << "1 1\n";
		return;
	}
	...
	i64 prod = (i64)accumulate(begin(A), end(A), (i128)1, [s = 2 * (i128)sum](i128 x, i128 y) { return min(s, x * y); });
	...
	rep(i, 0, m)
		rep(j, i, m)
	{
		i64 cur = P[I[j] + 1] / P[I[i]] - S[I[j] + 1] + S[I[i]];
		if (cur > best)
		{
			best = cur;
			l = I[i];
