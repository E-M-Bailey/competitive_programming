...
struct tree
{
	...
	vector<bool> L;
	tree(string const &s, vector<int> const &A) : n(rup2((int)s.size())), S(2 * n), L(2 * n)
	{
		for (int i = 0; i < (int)s.size(); i++)
			(s[i] == '1' ? S[i + n].second : S[i + n].first) = A[i];
		for (int i = n - 1; i > 0; i--)
		{
			S[i].first = S[i * 2].first ^ S[i * 2 + 1].first;
			S[i].second = S[i * 2].second ^ S[i * 2 + 1].second;
		}
	}
	...
	void pd(int i)
	{
		if (L[i])
		{
			...
			if (i < n)
			{
				swap(S[i * 2].first, S[i * 2].second);
				swap(S[i * 2 + 1].first, S[i * 2 + 1].second);
				L[i * 2] = !L[i * 2];
				L[i * 2 + 1] = !L[i * 2 + 1];
			}
		}
	...
	pair<int, int> query(int lo, int hi, int nlo, int nhi, int i)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
		...
		if (lo == nlo && hi == nhi)
			return S[i];
		int mid = (nlo + nhi) / 2;
		auto [a0, a1] = query(lo, hi, nlo, mid, i * 2);
		auto [b0, b1] = query(lo, hi, mid, nhi, i * 2 + 1);
		return { a0 ^ b0, a1 ^ b1 };
	}
	pair<int, int> query(int lo, int hi)
	{
		return query(lo, hi, 0, n, 1);
	}

	void update(int lo, int hi, int nlo, int nhi, int i)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
		...
		pd(i);
		int mid = (nlo + nhi) / 2;
		update(lo, hi, nlo, mid, i * 2);
		update(lo, hi, mid, nhi, i * 2 + 1);
		S[i].first = S[i * 2].first ^ S[i * 2 + 1].first;
		S[i].second = S[i * 2].second ^ S[i * 2 + 1].second;
	}
