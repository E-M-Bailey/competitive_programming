...
i64 constexpr P = 998244353;

/*
i64 constexpr R = 1ll << 32;
i64 constexpr R2 = R % P * R % P;
i64 constexpr M = (bezout<i64>(R - P, R).x % R + R);

i64 reduce(i64 x)
{
	return ((x * M & R - 1) * P + x) >> 32;
}
...
template<i64 a>
constexpr i64 powmod(i64 b)
{
	i64 ans = 1;
	for (i64 x = a; b; x = x * x % P, b /= 2)
		if (b % 2)
			ans = ans * x % P;
	return ans;
}

constexpr i64 powmod(i64 a, i64 b)
{
	i64 ans = 1;
	for (; b; a = a * a % P, b /= 2)
		if (b % 2)
...
constexpr i64 invmod(i64 a)
{
	return powmod(a, P - 2);
}

template<i64 a>
constexpr i64 invmod()
{
	return powmod<2>(P - 2);
}

constexpr i64 I2 = invmod<2>();
...
void go()
{
	...
	if (x == 0)
	{
		...
		{
			i64 ans = 1, cur = P - 1, p2k = powmod<2>(k);
			rep(i, 0, n)
			{
				ans = ans * (p2k + cur) % P;
				cur += cur;
	...
	{
		i64 s = 1, acc = 1, ans = 1, p2m = 1, p2n = P - powmod<2>(n), p2k = P - powmod<2>(k);
		rep(m, 1, k)
		{
			p2m += p2m;
			if (ans >= P) ans -= P;
			...
			p2k = p2k * I2 % P;
			i64 den = p2m - 1;
			s = s * (p2n + 1) % P * (p2k + 1) % P * p2m % P;
			ans = (ans * den + s) % P;
			acc = acc * den % P;
			p2n = p2n * I2 % P;
		}
		cout << ans * invmod(acc) % P << '\n';
	}
