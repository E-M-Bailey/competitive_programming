...
// sum, max prefix sum, max suffix sum, !answer
tuple<i64, i64, i64, bool> solve(vi32 const &A, map<int, int> &F, int l, int r)
{
	if (l == r)
		return { 0, 0, 0, false };
	int m, mx;
	...
	auto [ls, lps, lss, lans] = solve(A, F, l, m);
	auto [rs, rps, rss, rans] = solve(A, F2, m + 1, r);
	return
	{
		ls + mx + rs,
		max(lps, ls + mx + rps),
		max(rss, lss + mx + rs),
		lans || rans || lss > 0 || rps > 0
	};
}
...
void go()
{
	...
	for (int a : A)
		F[a]++;
	auto [s, ps, ss, ans] = solve(A, F, 0, n);
	cout << (ans ? "NO\n" : "YES\n");
}
