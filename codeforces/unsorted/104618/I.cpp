...
struct frac
{
	...
	{
		return (i64)a.n * b.d < (i64)b.n * a.d;
	}
	friend bool operator==(frac const &a, frac const &b)
	{
		return (i64)a.n * b.d == (i64)b.n * a.d;
...
struct Node
{
	...
	struct L { int val; bool set; };
	const T tneut = 0;     // neutral elements
	const L lneut{ 0, false };
...
void go()
{
	...
	sort(ALL(SL));
	SL.erase(unique(ALL(SL)), end(SL));
	sort(ALL(R2));
	R2.erase(unique(ALL(R2)), end(R2));
	int s = (int)SL.size(), r = (int)R2.size();

	A.resize(s);
	for (auto [sl, r2, b] : S)
	{
		int i = (int)(lower_bound(ALL(SL), sl) - begin(SL)), j = (int)(lower_bound(ALL(R2), r2) - begin(R2));
		A[i].emplace_back(j, b);
	}
	Node *tr = new Node(0, r);
