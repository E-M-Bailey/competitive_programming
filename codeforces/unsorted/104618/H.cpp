...
// https://cses.fi/paste/7b5c0925b9926d5f640805/

struct mNode
{
	typedef int T; // data type
	typedef int L;
	const T tneut = MAX<int> / 2;     // neutral elements
	const L lneut = MAX<int> / 2;
	...
	mNode(int lo, int hi) :lo(lo), hi(hi) {} // Large interval of tneut
	mNode(vector<T> &v, int lo, int hi) : lo(lo), hi(hi)
	{
		if (lo + 1 < hi)
		{
			int mid = lo + (hi - lo) / 2;
			l = new mNode(v, lo, mid); r = new mNode(v, mid, hi);
			val = f(l->val, r->val);
		}
		else val = v[lo];
	}
	...
	void push()
	{
		...
		{
			int mid = lo + (hi - lo) / 2;
			l = new mNode(lo, mid), r = new mNode(mid, hi);
		}
...
struct MNode
{
	typedef int T; // data type
	typedef int L;
	const T tneut = MIN<int> / 2;     // neutral elements
	const L lneut = MIN<int> / 2;
...
void go()
{
	...
	vi32 def(1000010);
	iota(ALL(def), 0);
	mNode *t1 = new mNode(def, 0, 1000010);
	MNode *t2 = new MNode(def, 0, 1000010);
	vi32 P(n);
	cin >> P;
	sort(ALL(P));
	for (auto &p : P)
	...
	for (auto [h, l, r] : S)
	{
		int lo = t1->query(l, r), hi = t2->query(l, r);
		t1->upd(l, r, lo);
		t2->upd(l, r, hi);
		//cerr << l << ' ' << r << ' ' << h << ' ' << lo << ' ' << hi << ' ' << (int)(upper_bound(ALL(P), hi) - lower_bound(ALL(P), lo)) << endl;
		ans = max(ans, (int)(upper_bound(ALL(P), hi) - lower_bound(ALL(P), lo)));
	}
	cout << ans;
}
