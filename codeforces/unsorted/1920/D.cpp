...
i64 constexpr LIM = 1'000'000'000'000'000'000;
...
void go()
{
	...
	while (n--)
	{
		...
		else
		{
			if (!E.back().A.empty())
			{
				E.push_back({ {}, E.back().siz });
			}
			i64 siz2 = (i64)(E.back().siz * (1 + x));
			//cerr << "- " << siz2 / (1 + x) << ' ' << E.back().siz << endl;
			if (siz2 > LIM || siz2 / (1 + x) != E.back().siz)
			{
				E.back().siz = LIM;
				continue;
			}
	...
	rrep(i, 0, (int)size(E))
	{
		rep(j, 0, q)
		{
			...
			if (Q[j] < 0)
				continue;
			i64 idx = Q[j] - (E[i].siz - E[i].A.size());
			// cerr << i << ' ' << Q[j] << ' ' << E[i].siz << ' ' << idx << endl;
			if (idx >= 0)
			{
				ans[j] = E[i].A[idx];
				Q[j] = -1;
