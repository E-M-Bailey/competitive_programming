...
void go()
{
	...
	vvi32 D(n, vi32(m, -1));
	vvbl V(n, vbl(m, false));
	queue<tuple<int, int, int>> L;
	rep(i, 0, n)
		rep(j, 0, m)
		if (G[i][j] == 'v')
			L.emplace(i, j, 0);
	while (!L.empty())
	{
		...
		{
			int i2 = di + i, j2 = dj + j;
			if (i2 < 0 || i2 >= n || j2 < 0 || j2 >= m || G[i2][j2] == '#' || V[i2][j2])
				continue;
			L.emplace(i2, j2, k + 1);
		}
	}

	auto const works = [&](int i, int j, int k)
		{
			vpi32 S;
			if (D[i][j] < k)
				return false;
			...
			while (!S.empty())
			{
				...
				for (auto [di, dj] : DIR1)
				{
					int i3 = di + i2, j3 = dj + j2;
					if (i3 < 0 || i3 >= n || j3 < 0 || j3 >= m || D[i3][j3] < k || V[i3][j3])
						continue;
					S.emplace_back(i3, j3);
				}
			}

			vvbl V2(n, vbl(m, false));
		};
}
