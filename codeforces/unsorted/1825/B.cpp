...
int64_t solve2(int n, int m, vi32 const &B)
{
	i64 a = B[0], b = B[n * m - 2], c = B[n * m - 1];
	return (b - a) * n * (m - 1) + (c - a) * (n - 1);
}
int64_t solve(int n, int m, vi32 const &B)
{
	return max(solve2(n, m, B), solve2(m, n, B));
}
...
void go()
{
	...
	sort(ALL(B));
	i64 ans = solve(n, m, B);
	swap(B[n * m - 2], B[n * m - 1]);
	ans = max(ans, solve(n, m, B));
	swap(B[n * m - 2], B[n * m - 1]);
	for (int &b : B)
		b = -b;
	reverse(ALL(B));
