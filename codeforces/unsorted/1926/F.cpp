...
public:
	...
	constexpr [[nodiscard]] optional_iterator operator-(const optional_iterator& it) { dassert(val == it.val); return
static_cast<difference_type>(done) - it.done; } constexpr [[nodiscard]] T& operator*() const { dassert(!done); return *opt; } constexpr [[nodiscard]]
T* operator->() const { dassert(!done); return opt; } constexpr [[nodiscard]] T& operator[](difference_type n) { return *(*this + n); } constexpr
[[nodiscard]] bool operator==(const optional_iterator& it) const noexcept { return cmp<equal_to>(it); } constexpr [[nodiscard]] bool operator!=(const
optional_iterator& it) const noexcept { return cmp<not_equal_to>(it); } constexpr [[nodiscard]] bool operator<(const optional_iterator& it) const
noexcept { return cmp<less>(it); } constexpr [[nodiscard]] bool operator<=(const optional_iterator& it) const noexcept { return cmp<less_equal>(it); }
...
template<class>
struct is_istrm : public false_type
{};
template<class C, class Tr>
struct is_istrm<basic_istream<C, Tr>> : public true_type
{};
template<class S>
concept istrm = is_istrm<S>::value;
template<class>
struct is_ostrm : public false_type
{};
template<class C, class Tr>
struct is_ostrm<basic_ostream<C, Tr>> : public true_type
{};
template<class S>
concept ostrm = is_ostrm<S>::value;
template<class T>
struct tuple_like_helper
{
	template<class U>
	static bool_constant<sizeof(U) == sizeof(U)> f(U *);
	static false_type							 f(...);
	using type = decltype(f((tuple_size<T> *)nullptr));
};
template<class T>
concept tuple_like = tuple_like_helper<remove_reference_t<T>>::type::value;
template<class R, class S>
concept read_as_range = !tuple_like<R> && ranges::range<R> && ranges::output_range<R, ranges::range_value_t<R>> && istrm<S>;
template<class R, class S>
concept write_as_range = !tuple_like<R> && ranges::input_range<R> && ostrm<S>;

template<class T>
[[nodiscard]] T read(istrm auto &is)
{
	T x;
	is >> x;
	return x;
...
template<istrm S>
S &operator>>(S &is, tuple_like auto &&tup)
{
	return apply(
		[&](auto &&...ts) -> auto &
		{
...
template<ostrm S>
S &operator<<(S &os, tuple_like auto const &tup)
{
	return apply(
		[&](auto &&...ts) -> auto &
		{
...
template<class T, istrm S>
S &operator>>(S &is, optional<T> &&opt)
{
	opt.emplace(read<T>(is));
	return is;
}
template<class T, istrm S>
S &operator>>(S &is, optional<T> &opt)
...
template<class T, ostrm S>
S &operator<<(S &os, optional<T> const &opt)
{
	if (opt)
		os << *opt;
	return os;
...
template<istrm S>
S &operator>>(S &is, read_as_range<S> auto &&r)
{
...
template<ostrm S>
S &operator<<(S &os, write_as_range<S> auto const &r)
{
	for (auto it = begin(r); it != end(r); ++it)
		os << *it;
	return os;
...
auto &operator>>(istrm auto &is, vector<bool> &r)
{
	for (auto it = begin(r); it != end(r); it++)
		*it = read<char>(is) == '1';
	return is;
...
concept template_constructible_from = requires { C(declval<Args>()...); };
...
constexpr struct from_range_t
{
} from_range;
template<class C>
concept reservable_container = ranges::sized_range<C> && requires(C &c, ranges::range_size_t<C> n) {
	c.reserve(n);
	{
		c.capacity()
	} -> same_as<decltype(n)>;
...
concept container_insertable = requires(C &c, R &&r) {
	requires(
		requires { c.push_back(forward<R>(r)); } || requires { c.insert(c.end(), forward<R>(r)); });
};
...
template<class T, size_t N, ranges::input_range R>
struct to_helper_1<array<T, N>, R> // Nonstandard
{
...
template<size_t N, ranges::input_range R>
constexpr auto to(R &&r) // Nonstandard
{
...
constexpr auto to(R &&r, Args &&...args)
{
	...
	if constexpr (template_constructible_from<C, R, Args...>)
		return to<decltype(C(declval<R>(), declval<Args>()...))>(forward<R>(r), forward<Args>(args)...);
	else if constexpr (template_constructible_from<C, from_range_t, R, Args...>)
		return to<decltype(C(from_range, declval<R>(), declval<Args>()...))>(forward<R>(r), forward<Args>(args)...);
	else
		return to<decltype(C(declval<input_iterator>(), declval<input_iterator>(), declval<Args>()...))>(forward<R>(r), forward<Args>(args)...);
}
...
private:
	...
	tuple<Args...> b;
	template<class S, size_t... I, ranges::range R>
	static constexpr C call(S &&s, index_sequence<I...>, R &&r)
	{
		return to<C, R, Args...>(forward<R>(r), get<I>(forward<S>(s).b)...);
	}

public:
	template<class... A>
	explicit constexpr To1(int, A &&...a) noexcept((is_nothrow_constructible_v<Args, A> && ...)) : b(forward<A>(a)...)
	{
		static_assert(sizeof...(Args) == sizeof...(A));
	}
	template<ranges::range R>
	constexpr C operator()(R &&r) &
	{
		return call(*this, I(), forward<R>(r));
	}
	template<ranges::range R>
	...
	constexpr C operator()(R &&r) &&
	{
		return call(move(*this), I(), forward<R>(r));
	}
	template<ranges::range R>
	constexpr C operator()(R &&r) const &&
	{
...
{
private:
	template<class S, ranges::range R>
	static constexpr auto call(S &&s, R &&r)
	{
		return to<N>(forward<R>(r));
	}
...
public:
	...
	constexpr auto operator()(R &&r) &&
	{
		return call(move(*this), forward<R>(r));
	}
	template<ranges::range R>
	constexpr auto operator()(R &&r) const &&
	{
...
private:
	...
	tuple<Args...> b;
	template<class S, size_t... I, ranges::range R>
	static constexpr auto call(S &&s, index_sequence<I...>, R &&r)
	{
		return to<C, R, Args...>(forward<R>(r), get<I>(forward<S>(s).b)...);
	}
...
public:
	template<class... A>
	explicit constexpr To3(int, A &&...a) noexcept((is_nothrow_constructible_v<Args, A> && ...)) : b(forward<A>(a)...)
	{
		static_assert(sizeof...(Args) == sizeof...(A));
	}
	template<ranges::range R>
...
{
	return To1<C, Args...>(0, forward<Args>(args)...);
}
template<size_t N>
constexpr auto to()
{
...
constexpr auto to(Args &&...args)
{
	return To3<C, Args...>(0, forward<Args>(args)...);
}
...
template<class T, class C = char, class Tr = char_traits<C>, class D = ptrdiff_t>
class lazy_istream_iterator
{
public:
...
public:
	...
	lazy_istream_iterator &operator++()
	{
		if (!cache)
			ignore = read<T>(s);
		cache.reset();
	...
	[[nodiscard]] lazy_istream_iterator operator++(int)
	{
		if (!cache)
			s >> cache;
		lazy_istream_iterator cur(s);
		swap(cur.cache, cache);
		return cur;
	}
};
...
private:
	...
	static constexpr int lo_idx(T t)
	{
		return countr_zero<U>(t);
	}
	static constexpr int hi_idx(T t)
	{
		return BITS - countl_zero<U>(t);
	}
	constexpr T a_lo() const
	{
		return a & -a;
	}
	constexpr T b_hi() const
	{
		return T(1) << hi_idx(b);
	}
	constexpr void sw(T bit)
	{
		a ^= bit;
		b ^= bit;
	}
	template<template<class> class C>
	constexpr auto cmp(const biterator &it) const noexcept
	{
		return C<T>()(it.a, a);
	}
...
public:
	...
	constexpr biterator &operator++() noexcept
	{
		dassert(a);
		sw(a_lo());
		return *this;
	...
	[[nodiscard]] constexpr biterator operator++(int) noexcept
	{
		auto it = *this;
		++*this;
		return it;
	...
	[[nodiscard]] constexpr int idx() const noexcept
	{
		return lo_idx(a);
	}
	[[nodiscard]] constexpr T bit() const noexcept
	{
		return a_lo();
	}
	[[nodiscard]] constexpr pair<int, T> operator*() const noexcept
	{
		dassert(a);
		return {idx(), bit()};
	}
	[[nodiscard]] constexpr bool operator==(const biterator &it) const noexcept
	{
		return cmp<equal_to>(it);
	}
	[[nodiscard]] constexpr bool operator!=(const biterator &it) const noexcept
	{
		return cmp<not_equal_to>(it);
	}
	[[nodiscard]] constexpr bool operator<(const biterator &it) const noexcept
	{
		return cmp<less>(it);
	...
	[[nodiscard]] constexpr bool operator<=(const biterator &it) const noexcept
	{
		return cmp<less_equal>(it);
	}
	[[nodiscard]] constexpr bool operator>(const biterator &it) const noexcept
	{
		return cmp<greater>(it);
	}
	[[nodiscard]] constexpr bool operator>=(const biterator &it) const noexcept
	{
		return cmp<greater_equal>(it);
	}
	[[nodiscard]] constexpr strong_ordering operator<=>(const biterator &it) const noexcept
	{
		return cmp<compare_three_way>(it);
	}
};

template<integral T>
struct bits : public ranges::view_interface<bits<T>>
{
	T x;
	[[nodiscard]] constexpr bits(T val = 0) : x(val) {}
	[[nodiscard]] constexpr biterator<T> begin()
	{
		return biterator<T>(x);
	}
	[[nodiscard]] constexpr biterator<T> end()
	{
		return biterator<T>();
	}
};
...
template<integral T>
[[nodiscard]] constexpr T rup2(T x) noexcept
{
	if (x-- == 0)
		return 1;
...
[[nodiscard]] constexpr T isqrtc(T x)
{
	T ub = ((T)1 << (8 * sizeof(x) - countl_zero(x) + 1) / 2) + 1;
	return *ranges::lower_bound(views::iota((T)0, ub),
								x,
								{},
								[](T r)
								{
...
template<unsigned_integral T>
[[nodiscard]] constexpr T isqrtf(T x)
{
	T r = isqrtc(x);
	return r - (r * r != x);
}
...
template<uint64_t N>
constexpr uint32_t ISQRTC = static_cast<uint32_t>(isqrtc(N));
template<uint64_t N>
constexpr uint32_t ISQRTF = ISQRTC<N> - (ISQRTC<N> * ISQRTC<N> != N);
...
template<uint32_t a>
constexpr uint32_t PHI = phi(a);
template<uint32_t a>
constexpr bool PRIME = PHI<a> + 1 == a;
...
struct mint
{
	...
	constexpr mint(integral auto u) : v(static_cast<uint32_t>(u < 0 ? u % M + M : u % M))
	{
		if (v == M)
			v = 0;
	}
	[[nodiscard]] constexpr uint32_t val() const
	{
		return v;
	}
	...
	[[nodiscard]] constexpr mint operator++(int)
	{
		mint a = *this;
		++*this;
		return a;
	...
	[[nodiscard]] constexpr friend mint operator+(mint a)
	{
		return a;
	}
	[[nodiscard]] constexpr friend mint operator-(mint a)
	...
	constexpr mint &operator+=(integral auto b)
	{
		return *this += mint(b);
	}
	[[nodiscard]] constexpr friend mint operator+(mint a, mint b)
	{
		return a += b;
	}
	[[nodiscard]] constexpr friend mint operator+(mint a, integral auto b)
	{
		return a + mint(b);
	}
	[[nodiscard]] constexpr friend mint operator+(integral auto a, mint b)
	{
		return mint(a) + b;
	}
	constexpr mint &operator-=(mint b)
	...
	[[nodiscard]] constexpr friend mint mpow(mint a, integral auto b)
	{
		return mpow(a, pow_t(b));
	}
	[[nodiscard]] constexpr mint pow(pow_t b) const
	{
		return mpow(*this, b);
	}
	[[nodiscard]] constexpr mint pow(integral auto b) const
	{
		return mpow(*this, pow_t(b));
	}
	...
	[[nodiscard]] constexpr friend mint minv(mint a)
		requires PRIME
	{
		return a.inv();
	}
	...
	[[nodiscard]] constexpr friend bool operator==(mint a, mint b)
	{
		return a.v == b.v;
	}
	[[nodiscard]] constexpr friend bool operator==(mint a, integral auto b)
	{
		return a == mint(b);
	}
	[[nodiscard]] constexpr friend bool operator==(integral auto a, mint b)
	{
		return mint(a) == b;
	}
	[[nodiscard]] constexpr friend bool operator!=(mint a, mint b)
...
#define TDEFS1(T)                                                                                                                                    \
	using p##T		  = pair<T, T>;                                                                                                                  \
	using a2##T		  = array<T, 2>;                                                                                                                 \
	using v##T		  = vector<T>;                                                                                                                   \
	using v##p##T	  = vector<p##T>;                                                                                                                \
	using v##a2##T	  = vector<a2##T>;                                                                                                               \
	using vv##T		  = vector<v##T>;                                                                                                                \
	using vv##p##T	  = vector<v##p##T>;                                                                                                             \
	using vv##a2##T	  = vector<v##a2##T>;                                                                                                            \
	using vvv##T	  = vector<vv##T>;                                                                                                               \
	using vvv##p##T	  = vector<vv##p##T>;                                                                                                            \
	using vvv##a2##T  = vector<vv##a2##T>;                                                                                                           \
	using vvvv##T	  = vector<vvv##T>;                                                                                                              \
	using vvvv##p##T  = vector<vvv##p##T>;                                                                                                           \
	using vvvv##a2##T = vector<vvv##a2##T>;                                                                                                          \
	using l##T		  = list<T>;                                                                                                                     \
	using l##p##T	  = list<p##T>;                                                                                                                  \
	using l##a2##T	  = list<a2##T>;                                                                                                                 \
	using vl##T		  = vector<l##T>;                                                                                                                \
	using vl##p##T	  = vector<l##p##T>;                                                                                                             \
	using vl##a2##T	  = vector<l##a2##T>;                                                                                                            \
	using s##T		  = set<T>;                                                                                                                      \
	using s##p##T	  = set<p##T>;                                                                                                                   \
	using s##a2##T	  = set<a2##T>;                                                                                                                  \
	using vs##T		  = vector<s##T>;                                                                                                                \
	using vs##p##T	  = vector<s##p##T>;                                                                                                             \
	using vs##a2##T	  = vector<s##a2##T>;                                                                                                            \
	template<class Pr>                                                                                                                               \
	using S##T = set<T, Pr>;                                                                                                                         \
	template<class Pr>                                                                                                                               \
	using S##p##T = set<p##T, Pr>;                                                                                                                   \
	template<class Pr>                                                                                                                               \
	using S##a2##T = set<a2##T, Pr>;                                                                                                                 \
	template<class Pr>                                                                                                                               \
	using vS##T = vector<S##T<Pr>>;                                                                                                                  \
	template<class Pr>                                                                                                                               \
	using vS##p##T = vector<S##p##T<Pr>>;                                                                                                            \
	template<class Pr>                                                                                                                               \
	using vS##a2##T	 = vector<S##a2##T<Pr>>;                                                                                                         \
	using ms##T		 = multiset<T>;                                                                                                                  \
	using ms##p##T	 = multiset<p##T>;                                                                                                               \
	using ms##a2##T	 = multiset<a2##T>;                                                                                                              \
	using vms##T	 = vector<ms##T>;                                                                                                                \
	using vms##p##T	 = vector<ms##p##T>;                                                                                                             \
	using vms##a2##T = vector<ms##a2##T>;                                                                                                            \
	template<class Pr>                                                                                                                               \
	using MS##T = multiset<T, Pr>;                                                                                                                   \
	template<class Pr>                                                                                                                               \
	using MS##p##T = multiset<p##T, Pr>;                                                                                                             \
	template<class Pr>                                                                                                                               \
	using MS##a2##T = multiset<a2##T, Pr>;                                                                                                           \
	template<class Pr>                                                                                                                               \
	using vMS##T = vector<MS##T<Pr>>;                                                                                                                \
	template<class Pr>                                                                                                                               \
	using vMS##p##T = vector<MS##p##T<Pr>>;                                                                                                          \
	template<class Pr>                                                                                                                               \
	using vMS##a2##T = vector<MS##a2##T<Pr>>;                                                                                                        \
	using us##T		 = unordered_set<T>;                                                                                                             \
	using us##p##T	 = unordered_set<p##T>;                                                                                                          \
	using us##a2##T	 = unordered_set<a2##T>;                                                                                                         \
	using vus##T	 = vector<us##T>;                                                                                                                \
	using vus##p##T	 = vector<us##p##T>;                                                                                                             \
	using vus##a2##T = vector<us##a2##T>;                                                                                                            \
	template<class H, class Eq>                                                                                                                      \
	using US##T = unordered_set<T, H, Eq>;                                                                                                           \
	template<class H, class Eq>                                                                                                                      \
	using US##p##T = unordered_set<p##T, H, Eq>;                                                                                                     \
	template<class H, class Eq>                                                                                                                      \
	using US##a2##T = unordered_set<a2##T, H, Eq>;                                                                                                   \
	template<class H, class Eq>                                                                                                                      \
	using vUS##T = vector<US##T<H, Eq>>;                                                                                                             \
	template<class H, class Eq>                                                                                                                      \
	using vUS##p##T = vector<US##p##T<H, Eq>>;                                                                                                       \
	template<class H, class Eq>                                                                                                                      \
	using vUS##a2##T  = vector<US##a2##T<H, Eq>>;                                                                                                    \
	using ums##T	  = unordered_multiset<T>;                                                                                                       \
	using ums##p##T	  = unordered_multiset<p##T>;                                                                                                    \
	using ums##a2##T  = unordered_multiset<a2##T>;                                                                                                   \
	using vums##T	  = vector<ums##T>;                                                                                                              \
	using vums##p##T  = vector<ums##p##T>;                                                                                                           \
	using vums##a2##T = vector<ums##a2##T>;                                                                                                          \
	template<class H, class Eq>                                                                                                                      \
	using UMS##T = unordered_multiset<T, H, Eq>;                                                                                                     \
	template<class H, class Eq>                                                                                                                      \
	using UMS##p##T = unordered_multiset<p##T, H, Eq>;                                                                                               \
	template<class H, class Eq>                                                                                                                      \
	using UMS##a2##T = unordered_multiset<a2##T, H, Eq>;                                                                                             \
	template<class H, class Eq>                                                                                                                      \
	using vUMS##T = vector<UMS##T<H, Eq>>;                                                                                                           \
	template<class H, class Eq>                                                                                                                      \
	using vUMS##p##T = vector<UMS##p##T<H, Eq>>;                                                                                                     \
	template<class H, class Eq>                                                                                                                      \
	using vUMS##a2##T = vector<UMS##a2##T<H, Eq>>;                                                                                                   \
	using q##T		  = queue<T>;                                                                                                                    \
	using q##p##T	  = queue<p##T>;                                                                                                                 \
	using q##a2##T	  = queue<a2##T>;                                                                                                                \
	using vq##T		  = vector<q##T>;                                                                                                                \
	using vq##p##T	  = vector<q##p##T>;                                                                                                             \
	using vq##a2##T	  = vector<q##a2##T>;                                                                                                            \
	using d##T		  = deque<T>;                                                                                                                    \
	using d##p##T	  = deque<p##T>;                                                                                                                 \
	using d##a2##T	  = deque<a2##T>;                                                                                                                \
	using vd##T		  = vector<d##T>;                                                                                                                \
	using vd##p##T	  = vector<d##p##T>;                                                                                                             \
	using vd##a2##T	  = vector<d##a2##T>;                                                                                                            \
	using pq##T		  = priority_queue<T>;                                                                                                           \
	using pq##p##T	  = priority_queue<p##T>;                                                                                                        \
	using pq##a2##T	  = priority_queue<a2##T>;                                                                                                       \
	using vpq##T	  = vector<pq##T>;                                                                                                               \
	using vpq##p##T	  = vector<pq##p##T>;                                                                                                            \
	using vpq##a2##T  = vector<pq##a2##T>;                                                                                                           \
	template<class Pr>                                                                                                                               \
	using PQ##T = priority_queue<T, v##T, Pr>;                                                                                                       \
	template<class Pr>                                                                                                                               \
	using PQ##p##T = priority_queue<p##T, v##p##T, Pr>;                                                                                              \
	template<class Pr>                                                                                                                               \
	using PQ##a2##T = priority_queue<a2##T, v##a2##T, Pr>;                                                                                           \
	template<class Pr>                                                                                                                               \
	using vPQ##T = vector<PQ##T<Pr>>;                                                                                                                \
	template<class Pr>                                                                                                                               \
	using vPQ##p##T = vector<PQ##p##T<Pr>>;                                                                                                          \
	template<class Pr>                                                                                                                               \
	using vPQ##a2##T = vector<PQ##a2##T<Pr>>;
#define TDEFS3(T, N, U)                                                                                                                              \
	U T N;                                                                                                                                           \
	TDEFS1(N)
...
{
template<input_or_output_iterator I, subrange_kind K = sized_sentinel_for<I, I> ? subrange_kind::sized : subrange_kind::unsized>
		...
		for (int prv = 0; prv < N; prv++)
		{
			...
			for (int c = 0; c < (1 << 7); c++)
			{
				if (b & (c << 1) & (c >> 1))
					continue;
				int d = c & (a << 1) & (a >> 1);
				B[R[c][d]] = min(B[R[c][d]], pcost + __builtin_popcount(c ^ cur));
			}
		}
		A.swap(B);
	}
