...
typedef tuple<int, int, string> ent;

pair<int, int> ab(ent const &x)
{
...
void go()
{
	...
	sort(ALL(A), [&](ent const &x, ent const &y) { return ab(x) < ab(y); });
	vector<ent> B;

	vector<ent> PA;
	rep(i, 0, n)
	{
		if (i < n - 1 && ab(A[i]) == ab(A[i + 1]))
		{
			PA.push_back(move(A[i]));
			PA.push_back(move(A[i + 1]));
			i++;
		}
		else
	...
	rep(i, 0, (int)B.size())
	{
		if (i < m - 1 && aa(B[i]) == aa(B[i + 1]))
		{
			PB.push_back(move(B[i]));
			PB.push_back(move(B[i + 1]));
			i++;
		}
	}
	...
	cout << PA.size() / 2 << '\n';
	srep(i, 0, (int)PA.size(), 2)
	{
		cout << get<2>(PB[i]) << ' ' << get<2>(PA[i]) << '\n';
		cout << get<2>(PB[i + 1]) << ' ' << get<2>(PA[i + 1]) << '\n';
	}
}
