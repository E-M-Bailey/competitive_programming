...
{
	T.update(i, 1);
	map<int, int>::iterator a, b = I.upper_bound(i);
	if (b == begin(I) || (a = prev(b))->second <= i)
	{
		I.emplace(i, i + 1);
		F[{i, i + 1}]++;
		return;
	...
	{
		F[*a]--;
		if (b == end(I) || a->second < b->first)
		{
			a->second++;
			cnt = false;
		}
		else if (a->first > 0 && (a == begin(I) || prev(a)->second < a->first))
		{
			auto na = I.extract(a);
			na.key()--;
			a = I.insert(b, move(na));
			cnt = false;
		}
		else
		{
...
{
	T.update(i, -1);
	auto b = I.upper_bound(i), a = prev(b);
	...
	int q = T.query(a->first);
	if (T.query(a->second - 1) == q)
		a->second--;
	else if (T.query(a->first + 1) == q - 1)
	{
		auto na = I.extract(a);
		na.key()++;
		a = I.insert(b, move(na));
	...
	{
		// Maintain sum [i, lo) >= 0, sum [i, hi) < 0
		int lo = a->first, hi = a->second;
		while (hi - lo > 1)
		{
			int mid = (lo + hi) / 2;
			(T.query(mid) < q ? hi : lo) = mid;
		}
		b = I.emplace_hint(b, hi, a->second);
		F[*b]++;
		a->second = lo;
	}
	F[*a]++;
