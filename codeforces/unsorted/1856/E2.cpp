...
void go()
{
	...
	rrep(i, 0, n)
	{
		auto &SS = S[i];
		sort(ALL(SS));
		int ss = accumulate(ALL(SS), 0);
		// cerr << ss << endl;
		if (SS.empty()) {}
		else if (SS.back() * 2 >= ss)
			ANS[i] += (i64)SS.back() * (ss - SS.back());
		else
		{
			int m = (int)SS.size();//, ct = 1;

			int *A = DP1, *B = DP2;
			int pos = 0;
			int lim = ss / 2 + 1;
			//memset(A, -1, lim * sizeof(int));
			fill_n(A, lim, -1);
			A[0] = 0;
			while (pos < m)
			{
				int cur = SS[pos], npos = pos;
				do
					++npos;
				while (npos < m && SS[npos] == cur);
				int ct = npos - pos;
				...
				rep(j, 0, lim)
				{
					if (A[j] >= 0)
						B[j] = 0;
					else if (j >= cur && B[j - cur] >= 0 && B[j - cur] < ct)
						B[j] = B[j - cur] + 1;
					else
						B[j] = -1;
				}
