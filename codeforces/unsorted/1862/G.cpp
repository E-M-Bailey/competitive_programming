...
void go()
{
	...
	for (int a : A)
		++AF[a];
	for (auto it = begin(AF), jt = next(it); jt != end(AF); it = jt++)
		++GF[jt->first - it->first];

	auto const gf_erase = [&](int g)
		{
			auto it = GF.find(g);
			assert(it != end(GF));
			if (--it->second == 0)
	...
	auto const gf_insert = [&](int g)
		{
			auto [it, ins] = GF.emplace(g, 1);
			if (!ins)
				++it->second;
		};
	auto const af_erase = [&](int x)
		{
			auto it = AF.find(x);
			assert(it != end(AF));
			if (--it->second == 0)
			{
				bool a = it != begin(AF), b = next(it) != end(AF);
				if (a)
					gf_erase(it->first - prev(it)->first);
				if (b)
					gf_erase(next(it)->first - it->first);
				if (a && b)
					gf_insert(next(it)->first - prev(it)->first);
				AF.erase(it);
			}
		};
	auto const af_insert = [&](int x)
		{
			auto [it, ins] = AF.emplace(x, 1);
			if (ins)
			{
				bool a = it != begin(AF), b = next(it) != end(AF);
				if (a && b)
					gf_erase(next(it)->first - prev(it)->first);
				if (a)
					gf_insert(it->first - prev(it)->first);
				if (b)
					gf_insert(next(it)->first - it->first);
			}
			else
				++it->second;
		};
	...
	while (q--)
	{
		...
		af_insert(x);
		A[i] = x;
		cout << (GF.empty() ? 0 : prev(end(GF))->first) + prev(end(AF))->first << ' ';
	}
	cout << '\n';
}
