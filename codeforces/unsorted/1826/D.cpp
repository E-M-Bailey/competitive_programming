...
struct ent
{
	int len, mx, l1, l2, r1, r2, ans;
};

vi32 B;
const int NIL = INT_MIN / 3;

ent dc(int l, int r)
{
	if (l + 1 == r)
		return ent{1, B[l], B[l] - 1, NIL, B[l] - 1, NIL, NIL};

	int mid = (l + r) / 2;
	ent L = dc(l, mid), R = dc(mid, r);

	return ent{
		L.len + R.len,
		max(L.mx, R.mx),
		max(L.l1, R.l1 - L.len),
		max({L.l2, L.mx + R.l1 - L.len, R.l2 - L.len}),
		max(R.r1, L.r1 - R.len),
		max({R.r2, R.mx + L.r1 - R.len, L.r2 - R.len}),
		max({L.ans, R.ans, L.r2 + R.l1, L.r1 + R.l2})
	};
}
