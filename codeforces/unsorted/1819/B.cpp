...
const auto c1 = [](int a, int b)
{
	auto [ah, aw] = A[a];
	auto [bh, bw] = A[b];
	return tuple(ah, aw, a) > tuple(bh, bw, b);
};
const auto c2 = [](int a, int b)
{
	auto [ah, aw] = A[a];
	auto [bh, bw] = A[b];
	return tuple(aw, ah, a) > tuple(bw, bh, b);
};

pi64 cmp(bool h)
{
	typedef Si32<decltype(c1)> S1;
	typedef Si32<decltype(c2)> S2;
	S1 s1(c1);
	S2 s2(c2);
	vector<pair<S1::iterator, S2::iterator>> IT;

	rep(i, 0, n)
	{
		auto it1 = s1.insert(i).first;
		auto it2 = s2.insert(i).first;
		IT.emplace_back(it1, it2);
	}

	/*i64 fst = h ? A[*s1.begin()].first : A[*s2.begin()].second;
	if (area % fst)
		return { -1, -1 };
	//cerr << area << ' ' << fst << endl;
	pi64 ans = make_pair(fst, area / fst);
	if (!h)
		ans = flip(ans);*/
	...
	while (!s1.empty())
	{
		...
		{
			//cerr << "h" << endl;
			dim = A[*s1.begin()].first;
			while (!s1.empty() && A[cur = *s1.begin()].first == dim)
			{
				//cerr << ' ' << cur << ' ' << dim << endl;
				acc += A[cur].second;
				s1.erase(IT[cur].first);
				s2.erase(IT[cur].second);
			}
			//if (!s1.empty())
			//	cerr << ' ' << *s1.begin() << endl;
			stages.emplace_back(dim, acc);
		}
		else
		{
			//cerr << "w" << endl;
			dim = A[*s2.begin()].second;
			while (!s1.empty() && A[cur = *s2.begin()].second == dim)
			{
				//cerr << ' ' << cur << endl;
				acc += A[cur].first;
				s1.erase(IT[cur].first);
			...
			stages.emplace_back(acc, dim);
		}
		h = !h;
	}
	pi64 acc2 = stages.back();
	stages.pop_back();
	while (!stages.empty())
	{
		//cerr << acc2.first << ' ' << acc2.second << ' ' << stages.back().first << ' ' << stages.back().second << ' ' << h << endl;
		if (h ? acc2.first != stages.back().first : acc2.second != stages.back().second)
			return { -1, -1 };
		if (h)
			acc2.second += stages.back().second;
		else
			acc2.first += stages.back().first;
		stages.pop_back();
		h = !h;
	}
