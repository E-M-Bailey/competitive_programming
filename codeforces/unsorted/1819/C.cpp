...
bool dfs(int u, int v, int d = 0, bool r = false)
{
	vector<int> IN, LF;
	for (int w : G[v])
		if (w != u)
			(G[w].size() == 1 ? LF : IN).push_back(w);
	if (d == 1 && IN.size() == 2)
	{
		if (!dfs(v, IN[0], d + 1, false))
			return false;
		ans.push_back(v);
		if (!dfs(v, IN[1], d + 1, true))
			return false;
		for (int w : LF)
			ans.push_back(w);
	}
	...
	{
		if (r)
			copy(ALL(LF), back_inserter(ans));
		else
			ans.push_back(v);
		if (!IN.empty() && !dfs(v, IN[0], d + 1, !r))
			return false;
		if (r)
			ans.push_back(v);
		else
