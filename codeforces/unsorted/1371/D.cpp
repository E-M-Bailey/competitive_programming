...
vector<vector<bool>> solve(int n, int k)
{
	int l = n / 2, r = (n + 1) / 2;
	bool flip = k > l * l + r * r
	if (k > l * l + r * r)
...
void go()
{
	...
	rep(mask, 0, 1 << (N * N))
	{
		...
		int dr = *max_element(ALL(R)) - *min_element(ALL(R));
		int dc = *max_element(ALL(C)) - *min_element(ALL(C));
		int b = dr * dr + dc * dc;
		if (b < B[k])
		{
			B[k] = b;
