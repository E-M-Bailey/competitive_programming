...
struct dso
{
	...
	void onion(int x, int y, int w, int s, int &ans)
	{
		x = find(x);
		y = find(y);
		assert(x != y);
		if (S[x] < S[y])
			swap(x, y);
		P[y] = x;
		i64 ex = (i64)(S[x] - 1) * (S[x] - 2) / 2;
		i64 ey = (i64)(S[y] - 1) * (S[y] - 2) / 2;
		S[x] += S[y];
		i64 e = (i64)(S[x] - 1) * (S[x] - 2) / 2;
		ans = (int)(ans * mpow(s - w + 1, e - ex - ey) % MOD);
	}
};
