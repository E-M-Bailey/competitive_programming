...
ull modpow(ull b, ull e, ull mod, ull st = 1)
{
	ull ans = st;
...
i64 constexpr MOD = 1e9 + 7;

vector<pair<int, i64>> pfac(int n)
{
	...
	for (auto p : primes)
	{
		if (ans.empty() || ans.back().first != (int)p)
			ans.emplace_back((int)p, 1);
		else
			ans.back().second++;
	}
	return ans;
...
int constexpr MAXN = 100, MAXP = 9;

int n, m = 0, g = 0;
int A[MAXN], B[MAXN];

// index -> compressed prime factorization
vector<pair<int, i64>> AP[MAXN], BP[MAXN];

// prime -> indices
vector<int> PA[MAXN * MAXP], PB[MAXN * MAXP];
...
int pindex(int p)
{
	return (int)(lower_bound(P, P + m, p) - P);
}

i64 freq(int j, vector<pair<int, i64>> const &Z)
{
	auto it = partition_point(begin(Z), end(Z), [j](pair<int, i64> z) { return z.first < j; });
	return it == end(Z) || it->first != j ? 0 : it->second;
}
...
void input()
{
	...
	rep(i, 0, n)
	{
		for (auto [p, f] : AP[i] = pfac(A[i]))
			P[m++] = p;
		for (auto [p, f] : BP[i] = pfac(B[i]))
			P[m++] = p;
	}
	sort(P, P + m);
	m = (int)(unique(P, P + m) - P);
	rep(i, 0, n)
	{
		rep(j, 0, (int)AP[i].size())
			PA[AP[i][j].first = pindex(AP[i][j].first)].push_back(i);
		rep(j, 0, (int)BP[i].size())
			PB[BP[i][j].first = pindex(BP[i][j].first)].push_back(i);
	}
}

vector<pair<int, i64>> const &term(int i, i64 x)
{
	if (x < 0)
	{
		cout << "-1";
	...
	static vector<pair<int, i64>> Z;
	if (x == 0)
		return Z = AP[i];
	Z.clear();
	int sa = (int)AP[i].size(), sb = (int)BP[i].size();
	int a = 0, b = 0;
	while (a < sa || b < sb)
	{
		if (b == sb || (a != sa && AP[i][a].first < BP[i][b].first))
		{
			Z.emplace_back(AP[i][a].first, AP[i][a].second);
			a++;
		}
		else if (a == sa || AP[i][a].first > BP[i][b].first)
		{
			Z.emplace_back(BP[i][b].first, (i64)BP[i][b].second * x);
			b++;
		}
		else
		{
			Z.emplace_back(AP[i][a].first, AP[i][a].second + (i64)BP[i][b].second * x);
			a++;
			b++;
		}
	}
...
bool is_term(int i, vector<pair<int, i64>> const &Z)
{
	if (AP[i] == Z)
		return true;
	int sa = (int)AP[i].size(), sb = (int)BP[i].size(), sc = (int)Z.size();
	int a = 0, b = 0, c = 0;
	i64 x = -1;
	while (a < sa || b < sb)
	{
		...
		if (b == sb || (a != sa && AP[i][a].first < BP[i][b].first))
		{
			if (AP[i][a].first != Z[c].first || AP[i][a].second != Z[c].second)
				return false;
			a++;
		}
		else if (a == sa || AP[i][a].first > BP[i][b].first)
		{
			if (x == -1)
				x = Z[c].second / BP[i][b].second;
			if (BP[i][b].first != Z[c].first || BP[i][b].second * x != Z[c].second)
				return false;
			b++;
		}
		else
		{
			if (x == -1)
				if ((x = (Z[c].second - AP[i][a].second) / BP[i][b].second) < 0)
					return false;
			if (AP[i][a].first != Z[c].first || AP[i][a].second + BP[i][b].second * x != Z[c].second)
				return false;
			a++;
			b++;
		}
...
template<bool fail = 1>
void answer(vector<pair<int, i64>> const &Z)
{
	rep(i, 0, n)
		if (!is_term(i, Z))
		{
...
bool some(int j)
{
	...
	rep(i, 0, n)
		if (!S[i])
			O[k++] = i;
	// find the answer's factorization
	i64 y = freq(j, AP[O[0]]);
	i64 a0 = freq(j, AP[PB[j][0]]), b0 = freq(j, BP[PB[j][0]]);
	i64 x0 = (y - a0) / b0;
	auto const &Z = term(PB[j][0], x0);
	answer(Z);
	return true;
}
...
i64 da_gcd(int i)
{
	i64 ans = 0;
	rep(j, 0, m)
		ans = gcd(ans, freq(j, AP[i]) - freq(j, AP[0]));
	return ans;
}
...
bool diff(int i)
{
	// a00 + b00 x0 = ai0 + bi0 xi0
	// a01 + b01 xi = ai1 + bi1 xi1
	// [b00 -bi0] [x0] = [ai0 - a00]
	// [b01 -bi1] [xi]   [ai1 - a01]
	// [x0] = [-bi1 bi0] [ai0 - a00] / (b01 * bi0 - b00 * bi1)
	// [xi]   [-b01 b00] [ai1 - a01]
	int j0 = BP[0][0].first, j1 = BP[0][1].first;
	i64 a00 = freq(j0, AP[0]), a01 = freq(j1, AP[0]), ai0 = freq(j0, AP[i]), ai1 = freq(j1, AP[i]);
	i64 b00 = BP[0][0].second, b01 = BP[0][1].second, bi0 = BP[i][0].second, bi1 = BP[i][1].second;
	i64 det = b01 * bi0 - b00 * bi1;
	i64 u = a00 - ai0, v = ai1 - a01;
	i64 x0 = bi1 * u + bi0 * v;
	auto const &Z = term(0, x0 / det);
	answer(Z);
	return true;
}
...
bool same()
{
	...
	{
		rep(j, 0, m)
			AG[i] = gcd(AG[i], DA[j] = freq(j, AP[i]) - freq(j, AP[0]));
		if (DA[0] < 0)
			AG[i] = -AG[i];
		rep(j, 0, m)
			if (DA[j] != M[j] * AG[i])
				return false;
		tie(x, y) = crt(x, y, (AG[i] % BG[i] + BG[i]) % BG[i], BG[i]);
	}
	i64 ma = *max_element(AG, AG + n);
	if (x < ma)
		x += (ma - x + y - 1) / y * y;
	auto const &Z = term(0, x / BG[0]);
	answer(Z);
	return true;
}
...
bool all()
{
	rep(i, 0, n)
		BG[i] = b_gcd(i);
	rep(j, 0, m)
		M[j] = BP[0][j].second / BG[0];
	rep(i, 1, n) rep(j, 0, m)
		if (BP[i][j].second != M[j] * BG[i])
			return diff(i);
	return same();
}
...
void go()
{
	...
	// All primes occur in all b-values (so m <= 9)
	if (!all())
	{
		cout << "-1";
		return;
