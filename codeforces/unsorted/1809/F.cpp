...
void go()
{
	...
	auto it = find(ALL(B), 1);
	if (it == end(B))
	{
		i64 ans = accumulate(ALL(A), 0ll) * 2;
		rep(i, 0, n)
			cout << ans << ' ';
		cout << '\n';
		return;
	...
	vvi32 I;
	for (int i = 0, j = it - begin(B); i < n; i++, j++)
	{
		if (j == n)
			j = 0;
		if (B[j] == 1)
	...
	rep(i, 0, m)
	{
		...
		for (int j : I[i])
		{
			...
			if (l > k)
				c += min(l - k, A[j]);
			P[i].push_back(P[i].back() + c);
		}
		tc += P[i].back();
		for (int j : I[i])
		{
			S[i].push_back(j == I[i][0] ? P[i].back() : l * 2);
			l -= A[j];
		}
	}
	...
	rep(i, 0, m)
		rep(j, 0, size(I[i]))
			ans[I[i][j]] = tc - P[i].back() + P[i][j] + S[i][j];
	for (i64 a : ans)
		cout << a << ' ';
	cout << '\n';
}