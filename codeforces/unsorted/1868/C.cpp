...
{
	int r = 63 - __builtin_clzll(n);
	i64 bit = (i64)1 << r;
	if (n & (bit / 2))
		return { bit - 1, n - bit };
	else
		return { n - (bit + 1) / 2, (bit - 1) / 2 };
}

void rec(i64 n)
{
	auto [fit, ins] = F.try_emplace(n);
	if (!ins)
		return;
	auto git = G.try_emplace(n).first;
	if (n == 0)
		return;
	auto [l, r] = split(n);
	rec(l);
	...
	auto const &lf = F.find(l)->second, &rf = F.find(r)->second;
	auto const &lg = G.find(l)->second, &rg = G.find(r)->second;
	auto &f = fit->second, &g = git->second;

	auto gs = max(lg.size(), rg.size());
	g.resize(gs + 1);
	g[0] = 1;
	rep(i, 0, lg.size())
		g[i + 1] = lg[i];
	rep(i, 0, rg.size())
		g[i + 1] = (g[i + 1] + rg[i]) % mod;

	auto gg = conv(lg, rg);
	auto fs = max({ lf.size(), rf.size(), gg.size() + 3, lg.size() + 2, rg.size() + 2, (size_t)2 });
	f.resize(fs);
	rep(i, 0, lf.size())
		f[i] = lf[i];
	rep(i, 0, rf.size())
		f[i] = (f[i] + rf[i]) % mod;
	rep(i, 0, gg.size())
		f[i + 3] = (f[i + 3] + gg[i]) % mod;
	rep(i, 0, lg.size())
		f[i + 2] = (f[i + 2] + lg[i]) % mod;
	rep(i, 0, rg.size())
		f[i + 2] = (f[i + 2] + rg[i]) % mod;
	f[1] = (f[1] + 1) % mod;
}
...
void go()
{
	...
	i64 ans = 0;
	auto const &f = F.find(n)->second;
	i64 inv = modpow(m, mod - 2);
	i64 prv = 0;
	for (i64 x = 1; x <= m; x++)
	{
		i64 y = x * inv % mod;
		i64 cur = 0;
		for (auto i = f.size(); i-- > (size_t)0;)
			cur = (cur * y % mod + f[i]) % mod;
		ans = (ans + (mod + cur - prv) % mod * x % mod) % mod;
		prv = cur;
	}
