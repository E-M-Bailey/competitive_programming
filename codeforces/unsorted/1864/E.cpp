...
i64 mpow(i64 a, i64 b, i64 st)
{
	i64 ans = st % MOD;
	for (a %= MOD; b; a = a * a % MOD, b /= 2)
		if (b % 2)
			ans = ans * a % MOD;
	return ans;
}
...
{
	i64 ans = 0;
	for (i64 i; a != 0 && b != 0 && (i = __builtin_clzll(a)) == __builtin_clzll(b);)
	{
		i = ~(1 << (63 - i));
		a &= i;
		b &= i;
...
i64 self_val(pi64 b)
{
	i64 pc = __builtin_popcountll(b.first);
	i64 ans = b.second * (i64)b.second;
	return (pc + 1) * ans;
}

i64 val(pi64 s)
{
	if (s.first == LLONG_MAX)
		return 0;
	return max((i64)3, s.first * 2 + 3) * s.second;
}
...
void go()
{
	...
	vpi64 S;
	i64 cur = 0;
	S.emplace_back(LLONG_MAX, B[0].second - 1);
	rep(i, 1, m)
	{
		auto [a, f] = B[i - 1];
		auto [b, g] = B[i];
		...
		cur += val(pi64(l, mul));
		S.emplace_back(l, mul);
		S.emplace_back(LLONG_MAX, g - 1);
		ans += cur * g;
	}
	i64 den = (i64)n * n;
	i64 g = gcd(ans, den);
	ans = mpow(den / g, MOD - 2, ans / g);
	cout << ans << '\n';
}
