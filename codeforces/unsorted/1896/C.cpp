...
void go()
{
	...
	cin >> B;
	sort(ALL(B));
	rotate(begin(B), begin(B) + x, end(B));
	rep(i, 0, n)
		x -= B[i] < A[i].first;
	if (x != 0)
	{
	...
	{
		cin >> b;
		p = int(upper_bound(ALL(A), pair{ b, n }) - begin(A));
	}
	sort(ALL(B));
	vpi32 O;
	dpi32 I;
	...
	rep(i, 0, m)
	{
		if (x > 0)
		{
			ans[A[r--].second] = I.back().first;
			I.pop_back();
			x--;
		}
		else
		{
			ans[A[l++].second] = I.front().first;
			I.pop_front();
		}
	}
	rep(i, l, r + 1)
		ans[A[i].second] = O[i - l].first;
	cout << "YES\n";
	for (int a : ans)
		cout << a << ' ';
	cout << '\n';
