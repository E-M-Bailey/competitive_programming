...
int PS[3001], PT[3001];
// len, l1 -> min k
int DP1S[3001][3001], DP1T[3001][3001];
// len, k -> max l1
int DP2S[3001][3001], DP2T[3001][3001];
// len, l0 -> max l1 s.t. l1 precedes l0
int DP3S[3001][3001], DP3T[3001][3001];
...
void go()
{
	...
	rep(i, 0, n + 1)
	{
		...
		rep(l1, 0, i + 1)
		{
			DP1S[i][l1] = l1 - PS[i] + PS[i - l1];
			DP1T[i][l1] = l1 - PT[i] + PT[i - l1];
			if (i > l1)
			{
				DP1S[i][l1] = min(DP1S[i][l1], DP1S[i - 1][l1]);
				DP1T[i][l1] = min(DP1T[i][l1], DP1T[i - 1][l1]);
			}
			while (sk < DP1S[i][l1]) DP2S[i][sk++] = l1 - 1;
			while (tk < DP1T[i][l1]) DP2T[i][tk++] = l1 - 1;
		}
		while (sk <= n) DP2S[i][sk++] = i;
		while (tk <= n) DP2T[i][tk++] = i;
		rep(l0, 0, i + 1)
		{
			int scost = PS[i] - PS[i - l0];
			int tcost = PT[i] - PT[i - l0];
			DP3S[i][l0] = scost > k ? -1 : DP2S[i - l0][k - scost];
			DP3T[i][l0] = tcost > k ? -1 : DP2T[i - l0][k - tcost];
			if (i > l0)
			{
			DP3S[i][l0] = max(DP3S[i][l0], DP3S[i - 1][l0]);
			DP3T[i][l0] = max(DP3T[i][l0], DP3T[i - 1][l0]);
			}
		}
	}
	...
	rep(j, 1, n + 1)
	{
		...
		rep(l0, 0, n + 1)
		{
		    int l1 = max(DP3S[n][l0], DP3T[n][l0]);
		    if (l1 >= 0)
		        ans = max(ans, j * l0 + l1);
		}
		cout << ans << ' ';
