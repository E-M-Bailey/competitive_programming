...
struct Node
{
	...
	//ll s;      // sum
	//int l = 0; // lazy value
	Node(int val, ll pr) : p(pr), c{}, t(T), v(val) {}
...
template<bool Pd, class F> void each(Node *n, F &&f)
{
	if (Pd ? push(n) : n)
	{
		each<Pd, F>(n->c[0], forward<F>(f));
		forward<F>(f)(n);
		each<Pd, F>(n->c[1], forward<F>(f));
	}
}
Node *insert(Node *r, int key)
{
	auto [a, bc] = split(r, [key](Node *nd) { return nd->v >= key; });
	auto [b, c] = split(bc, [key](Node *nd) { return nd->v > key; });
	return join(a, join(b ? : new Node(key), c)); // memory leak!
}
void dbg(Node *nd)
{
	each<false>(nd, [](Node *n) { cerr << n << '_' << n->v << '_' << n->n << ' '; });
}
...
struct tree
{
	...
	int mk_ch(int p)
	{
		...
		P.emplace_back();
		P[ans].push_back(p);
		while (P[ans].size() <= P[P[ans].back()].size())
			P[ans].push_back(P[P[ans].back()][P[ans].size() - 1]);
		return ans;
	}
	int p(int nd, int ct)
	{
...
void go()
{
	...
	while (q--)
	{
		...
		if (c == '+')
		{
			...
			cin >> x;
			++T;
			roots.push_back(insert(roots[st.back()], x));
			tr.mk_ch(st.back());
			st.push_back(T);
		}
		else if (c == '-')
		{
			int k;
			cin >> k;
			int ver = tr.p(st.back(), k);
			st.push_back(ver);
		}
		else if (c == '!')
			st.pop_back();
		else
			cout << (roots[st.back()] ? roots[st.back()]->n : 0) << endl;
	}
}
