...
struct ms_tree
{
	...
	ms_tree() = default;
	ms_tree(T const &data) : m((int)size(data)), n(rup2(m)), s(2 * n), mn(2 * n), mx(2 * n)
	{
		rrep(i, m, n)
		{
			mn[n + i] = { INT_MAX, i };
	...
	int last_lt(int lo, int hi, int nlo, int nhi, int i, int val)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo == nlo && hi == nhi)
		...
		int mid = (nlo + nhi) / 2;
		int ans = last_lt(lo, hi, mid, nhi, i * 2 + 1, val);
		if (ans < 0)
			ans = last_lt(lo, hi, nlo, mid, i * 2, val);
		return ans;
	}
	int last_lt(int lo, int hi, int val)
	{
		return last_lt(lo, hi, 0, m, 1, val);
	}
	int last_gt(int lo, int hi, int nlo, int nhi, int i, int val)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo == nlo && hi == nhi)
		...
		int mid = (nlo + nhi) / 2;
		int ans = last_gt(lo, hi, mid, nhi, i * 2 + 1, val);
		if (ans < 0)
			ans = last_gt(lo, hi, nlo, mid, i * 2, val);
		return ans;
	}
	int last_gt(int lo, int hi, int val)
	{
		return last_gt(lo, hi, 0, m, 1, val);
	}
} MST;
