...
void dfs(int v, int lim)
{
	...
	const auto cst = [&](int i, int ct) {
		if (ct == 0)
			return C[G[v][i]][0];
		else if (ct == 1)
			return C[G[v][i]][0] - S[G[v][i]];
		else
			return C[G[v][i]][ct - 1] - S[G[v][i]];
	};

	const auto cmp = [&](int i, int j)
	{
		return cst(i, I[i]) - cst(i, I[i] + 1) < cst(j, I[j]) - cst(j, I[j] + 1);
	};
	...
	rep(i, 0, lim)
	{
		...
		int j = Q.top();
		Q.pop();
		//cerr << "Adding " << v << ' ' << j << ' ' << I[j] << ' ' << C[v].back() << ' ' << cst(j, I[j]) << ' ' << cst(j, I[j] + 1) << endl;
		C[v].push_back(C[v].back() - cst(j, I[j]) + cst(j, I[j] + 1));
		I[j]++;
		if (I[j] < C[G[v][j]].size())
			Q.push(j);
	}
