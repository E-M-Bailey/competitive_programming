...
struct dso
{
	...
	vi32 P, S;
	vbl A;
	dso(int N): n(N), k(n), P(n), S(n, 1), A(n)
	{
		iota(ALL(P), 0);
	}
	int find(int x)
...
void go()
{
	...
	rep(i, 0, m)
	{
		...
		x--;
		y--;
		E[max(A[x], A[y])].emplace_back(x, y);
	}
	for (int a : A)
		if (a == n)
		{
	...
	for (int c = 0; c < n; c++)
	{
		...
		for (auto [x, y] : E[c])
		{
			...
			if (D.A[x] && sx >= c)
			{
				//cerr << " inserting " << sx - c << ' ' << Q.size() << endl;
				Q[sx - c].insert(x);
			}
		}
		for (int x : Q[0])
	...
	/*
	sort(ALL(E), [&](pi32 a, pi32 b) -> bool { return max(A[a.first], A[a.second]) < max(A[b.first], A[b.second]); });
	...
	for (;;)
	{
		...
		{
			//cerr << "yesno " << j << endl;
			cout << (min(A[i], A[j]) == 0 ? "YES\n" : "NO\n");
			return;
		}
		else if (G[j].size() == 1)
			Q.emplace(G[j].begin()->second, j);
	}*/
}
