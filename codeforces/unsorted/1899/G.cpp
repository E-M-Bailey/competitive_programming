...
public:
	...
	inline rmq(It1 first, It2 last, Op const &cmp = Op{}) : A(first, last), M(A.size()), S(A.size()), op(cmp)
	{
		m = (int)A.size() / 64;
		uint64_t mask = 0;
		for (int i = (int)A.size(); i-- > 0; mask <<= 1)
		{
			while (mask && op(A[i], A[i + __builtin_ctzll(mask & -mask)]))
				mask &= mask - 1;
			M[i] = mask |= 1;
		}
		for (int i = 0; i < m; i++)
...
rmq(It1, It2, Op) -> rmq<typename iterator_traits<It1>::value_type, Op>;
*/
...
struct merge_sort_tree
{
	int n;
	vector<vector<int>> s;
	merge_sort_tree(vector<int> const &A) : n(rup2((int)A.size())), s(n * 2)
	{
		rep(i, 0, (int)A.size())
			s[i + n] = { A[i] };
		rrep(i, 1, n)
		{
			s[i].resize(size(s[i * 2]) + size(s[i * 2 + 1]));
			merge(ALL(s[i * 2]), ALL(s[i * 2 + 1]), begin(s[i]));
		}
	}
	bool intersects(int l, int r, int nl, int nr, int i, int lo, int hi)
	{
		l = max(l, nl);
		r = min(r, nr);
		if (l >= r)
		...
		if (l == nl && r == nr)
		{
			auto it = lower_bound(ALL(s[i]), lo);
			return it != end(s[i]) && *it < hi;
		}
		int nm = (nl + nr) / 2;
		return intersects(l, r, nl, nm, i * 2, lo, hi) || intersects(l, r, nm, nr, i * 2 + 1, lo, hi);
	}
	bool intersects(int l, int r, int lo, int hi)
	{
		//cerr << l << ' ' << r << ' ' << lo << ' ' << hi << ": " << intersects(l, r, 0, n, 1, lo, hi) << endl;
		return intersects(l, r, 0, n, 1, lo, hi);
	}
};
...
void go()
{
	...
	//auto m = rmq(ALL(L), less<int>{});
	//auto M = rmq(ALL(L), greater<int>{});
	merge_sort_tree T(L);
	...
	while (q--)
	{
		...
		l--;
		x--;
		//int lo = m.query(l, r), hi = M.query(l, r);
		int lo = L[Q[x]], hi = R[Q[x]];
		//cerr << l << ' ' << r << ' ' << lo << ' ' << hi << ' ' << lo2 << ' ' << hi2 << endl;
		cout << (T.intersects(l, r, lo, hi) ? "Yes\n" : "No\n");
		//cout << (lo2 <= hi && lo <= hi2 ? "Yes\n" : "No\n");
	}
	cout << '\n';
}
