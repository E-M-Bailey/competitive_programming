...
void go()
{
	...
	vvvi64 DP(n + 1, vvi64(m + 1, vi64(k)));
	DP[0][0][0] = 1;
		...
		rep(c, 0, m + 1)
	{
		...
		rrep(p, 1, k)
		{
			acc = (acc + DP[i][c][p]) % MOD;
			DP[i + 1][c][p] = (DP[i + 1][c][p] + acc) % MOD;
			DP[i + 1][c][p] = (DP[i + 1][c][p] + DP[i][c][p - 1] * (k - p + 1)) % MOD;
		}
		if (c < m)
			DP[i + 1][c + 1][0] = (DP[i + 1][c + 1][0] + DP[i][c][k - 1]) % MOD;
	}

	i64 ans = 0;
	rep(c, 1, m + 1)
		rep(p, 0, k)
			ans = (ans + DP[n][c][p] * c) % MOD;
	cout << ans;
	...
	/*
		rep(l, 1, k)
		{
			...
			DP[l][m] = S[l][m];
			rep(c, 0, m)
				DP[l][c] = (MOD + S[l][c] - S[l][c + 1]) % MOD;
		}
		rep(l, k, n + 1)
		{
			S[l][0] = S[l - 1][0] * k % MOD;
			rep(c, 1, m + 1)
				S[l][c] = (S[l - 1][c] * k + DP[l - k][c - 1] * fac) % MOD;
			DP[l][m] = S[l][m];
			rep(c, 0, m)
				DP[l][c] = (MOD + S[l][c] - S[l][c + 1]) % MOD;
		}
