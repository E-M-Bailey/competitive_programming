...
int dfs2(int u, int v, int s)
{
	int ans = s * 2 <= s + S[v] ? v : -1;
	for (int w : G[v])
	{
		if (w == u || L[w] >= 0)
			continue;
		int rec = dfs2(v, w, s + S[v] - S[w]);
		if (rec >= 0)
			return rec;
		if (S[w] * 2 > s + S[v])
			ans = -1;
...
int dfs3(int v, int l)
{
	dfs1(v, v);
	int cv = dfs2(v, v, 0);
	L[cv] = l;
	// cerr << "dfs3 " << v << ' ' << l << ' ' << cv << endl;
	// for (int ll : L)
	// 	cerr << ' ' << ll;
	// cerr << endl;
	S[cv] = S[v];
...
// root white: max_k(sum_i(dp(c_i, k_i) + |k - 2 * k_i|))
// account for root black: dp(v, k) += dp(v, k - 1)

vi32 conv(vi32 a, vi32 b)
{
	if (a.empty())
		return b;
	vi32 ans(a.size() + b.size() - 1);
	rep(i, 0, a.size())
		rep(j, 0, b.size())
...
unordered_map<int, unordered_map<int, vi32>> DP;

vi32 &dfs4(int v, int k)
{
	if (DP[v].count(k))
		return DP[v][k];
	auto &dp = DP[v][k];
