...
struct tree
{
	int n;
	vi64 M, L;
	tree(int N) : n(rup2(N)), M(2 * n), L(2 * n) {}
	void pd(int i)
	{
		M[i] += L[i];
		L[i * 2] += L[i];
	...
	i64 qry(int lo, int hi, int nlo, int nhi, int i)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
		...
		pd(i);
		int mid = (nlo + nhi) / 2;
		return max(qry(lo, hi, nlo, mid, i * 2), qry(lo, hi, mid, nhi, i * 2 + 1));
	}
	i64 qry(int lo, int hi)
	{
		return qry(lo, hi, 0, n, 1);
	}
	void upd(int lo, int hi, int nlo, int nhi, int i, i64 d)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
		...
		pd(i);
		int mid = (nlo + nhi) / 2;
		upd(lo, hi, nlo, mid, i * 2, d);
		upd(lo, hi, mid, nhi, i * 2 + 1, d);
		M[i] = max(M[i * 2] + L[i * 2], M[i * 2 + 1] + L[i * 2 + 1]);
	}
	void upd(int lo, int hi, i64 d)
	{
		upd(lo, hi, 0, n, 1, d);
	}
};
...
void go()
{
	...
	vector<vector<pair<int, i64>>> I(k + 1);
	i64 cst = 0;
	rep(i, 0, n)
	{
		int x, y;
