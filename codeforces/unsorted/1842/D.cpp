...
void go()
{
	...
	for (;;)
	{
		...
		P.emplace_back();
		for (int v = D[n - 1]; v >= 0; v = PR[v])
		{
			P.back().emplace_back(v, A[PR[v]][v]);
			A[PR[v]][v] = -1;
		}
		reverse(ALL(P.back()));
	}
