...
pi64 d(const vi32& A)
{
	...
	i64 a = 0, b = 0;
	rep(i, 0, n - 1)
		a += nCr(n - 2, i) * A[i] % M * ((i + n) % 2 ? -1 : 1);
	rep(i, 0, n - 1)
		b += nCr(n - 2, i) * A[i + 1] % M * ((i + n) % 2 ? -1 : 1);
	return pi64((a % M + M) % M, (b % M + M) % M);
}
