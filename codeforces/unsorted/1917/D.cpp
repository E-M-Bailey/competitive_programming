...
struct FT
{
	...
	{
		// a[pos] += dif
		for (; pos < (ll)size(s); pos |= pos + 1) s[pos] += dif;
	}
	ll query(ll pos)
	{
		pos = max(0ll, min(pos, (ll)size(s)));
	...
	ll lower_bound(ll sum)
	{
		...
		{
			if (pos + pw <= (ll)size(s) && s[pos + pw - 1] < sum)
				pos += pw, sum -= s[pos - 1];
		}
		return pos;
	}
...
ll inversions(vector<int> const &A, int lim)
{
	FT T(lim);
	ll ans = 0;
...
void go()
{
	...
	FT cnt(k * 2 - 1);
	for (int i = 1 - k; i < k; i++)
		cnt.update(i + k - 1, k - abs(i));

	ll ans = inversions(Q, k) * n % MOD;
	//cerr << ans << endl;
	...
	for (ll p : P)
	{
		...
		{
			ll hi = i >= 0 ? p << i : ((p - 1) >> -i) + 1;
			ll lo = (i - 1) >= 0 ? p << (i - 1) : ((p - 1) >> -(i - 1)) + 1;
			//cerr << i << ' ' << lo << ' ' << hi << ' ' << cnt.query(max(0, i + k - 1)) << ' ' << freq.query(hi) - freq.query(lo) << endl;
			ans = (ans + cnt.query(i + k - 1) % MOD * (freq.query(hi) - freq.query(lo))) % MOD;
		}
		freq.update((int)p, 1);
		//cerr << ans << endl;
	}
