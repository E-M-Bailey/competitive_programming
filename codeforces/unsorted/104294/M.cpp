...
void go()
{
	...
	vector<array<vi32, 2>> C(n);
	vector<pair<int, bool>> I(n);

	rep(i, 0, n)
	{
		C[i][0].push_back(i);
		I[i] = make_pair(i, false);
	}

	const auto sz = [&](int i) -> int
	{
		return int(C[i][0].size() + C[i][1].size());
	};

	vbl alone(n, true);
	int minor = n, major1 = 0;
	bool major2 = false;
	int big = 0;
	...
	while (q--)
	{
		...
		else
		{
			...
			{
				big -= max(C[ai][0].size(), C[ai][1].size());
				if (C[ai][0].empty() ^ C[ai][1].empty())
				{
					major1--;
				}
			}
			...
			{
				big -= max(C[bi][0].size(), C[bi][1].size());
				if (C[bi][0].empty() ^ C[bi][1].empty())
				{
					major1--;
				}
			}
			...
			if (C[ai][0].empty() ^ C[ai][1].empty())
				major1++;
			else if (C[ai][0].size() && C[ai][1].size())
				major2 = true;
		}
