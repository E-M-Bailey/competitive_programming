...
void go()
{
	...
	rep(i, 1, n)
	{
		...
		G[i].emplace_back(i - 1, 2);
	}
	rep(i, 0, n)
	{
		G[i].emplace_back(X[i] + n, 1);
		G[X[i] + n].emplace_back(i, 1);
	}

	vvi32 D(26 * 26, vi32(N, INT_MAX / 2));
	vbl V(N);
	rep(i, 0, 26 * 26)
	{
		fill(ALL(V), false);
		int *fr = Q + 200000, *bk = fr;
		*bk++ = i + n;
		D[i][i + n] = 0;
		while (bk != fr)
		{
			...
			V[v] = true;
			for (auto [w, wt] : G[v])
				if (int dw = d + wt; !V[w] && dw < D[i][w])
				{
					D[i][w] = dw;
					Q.push_back(wt == 2 ? ~w : w);
				}
		}
	}
