...
typedef __gnu_pbds::tree<int, int, less<int>, __gnu_pbds::rb_tree_tag, __gnu_pbds::tree_order_statistics_node_update> os_tree;

int constexpr MAXN = 300'010;
...
void dfs1(int u, int v)
{
	...
	NS[v] = 1;
	int bi = 0, bs = 0;
	rep(i, 0, (int)NC[v].size())
	{
		int w = NC[v][i];
		ND[w] = ND[v] + 1;
		NC[w].erase(find(begin(NC[w]), end(NC[w]), v));
		dfs1(v, w);
		NS[v] += NS[w];
		if (NS[w] > bs)
		{
...
{
	int a = NL[v], q = p;
	PL[p].insert({ LK[a] = (int)PN[p].size(), a });
	PN[p].push_back(v);
	NP[v] = p;
	LP[a] = p;
	rep(i, 0, (int)NC[v].size())
...
bool dfs3(int v, int a)
{
	if (NL[v] != a)
		return false;
	a++;
	sort(begin(NC[v]), end(NC[v]), [](int u, int w) { return NL[u] < NL[w]; });
	for (int w : NC[v])
	{
		if (!dfs3(w, a))
			return false;
...
void go()
{
	...
	while (a + 1 < n)
	{
		int v = PN[LP[a]][PL[LP[a]].order_of_key(LK[a])];
		bool stop = false;
		for (int w : NC[v])
			stop |= NL[w] > a;
		if (stop)
	...
	rep(b, a + 2, n)
	{
		int v = PN[LP[b]][PL[LP[b]].order_of_key(LK[b])];
		for (int w : NC[v])
		{
			int c = PL[NP[w]].find_by_order(ND[w] - ND[PN[NP[w]][0]])->second;
			if (c > a && c < b)
			{
				cout << "NO";
				return;
	...
	for (; a >= 0; a--)
	{
		int p = LP[a], k = LK[a], i = (int)PL[p].order_of_key(k), v = PN[p][i];
		ans += ND[v];
		for (;;)
		{
			if (i > 0)
			...
			if (v == 0)
				break;
			int u = NU[v], q = NP[u], j = ND[u] - ND[PN[q][0]];
			auto bit = PL[q].find_by_order(j);
			int l = bit->first, b = bit->second;
			PL[p].find(k)->second = b;
			bit->second = a;
			LP[b] = p;
			LP[a] = p = q;
			LK[b] = k;
