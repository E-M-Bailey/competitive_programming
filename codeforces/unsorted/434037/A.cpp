...
struct Tree
{
	int n, m;
	vvi64 S, L;
	Tree(vi64&& LV): n(size(LV))
	{
		S.push_back(forward<vi64>(LV));
		L.emplace_back(n);
	}
}*/
...
void go()
{
	...
	rep(i, 0, n / 2)
	{
		...
		S[1 + min(A[i].first, A[i].second)]++;
		E[k + max(A[i].first, A[i].second)]++;
		//cerr << A[i].first << ' ' << A[i].second << ' ' << A[i].first + A[i].second << ' ' << 1 + min(A[i].first, A[i].second) << ' ' << k + max(A[i].first, A[i].second) << endl;
	}
	int ans = n;
	int s = 0;
	for (int i = 2; i <= 2 * k; i++)
	{
		s += S[i];
		//cerr << i << ' ' << S[i] << ' ' << E[i] << ' ' << s << ' ' << F[i] << ' ' << n - s - F[i] << endl;
		ans = min(ans, n - s - F[i]);
		s -= E[i];
	}
	cout << ans << endl;
