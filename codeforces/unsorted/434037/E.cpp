...
mnt cnt(int n, int f)
{
	//cerr << n << ' ' << f << endl;
	if (n < f) return 0;
	//cerr << ' ' << F[n - 1] / (F[f - 1] * F[n - f]) << endl;
	return F[n - 1] / (F[f - 1] * F[n - f]);
}
...
void go()
{
	...
	cin >> n >> f;
	vi32 PF;
	for (int n2 = n; n2 > 1; n2 /= LP[n2])
		PF.push_back(LP[n2]);
	PF.resize(unique(ALL(PF)) - begin(PF));
	int k = size(PF);
