...
{
	int s, sz, ms, am;
	ent(int val = 0): s(val), sz(1), ms(val), am(0) {}
	ent(const ent& l, const ent& r): s(l.s + r.s), sz(l.sz + r.sz), ms(l.ms), am(l.am)
	{
		if (r.ms + l.s < ms)
		{
			ms = r.ms + l.s;
...
struct Tree
{
	int n, m;
	vvent S;
	Tree(int N): n(rup2(N)), S(1, vent(n))
	{
		while (size(S.back()) > 1)
		{
			vent L;
			L.reserve(size(S.back()) / 2);
			rep(i, 0, size(S.back()) / 2)
				L.emplace_back(S.back()[i * 2], S.back()[i * 2 + 1]);
			S.push_back(move(L));
		}
		m = size(S);
	}
	ent query(int lo, int hi, int nlo, int nhi, int i, int j)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
		...
		ent L = query(lo, hi, nlo, mid, i - 1, j * 2);
		ent R = query(lo, hi, mid, nhi, i - 1, j * 2 + 1);
		return ent(L, R);
	}
	ent query(int lo, int hi)
	{
		ent ans = query(lo, hi, 0, n, m - 1, 0);
		ans.am += lo;
		return ans;
	}
	...
	{
		S[0][pos] = ent(val);
		for (int i = 1, j = pos / 2; i < m; i++, j /= 2)
			S[i][j] = ent(S[i - 1][j * 2], S[i - 2][j * 2 + 1]);
	}
};
...
void go()
{
	...
	for (int x : X)
	{
		while (!A3.empty() && A3.begin()->first <= x)
			A2.insert(A3.extract(A3.begin()));
		while (!A2.empty() && A2.begin()->first < x)
			A1.insert(A2.extract(A2.begin()));
		while (!B3.empty() && B3.begin()->first <= x)
		{
			T.update(B3.begin()->second, 0);
			B2.insert(B3.extract(B3.begin()));
		}
		while (!B2.empty() && B2.begin()->first < x)
		{
			T.update(B3.begin()->second, -1);
			B1.insert(B2.extract(B2.begin()));
		}
		int p2 = T.query(pos, n).am;
		if (p2 < )
		while (pos < p2)
		{
			B2.
