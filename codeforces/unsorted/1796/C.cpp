...
void go()
{
	...
	int m = 1 << (len - 1);
	// find [l, i) s.t. for all x, x * 3m/2 < 
	i64 i1 = len == 1 ? (i64)l : *ranges::lower_bound(views::iota(l, r), (i64)r, {}, [len](int x) -> i64 { return (i64)(x * 3) << (len - 2); });
	i64 i2 = *ranges::lower_bound(views::iota(l, r), (i64)r, {}, [len](int x) -> i64 { return (i64)x << (len - 1); });
	i64 ans = ((i1 - l) * len + i2 - i1) % mod;
	//cerr << l << ' ' << i1 << ' ' << i2 << ' ' << r << endl;
	cout << ans << endl;
}