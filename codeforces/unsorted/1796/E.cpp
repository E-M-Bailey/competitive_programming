...
// C: upper bound on min path length from shortest terminated path in subtree
// D: length of current path
// u->v->w points away from the root
// min_k: kth
// C(u, v) = min(min C(v, w) + min_2 D(v, w))
// D(u, v) = min D(v, w) + 1
// v a leaf:
// C(u, v) = infinity
// D(u, v) = 1

constexpr int NIL = MAX<int> / 2;

struct vert
{
	int v;
	// two smallest values of C(v, w) for any w
	int c1 = NIL, c2 = NIL;
	// Value of w achieving c1
	int ac1 = -1;
	// three smallest values of D(v, w) for any w
	int d1 = NIL, d2 = NIL, d3 = NIL;
	// Value of w achieving d1 and d2
	int ad1 = -1, ad2 = -1;
	vert(int v): v(v) {}
	void put_c(int c, int w)
	{
		if (c < c1)
		{
			c2 = c1;
	...
	int D2(int u) const
	{
		return u == ad1 || u == ad2 ? d3 : d2;
	}
	bool leaf(int u) const
	{
		return D1(u) == NIL;
	}
	int c(int u) const
	{
		return min(C1(u), D2(u));
	}
	int d(int u) const
	{
		return leaf(u) ? 1 : 1 + D1(u);
	}
	int a() const
	{
		return min(c(-1), d(-1));
...
void go()
{
	...
	// Compute C, D upward then downward
	dfs1(-1, 0);
	int ans = dfs2(-1, 0);
	cout << ans << endl;
}