...
void go()
{
	...
	vva2i32 DP(n + 1, va2i32(m + 1, { 0, MAX<int> / 2 }));
	vva2i32 P(n + 1, va2i32(m + 1));
	for (int i = 0; i <= n; i++)
		for (int j = 0; j <= m; j++)
		{
			if (i > 0 && j > 0 && s[i - 1] == t[j - 1])
			{
				int a = DP[i - 1][j - 1][0] - 1, b = DP[i - 1][j - 1][1] - 1;
				if (a < b)
				{
					DP[i][j][0] = a;
					P[i][j][0] = 0;
			...
			{
				DP[i][j][0] = i == 0 && j == 0 ? 0 : MAX<int> / 2;
				P[i][j][0] = i == 0 ? 3 : 1;
			}
			a2i32 pi = i > 0 ? DP[i - 1][j] : a2i32{MAX<int> / 2, MAX<int> / 2};
			a2i32 pj = j > 0 ? DP[i][j - 1] : a2i32{MAX<int> / 2, MAX<int> / 2};
			array<int, 4> A{pi[0] + 1, pi[1], pj[0] + 1, pj[1]};
			auto it = min_element(ALL(A));
			DP[i][j][1] = *it;
			P[i][j][1] = it - begin(A);
		}

	if (min(DP[n][m][0], DP[n][m][1]) > 0)
	{
		cout << "NO" << endl;
		return;
	}
	cout << "YES" << endl;
	int i = n, j = m, b = DP[n][m][1] < DP[n][m][0];
	string ans;

	//cerr << DP[0][0][0] << ' ' << DP[0][0][1] << endl;
