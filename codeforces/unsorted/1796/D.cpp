...
void go()
{
	...
	cin >> A;
	for (int& a : A) a -= x;
	vvi64 DP(n + 1, vi64(k + 1, MIN<i64> / 2));
	DP[0][0] = 0;
	i64 ans = 0;
	rep(i, 1, n + 1)
	{
		DP[i][0] = max((i64)0, DP[i - 1][0] + A[i - 1]);
		rep(j, 1, k + 1)
		{
			if (j > i)
			{
			...
			i64 a = max((i64)0, DP[i - 1][j]), b = max((i64)0, DP[i - 1][j - 1]);
			DP[i][j] = max((i64)0, b + A[i - 1] + 2 * x);
			if (j < i)
				DP[i][j] = max(DP[i][j], a + A[i - 1]);
		}
		rep(j, 0, k + 1) {
			if (n - i >= k - j)
				ans = max(ans, DP[i][j]);
