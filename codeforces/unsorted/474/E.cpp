...
struct Tree
{
	...
	static constexpr T unit = make_pair(0, -1);
...
void go()
{
	...
	{
		i64 h = H[i];
		pair<int, int> val = Tree::unit;
		{
			auto it = partition_point(begin(ICC), end(ICC), [&](i64 h2) { return h2 < h + d; });
			val = max(val, T.query(it - begin(ICC), m));
		}
		{
			auto it = partition_point(begin(ICC), end(ICC), [&](i64 h2) { return h2 <= h - d; });
			val = max(val, T.query(0, it - begin(ICC)));
		}
		P[i] = val.second;
		DP[i] = val.first + 1;
		T.update(CC[h], { val.first + 1, i });
	}

	int i = max_element(ALL(DP)) - begin(DP);
	cout << DP[i] << '\n';
