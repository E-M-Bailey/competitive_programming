...
void go()
{
	int m;
	cin >> m;
	vector<vector<pair<i64, i64>>> M(m);
	vi64 L(m), R(m), T(m);
	vi64 A;
	rep(i, 0, m)
	{
	...
	int k = (int)size(A);
	i64 lo = accumulate(ALL(L), (i64)0);
	i64 hi = accumulate(ALL(R), (i64)0);
	if (hi - lo + 1 > k)
	{
		cout << "0\n";
		return;
	...
	for (i64 a : A)
	{
		...
		for (int i : I[a])
		{
			i64 with = R[i], without = min(R[i], )
			other -= lower_bound(ALL(M[i]), pair{ a, MIN<i64> })->second;
		}
		best = min(best, )
	}
}
