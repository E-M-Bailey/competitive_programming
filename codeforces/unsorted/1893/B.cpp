...
void go()
{
	...
	vi32 X(n + m);
	copy(ALL(B), copy(ALL(A), begin(X)));
	sort(ALL(X));
	X.erase(unique(ALL(X)), end(X));
	int k = sz(X);
	for (int &a : A)
		a = int(lower_bound(ALL(X), a) - begin(X));
	for (int &b : B)
		b = int(lower_bound(ALL(X), b) - begin(X));
	...
	sort(ALL(B));
	auto mid = upper_bound(ALL(B), mn);
	vi32 C(n + m);
	//cerr << mn << endl;
	reverse_copy(begin(B), mid, copy(ALL(A), reverse_copy(mid, end(B), begin(C))));
	for (int c : C)
		cout << X[c] << ' ';
	cout << '\n';
}
...
void go()
{
	...
	for (int a : A)
	{
		...
		// add b's in [a, *it)
		auto lo = F.lower_bound(a), hi = F.upper_bound(*it);
		int s = sz(C);
		while (lo != hi)
		{
			while (lo->second--)
				C.push_back(lo->first);
			lo = F.erase(lo);
		}
		reverse(s + ALL(C));
		*it = a;
