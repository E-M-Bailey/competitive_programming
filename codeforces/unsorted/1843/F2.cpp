...
struct ent
{
	int s, lm, rm, m, lM, rM, M;

	ent rev() const
	{
		return { s, rm, lm, m, rM, lM, M };
	}

	friend ent operator+(ent const &l, ent const &r)
	{
		return ent
		{
			l.s + r.s,
			min(l.lm, l.s + r.lm),
			min(r.rm, r.s + l.rm),
			min(min(l.m, r.m), l.rm + r.lm),
			max(l.lM, l.s + r.lM),
			max(r.rM, r.s + l.rM),
			max(max(l.M, r.M), l.rM + r.lM)
		};
	}
};

constexpr ent single(int x)
{
	int m = min(0, x), M = max(0, x);
	return ent{ x, m, m, m, M, M, M };
}

ostream &operator<<(ostream &os, ent const &e)
{
	return os << '(' << e.s << '|' << e.lm << ',' << e.rm << ',' << e.m << '|' << e.lM << ',' << e.rM << ',' << e.M << ')';
}

constexpr ent UNIT = single(0);
...
void go()
{
	...
	rep(q, 0, n)
	{
		...
		if (t == '+')
		{
			...
			v--;
			L.emplace_back();
			L.back().emplace_back(v, single(x));
			int d = D[v] + 1;
			D.push_back(d);
			int p;
			ent e;
			for (int i = 0; tie(p, e) = L.back().back(), i < (int)L[p].size(); i++)
			{
				auto [pp, ee] = L[p][i];
				L.back().emplace_back(pp, e + ee);
				// if (D.back() != D[pp] + (1 << (i + 1)))
				// 	exit(-1);
				// cerr << D.back() << ' ' << D[p] << ' ' << D[pp] << endl;
			}
			//cerr << "+" << D.size() << '\n';
			//for (auto [p, e] : L[D.size() - 1])
			//	cerr << ' ' << p + 1 << ' ' << e.s << ' ' << e.lm << ' ' << e.rm << ' ' << e.m << ' ' << e.lM << ' ' << e.rM << ' ' << e.M << endl;
		}
		else
		{
			int u, v, k;
			...
			while (u != v)
			{
				...
				// 	exit(-1);
				int i = 0;
				while ((i + 1) < (int)min(L[u].size(), L[v].size() + 1) / 2 && L[u][i + 1].first != L[v][i + 1].first)
					i++;
				auto [pu, eu] = L[u][i];
				auto [pv, ev] = L[v][i];
				u = pu;
			...
			int x = X[u];
			ent e = l + single(x) + r.rev();
			// cerr << "?" << u + 1 << ' ' << v + 1 << '\n';
			// cerr << l << ' ' << single(x) << ' ' << r << ' ' << e << endl;
			//cerr << ' ' << e.s << ' ' << e.lm << ' ' << e.rm << ' ' << e.m << ' ' << e.lM << ' ' << e.rM << ' ' << e.M << endl;
			cout << (e.m <= k && k <= e.M ? "Yes\n" : "No\n");
		}
	}
}
