...
void go()
{
	...
	rep(q, 0, n)
	{
		...
		if (t == '+')
		{
			...
			X.push_back(x);
			v--;
			L.push_back({ pair(v, ent{x, x, x, x, x, x, x}) });
			int d = D[v] + 1;
			D.push_back(d);
			for (int i = 0; ; i++)
			{
		...
		else
		{
			...
			ent l{ 0, INT_MAX / 2, INT_MAX / 2, INT_MAX / 2, INT_MIN / 2, INT_MIN / 2, INT_MIN / 2 }, r = l;
			while (D[u] > D[v])
			{
				int i = 0, dd = 1;
				while (D[u] - dd * 2 >= D[v])
			...
			{
				int i = 0;
				while ((i + 1) < (int)L[u].size() && L[u][i + 1].first != L[v][i + 1].first)
					i++;
				auto [pu, eu] = L[u][i];
				auto [pv, ev] = L[v][i];
				u = pu;
			...
			int x = X[u];
			ent e = l + ent{x, x, x, x, x, x, x} + r;
			// cerr << "?" << u + 1 << ' ' << v + 1 << '\n';
			// cerr << ' ' << e.s << ' ' << e.lm << ' ' << e.rm << ' ' << e.m << ' ' << e.lM << ' ' << e.rM << ' ' << e.M << endl;
			cout << (k == 0 || (e.m <= k && k <= e.M) ? "Yes\n" : "No\n");
		}
	}
}
