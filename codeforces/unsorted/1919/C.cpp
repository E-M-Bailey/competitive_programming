...
{
	typedef int T;
	static constexpr T unit = INT_MAX / 4;
...
// DP_j(x): min cost achievable w/ j most recent index, x most recent value
// DP_0(inf) = 0
// DP_j+1(x) = DP_j(x) + [A[j] < A[j + 1]] (easy w/ offset)
// DP_j+1(A[j]) min= min(1 + min_x<A[j + 1] DP_j(x), min_x>=A[j + 1] DP_j(x))
...
void go()
{
	...
	{
		int a = A[i];
		int qry = offset + min(1 + T.query(0, a), T.query(a, n + 2));
		offset += prv < a;
		T.update(prv, min(T.query(prv, prv + 1), qry - offset));
		//rep(j, 0, n + 2)
		//	cerr << T.query(j, j + 1) + offset << ' ';
		//cerr << '\n';
		prv = a;
	}
	cout << offset + T.query(0, n + 2) << '\n';
	...
	rep(i, 0, n)
	{
		...
		int c = A[i];
		int d = i == n - 1 ? 0 : A[i + 1];
		bool good_cur = (b < c) + (c < d) > (b < d), good_prv = pv >= 0;
		//cerr << good_cur << ' ';
		if (good_cur && good_prv)
		{
			if (a < d)
			{
				int cv = T.query(c, n + 2) + 1;
				T.update(pi, pv);
				pi = c;
				pv = cv;
			}
