...
// p(S) = max[v in S](p(v) + sum[w in S\{v}](p(S\{v,w}))) / |S|

// |S| = 2: p(a) / 2
// |S| = 3: p(a) / 3 + p(b) / 3 + p(c) / 3
// |S| = 4: p(a) / 4 + p(b) / 4 + p(c) / 8
// |S| = 5: p(a) / 5 + p(b) / 5 + ...
// |S| = 6: p(a) / 6 + p(b)
...
vector<F> const &dis(int n)
{
	static vector<vector<F>> D{ vector<F>{}, vector<F>{1}, vector<F>{0.5, 0} };
	while ((int)D.size() <= n)
	{
		int i = (int)D.size();
		D.emplace_back(i);
		...
		D[i][0] = D[i][1] = iinv;
		rep(j, 2, i - 1)
			D[i][j] = (D[i - 2][j - 2] * (j - 1) + D[i - 2][j - 1] * (i - 1 - j)) * iinv;
		D[i][i - 1] = D[i - 2][i - 3] * (i - 2) * iinv;
	}
	return D[n];
}
