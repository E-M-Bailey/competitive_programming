...
array<uint8_t, 1 << 14> D[1 << 14];
// a b ab m am bm abm

//int sw_m(int msk)
//{
//	int x = msk & 0x7f, y = msk >> 7;
//	y = (y >> 4) | ((y ^ 0x8) & 0x8) | ((y & 0x7) << 4);
//	return x | (y << 7);
//}
	...
	rep(i, 0, 1 << 14)
	{
		...
		while (!Q.empty())
		{
			...
			while (s--)
			{
				int v = Q.front();
				Q.pop();
				for (int w : { v &(v >> 7 | 0x3f80), v | (v >> 7), v ^ ((v & 0x7f) << 7), v ^ 0x3c00 })
				{
					if (D[i][w] > d)
					{
						D[i][w] = d;
...
void go()
{
	...
	int A = a & ~b & ~m, B = b & ~a & ~m, AB = a & b & ~m, M = m & ~a & ~b, AM = a & m & ~b, BM = b & m & ~a, ABM = a & b & m;
	...
	int xx = 0x3fff ^ (int)X.to_ulong();
	for (int g = xx;; g = (g - 1) & xx)
	{
		dist = min(dist, D[S.to_ulong()][T.to_ulong() ^ g]);
		if (g == 0)
			break;
	}

	if (cc != c || dd != d || dist == UINT8_MAX)
	{
		cout << "-1\n";
		return;
	}
