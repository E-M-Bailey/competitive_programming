...
void go()
{
	...
	vvi64 DP(n + 1, vi64(k + 1));

	vi64 xpp(n + 1), ypp(n + 1), zpp(n + 1, -1);
	vi64 xpn(n + 1), ypn(n + 1), zpn(n + 1, -1);
	vi64 xnp(n + 1), ynp(n + 1), znp(n + 1, -1);
	vi64 xnn(n + 1), ynn(n + 1), znn(n + 1, -1);
	...
	rep(i, 1, n + 1)
	{
		rep(j, 1, k + 1)
		{
			...
			i64 z = DP[i][j];
			i64 a = A[i - 1], b = B[i - 1];
			DP[i][j] = max(DP[i][j], DP[i - 1][j - 1] + 2 * abs(a - b));
			if (zpp[i - j] >= 0) DP[i][j] = max(DP[i][j], zpp[i - j] + abs(xpp[i - j] - b) + abs(ypp[i - j] - a));
			if (zpn[i - j] >= 0) DP[i][j] = max(DP[i][j], zpn[i - j] + abs(xpn[i - j] - b) + abs(ypn[i - j] - a));
			if (znp[i - j] >= 0) DP[i][j] = max(DP[i][j], znp[i - j] + abs(xnp[i - j] - b) + abs(ynp[i - j] - a));
			if (znn[i - j] >= 0) DP[i][j] = max(DP[i][j], znn[i - j] + abs(xnn[i - j] - b) + abs(ynn[i - j] - a));
			//cerr << zpp[i - j] << ' ' << xpp[i - j] << ' ' << ypp[i - j] << ' ';
			if (zpp[i - j] >= 0 || z + a + b > zpp[i - j] + xpp[i - j] + ypp[i - j]) tie(xpp[i - j], ypp[i - j], zpp[i - j]) = { a, b, z };
			if (zpn[i - j] >= 0 || z + a - b > zpn[i - j] + xpn[i - j] - ypn[i - j]) tie(xpn[i - j], ypn[i - j], zpn[i - j]) = { a, b, z };
			if (znp[i - j] >= 0 || z - a + b > znp[i - j] - xnp[i - j] + ynp[i - j]) tie(xnp[i - j], ynp[i - j], znp[i - j]) = { a, b, z };
			if (znn[i - j] >= 0 || z - a - b > znn[i - j] - xnn[i - j] - ynn[i - j]) tie(xnn[i - j], ynn[i - j], znn[i - j]) = { a, b, z };
			//cerr << DP[i][j] << ' ';
		}
		//cerr << endl;
	}
