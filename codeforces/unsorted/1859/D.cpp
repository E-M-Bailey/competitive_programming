...
void go()
{
	...
	vector<int> FP;
	for (int i = 0; i < n; i++)
		if (i == n - 1 || A[i + 1].first > A[i].second)
			FP.push_back(A[i].second);
	...
	while (q--)
	{
		int x;
		cin >> x;
		auto it = partition_point(ALL(A), [x](pi32 ab) { return ab.first <= x; });
		if (it == begin(A) || prev(it)->second <= x)
			cout << x << ' ';
		else
			cout << *lower_bound(ALL(FP), x) << ' ';
	}
	cout << '\n';
