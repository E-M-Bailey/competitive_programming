...
struct dat
{
	...
	vusi32 IF;
	si32 D;
	dat(const vi32& A): IF(A.size())
	{
		//cerr << "size " << IF.size() << endl;
		for (int a : A)
		{
			F.emplace(a, 0);
			IF[0].insert(a);
	...
	int mad()
	{
		return D.empty() ? 0 : *prev(D.end());
	}
};*/

/*
mt19937_64 randy(duration_cast<nanoseconds>(high_resolution_clock::now().time_since_epoch()).count());
uniform_int_distribution<i64> distr(MIN<i64>, MAX<i64>);
...
struct Node
{
	...
	int up, c;
	i64 y;
	Node(int lo, int hi): lo(lo), hi(hi), up(hi), c(1), y(distr(randy)) {}
	void recalc();
};

int cnt(Node* n) { return n ? n->c : 0; }
int upp(Node* n) { return n ? n->up : MIN<int>; }
void Node::recalc() { c = cnt(l) + cnt(r) + 1; up = max({ upp(l), upp(r), hi }); }

template<class F> void each(Node* n, F f)
{
	if (n) { each(n->l, f); f(n->lo); each(n->r, f); }
}
...
int overlap(Node* n, int l, int r)
{
	int ans = -1;
	if (n->l )
}*/
...
entry operator+(const entry& l, const entry& r)
{
	entry ans;
	if (l.M < 0)
	return l.M < r.M ? r : l;
}
constexpr entry unit{ 0, -1 };
...
void insert(node*& r, node* x) {
	...
	bool d = cmp(r, x);
	insert(r->c[d], x);
	int del = height(r->c[1]) - height(r->c[0]);
	if (del < -1)
		rot(r)
}
...
void go()
{
	...
	Tree T(n);
	for (auto [u, v] : E)
		T.update(Tin[v], { Tout[v], Tin[v] });

	map<int, vi32, greater<>> I;
	rep(i, 0, n)
		I[A[i]].push_back(i);
	vi32 out(n), in(n);
	const auto upd = [&](int l, int r, int a, Tree& T, vi32& ans)
	{
		entry e;
		while ((e = T.query(0, r)).M > l)
	};

	for (const auto& [a, II] : I)
	{
		if (size(II) < 2)
			continue;
		entry e;
		...
		int k = size(II);
		int i1 = II[0], i2 = II[1], i2 = II[k - 2], i3 = II[k - 1];
		while ((e = T.query(i2 + 1, n)).M >= 0)
		{
			T2.update(e.aM, unit);
			in[P[e.aM]] = a;
		}
		while ((e = T.query(0, i3)).M >= 0)
		{
			T2.update(e.aM, unit);
			in[P[e.aM]] = a;
		}
	...
	int b = isqrtc(n);
			*/
			...
			/*
		const auto cmp = [&E, b](int i, int j)
		{
			auto [u1, v1] = E[i];
			auto [u2, v2] = E[j];
			int l1 = Tin[v1], r1 = Tout[v1], l2 = Tin[v2], r2 = Tout[v2];
			return tuple(l1 / b, r1, l1 % b) < tuple(l2 / b, r2, l2 % b);
		};*/
		//sort(ALL(Q), cmp);
	...
	/*
		...
		for (int q : Q)
		{
			int v = E[q].second, cl = Tin[v], cr = Tout[v];
			//cerr << "Processing [" << cl << ',' << cr << ')' << endl;
			while (r < cr)
			{
				out.erase(A[P[r]]);
				in.insert(A[P[r]]);
