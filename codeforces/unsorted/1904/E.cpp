...
// keywords: link cut tree lct subtree aggregation
// Based on https://codeforces.com/blog/entry/67637
const int N = 200010;
int sta[N], top;
...
struct ms
{
	...
	int max()
	{
		return freq.empty() ? 0 : rbegin(freq)->first;
	}
};

struct Node
{
	int ch[2], fa, height = 0;
	ms vir;
	bool rev;
} t[N];
...
void access(int x)
{
	...
	{
		Splay(x);
		t[x].vir.add(t[t[x].ch[1]].height);
		t[x].ch[1] = y;
		t[x].vir.rem(t[t[x].ch[1]].height);
		pushup(x);
	}
}
...
void pushup(int x)
{
	t[x].height = max({ t[t[x].ch[0]].height, t[t[x].ch[1]].height, t[x].vir.max() }) + 1;
}
