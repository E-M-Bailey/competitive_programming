...
/*struct Node
{
	struct T { int m, M; }; // data type
	typedef T L; // lazy type
	const T tneut{ INT_MAX, 0 };     // neutral elements
	const L lneut = { -1, -1 };
	T f(T a, T b) { return { min(a.m, b.m), max(a.M, b.M) }; } // (any associative fn)
	T apply(T a, L b)
	{
		if (b.m >= 0) a.m = b.m;
		if (b.M >= 0) a.M = b.M;
...
public:
	...
	template<class It1, class It2>
	inline void assign(It1 first, It2 last)
	{
		A.assign(first, last);
		M.resize(A.size());
		S.resize(A.size());
...
void go()
{
	...
	for (int i : I)
	{
		...
		{
			int mid = (lo + hi) / 2;
			int m = BB.query(mid, i);
			int M = ~AA.query(mid, i);
			if (M <= A[i] && m >= A[i])
				hi = mid;
			else
				lo = mid + 1;
		...
		{
			int mid = (lo + hi + 1) / 2;
			int m = BB.query(i, mid);
			int M = ~AA.query(i, mid);
			if (M <= A[i] && m >= A[i])
				lo = mid;
			else
				hi = mid - 1;
