...
// f(l, r): number of choices for [l, r)
// f(l, r) = f(l, mid) + f(mid + 1, r) + f(l, mid) * f(mid + 1, r) - 1
// f(l, n) = f(mid + 1, n) + f(l, mid) * f(mid + 1, m)
// f(0, r) = f(0, mid) + f(0, mid) * f(mid + 1, r)
...
i64 solve(int l, int r)
{
	if (l == r)
		return 1;
	int mid = R.argquery(l, r);
	i64 a = solve(l, mid), b = solve(mid + 1, r);
	//cerr << l << ' ' << r << ' ' << a << ' ' << b << endl;
	if (l == 0)
	{
		if (r == n)
