...
int min_cost_flow(vector<Edge> edges, int s, int t)
{
...
void go()
{
	...
	int const s = n + m, t = s + 1, sp = t + 1, tp = sp + 1;
	...
	rep(i, 0, n) rep(j, 0, m)
	{
		...
		if (M[i][j] == 0)
		{
			// u -> v, cap 1, cost 1, dem 0
			ed.push_back({ u, v, 1, 1 });
		}
		else
		{
			// u -> v, cap 1, cost 0, dem 1
			CA[i]++;
			CB[j]++;
			//ed.push_back({ sp, v, 1, 0 });
			//ed.push_back({ u, tp, 1, 0 });
			k++;

			// v -> u, cap 1, cost 1, dem 0
			ed.push_back({ v, u, 1, 1 });
		}
	}
	rep(i, 0, n)
	{
		if (CA[i])
			ed.push_back({i, tp, CA[i], 0});
	}
	rep(j, 0, m)
	{
		if (CB[j])
			ed.push_back({sp, j + n, CB[j], 0});
	}
	rep(i, 0, n)
	{
		// s -> i, cap A[i], cost 0, dem A[i]
		ed.push_back({ sp, i, A[i], 0 });
	}
	ed.push_back({s, tp, accumulate(ALL(A), 0), 0});
	rep(j, 0, m)
		ed.push_back({ j + n, t, B[j], 0 });
	ed.push_back({ t, s, k, 0 });

	//cerr << k << endl;
	//for (auto [u, v, cap, cst] : ed)
	//	cerr << u << ' ' << v << ' ' << cap << ' ' << cst << endl;

	int cst = min_cost_flow(ed, sp, tp);
	cout << cst << '\n';
}
