...
void go()
{
	...
	rep(i, 0, n)
		P[i] = i64(F[i]) * D[i];
	sort(ALL(P));
	const auto eval = [&](i64 m)
	{
		int a = end(P) - upper_bound(ALL(P), m + k);
		int b = lower_bound(ALL(P), m) - begin(P);
		//cerr << '[' << m << ',' << m + k << ']' << ' ' << a + b << endl;
		return a + b;
	};
	...
	for (i64 p : P)
	{
		i64 st = max(p - k, (i64)1), ed = p + k + 1;

		vvi32 I(ed - st);
		//cerr << st << ' ' << ed << ' ' << I.size() << endl;
		for (int u : U)
		{
			int i = (st + u - 1) / u * u - st;
			//cerr << ' ' << u << ' ' << i + st << ' ' << i << endl;
		...
		for (i64 s = st, i = 0, j = k + 1; s <= p; s++, i++, j++)
		{
			//cerr << s << ' ' << st << ' ' << I.size() << endl;
			//cerr << '[' << s << ',' << s + k << "]:" << endl;
			//cerr << ' ' << in << ' ' << us << endl;
			if (in == us)
				ans = min(ans, eval(s));
