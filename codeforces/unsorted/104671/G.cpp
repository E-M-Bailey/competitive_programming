...
void go()
{
	int n, q;
	cin >> n >> q;
	vector<pair<vector<pair<int, int>>, i64>> A;
	string s;
	...
	rep(i, 0, q)
	{
		...
		{
			A.emplace_back();
			A.back().first.resize(n);
			for (auto &[l, r] : A.back().first)
				cin >> l >> r;
			cin >> A.back().second;
		}
		else
		{
			...
			i64 ans = 0;
			for (auto const &[AA, x] : A)
			{
				i64 cur = 1;
				rep(j, 0, n)
				{
					i64 len = max(0, min(AA[j].second, Q[j].second) - max(AA[j].first, Q[j].first) + 1);
					cur = cur * len % MOD;
				}
				ans = (ans + cur * x) % MOD;
			}
