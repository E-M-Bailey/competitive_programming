...
i64 opt(i64 s, i64 k)
{
	...
	while (hi - lo > 2)
	{
		i64 a = (2 * lo + hi) / 3, b = (lo + 2 * hi + 2) / 3;
		if ((k - 4 * a) * (s + 20 * a) < (k - 4 * b) * (s + 20 * b))
			lo = a;
		else
			hi = b;
	}
	i64 mid = (lo + hi) / 2;
	return max({(k - 4 * lo) * (s + 20 * lo), (k - 4 * mid) * (s + 20 * mid), (k - 4 * hi) * (s + 20 * hi)});
}

i64 f(i64 s, i64 k, i64 mask)
{
	i64 d = s % 10, bit = 1 << d;
	i64 ans = s * k;
	if (d == 0)
	...
	if (d % 2 == 0)
		ans = max(ans, opt(s, k));
	if (!(mask & bit))
		ans = max(ans, f(s + d, k - 1, mask | bit));
	return ans;
}
