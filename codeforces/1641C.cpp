...
struct pl_segtree
{
		...
		n(rup2(n))
	{
		...
		for (int m = 1; m < n; m *= 2)
			...
			{
				node* p = N[i];
				all.push_back(p->l = new node);
				all.push_back(p->r = new node);
			}
	}
...
private:
	int val(int nlo, int nhi, node* N)
	{
		if (N == nullptr)
			return 0;
		else if (N->lz >= 0)
			return N->lz * (nhi - nlo + 1);
		else
			return N->val;
	}
	...
	bool leaf(Node* N)
	{
		return N->l == nullptr && N->r == nullptr;
	}

	node* update(int lo, int hi, int nlo, int nhi, node* N, int val, int lz)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lz < 0 && lo > hi || N == nullptr)
			return N;
		...
		else
		{
			...
			{
				int mid = (nlo + nhi) / 2;
				N2->l = update(lo, hi, nlo, mid, N2->l, val, N2->lz);
				N2->r = update(lo, hi, mid + 1, nhi, N2->r, val, N2->lz);
				N2->val = val(N2->l) + val(N2->r);
			}
			N2->lz = -1;
		}
		return N2;
	...
	int query(int lo, int hi, int nlo, int nhi, node* N)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lz < 0 && lo > hi || N == nullptr)
		...
		{
			int mid = (nlo + nhi) / 2;
			int lSum = query(lo, hi, nlo, mid, N->l);
			int rSum = query(lo, hi, mid + 1, nhi, N->r);
			return lSum + rSum;
		}
	}
...
public:
	...
	{
		V.push_back(update(lo, hi, 0, n - 1, V.back(), val, -1));
	}

	int query(int lo, int hi, int time)
	{
		return query(lo, hi, 0, n - 1, V[time]);
	}
};
...
struct upd1
{
	int l, r, tIns, tRes;
	upd1() = default;
	upd1(int l, int r, int tIns, int tRes) :
		l(l),
		r(r),
		tIns(tIns),
		tRes(tRes)
	...
	friend bool operator<(const upd1& l, const upd1& r)
	{
	...
	for (int i = 0; i < q; i++)
	{
		if (read<int>() == 0)
		{
			...
			if (read<int>() == 0)
				...
				U.emplace_back(l, r, ST.now(), 0);
		}
		else
		{
			int j = read<int>();
			Q.emplace_back(j, ST.now(), U.size() - 1);
		}
	}

	const auto res = [&](const upd1& u, int t)
	{
		return ST.query(u.l, u.r, t) == u.r - u.l;
	}
