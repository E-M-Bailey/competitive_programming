...
struct Tree {
	...
	MSS f(MSS l, MSS r) {
		auto [ls, ll, lr, lm] = l;
		auto [rs, rl, rr, rm] = r;
		return {ls + rs, max(ll, ls + rl), max(rr, rs + lr), max({lm, rm, lr + rl})};
	}
	constexpr T UNIT;
	vvms d;
	int n, m;
	Tree(const vi32& A) :
        ...
        for (int l = 1; l < n; l++)
            for (int j = 0; j < d[l].size(); j++)
                d[l][j] = f(d[l - 1][j * 2], d[l - 1][j * 2 + 1]);
	}
	MSS query(int lo, int hi, int nlo, int nhi, int l, int i) {
        lo = max(lo, nlo);
        hi = min(hi, nhi);
        if (lo >= hi)
            return UNIT;
        if (lo == nlo && hi == nhi)
            return d[l][i];
        int mid = (nlo + nhi) / 2;
        MSS L = query(lo, hi, nlo, mid, l - 1, i * 2);
        MSS R = query(lo, hi, mid, nhi, l - 1, i * 2 + 1);
        if (mask & (1 << l))
            swap(L, R);
        return f(L, R);
	}
    MSS query(int lo, int hi) {
