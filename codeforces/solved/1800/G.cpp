...
void dfs1(int u, int v)
{
	...
	for (int w : G[v])
		dfs1(v, w);
	ranges::sort(G[v], {}, [&](int w) { return make_tuple(H1[w], H2[w], S[w]); });
	SM[v] = true;
	unordered_map<u32, unordered_map<u32, unordered_map<int, vi32>>> F;
	for (int w : G[v])
		F[H1[w]][H2[w]][S[w]].push_back(w);
	int c1 = 0;
	for (const auto& [h1, FF] : F) for (const auto& [h2, FFF] : FF) for (const auto& [s, I] : FFF)
	{
		if (size(I) % 2)
		{
			c1++;
