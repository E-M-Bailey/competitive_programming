...
void go()
{
	...
	const auto eval = [&](i64 x)
	{
		if (x < 0 || x > m)
			return;
		auto [lo, hi] = equal_range(ALL(A), x);
		auto el = lo - begin(A), er = hi - begin(A);

		int l = (int)er - k;
		int r = (int)el + k - 1;

		i64 L = l >= 0 ? (A[l] + x) / 2 + 1 : 0;
		i64 R = r < n ? (A[r] + x + 1) / 2 - 1 : m;
		i64 val = max((i64)0, R - L + 1);
		if (make_pair(val, -x) > make_pair(ans, -best))
		{
			best = x;
			ans = val;
		}
