...
void odd_root(vi32& C, int k)
{
	int c = size(C);
	assert(k == 0 || c % 2);
	i64 p2 = mpow(2, k, c);
	vi32 C2(ALL(C));
	for (int i = 0, j = 0; i != c; i++, j = (j + p2) % c)
		C[j] = C2[i];
}

vi32 merge(const vi32* Cs, int l)
{
	assert(l);
	int c = size(Cs[0]);
	vi32 ans(l * c);
	int idx = 0;
	rep(i, 0, c) rep(j, 0, l) ans[idx++] = Cs[j][i];
	return ans;
}
...
void go()
{
	...
	rep(i, 0, n)
	{
		...
		C[size(cycle)].push_back(move(cycle));
	}
	vi32 ans(n);
	rep(c, 1, n + 1)
	{
		...
		if (c % 2 == 0 && (k > 20 || size(cycles) % (1 << k)))
		{
			cout << "NO\n";
			return;
		}
		vi32* ptr = data(cycles), * end = ptr + size(cycles);
		while (end != ptr)
		{
			int m = min(__builtin_ctz(end - ptr), k);
			int l = 1 << m;
			vi32* nxt = ptr + l;
			rep(p, ptr, nxt)
				odd_root(*p, k - m);
			vi32 merged = merge(ptr, l);
			rep(i, 1, size(merged))
				ans[merged[i - 1]] = merged[i];
			ans[merged.back()] = merged[0];
			ptr = nxt;
		}
		cycles.clear();
	}
