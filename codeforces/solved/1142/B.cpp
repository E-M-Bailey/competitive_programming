...
struct node {
	...
	friend node operator+(const node& l, const node& r) {
	    int sl = l.r - l.l, sr = r.r - r.l;
	    unordered_map<int, int> D;
	    for (int i = r.l; i < r.r; i++)
            D[A[i]] = max(D[A[i]], r.L[i - r.l]);
...
void go()
{
	...
	for (int i = 1; (1 << i) <= m; i++)
	{
		...
		{
			int k = BL[i - 1][j];
			BL[i][j] = k == MAX<int> ? MAX<int> : BL[i - 1][k];
		}
	}

	//for (const vi32& bl : BL)
	//{
	//	for (int b : bl)
	//		cerr << b << ' ';
	//	cerr << endl;
	...
	{
		int r = n - 1;
		for (D[i] = i; D[i] != MAX<int> && r;)
		{
			//cerr << D[i] << ' ' << flush;
			D[i] = BL[__builtin_ctz(r)][D[i]];
			r &= ~(r & -r);
		}
		//cerr << D[i] << endl;
		//cerr << D[i] << ' ' << flush;
