...
void go()
{
	...
	for (i64& a : A)
		cin >> a;
	array<pi64, 2> cur{ pi64(A[0], 0), pi64(A[0], 0) };
	for (int i = 1; i < n - 1; i++)
	{
		//cerr << i << ' ' << cur[0].first << ' ' << cur[0].second << ' ' << cur[1].first << ' ' << cur[1].second << endl;
		i64 a = A[i];
		array<i64, 2> X = a >= 2 * s ? array<i64, 2>{s, a - s} : array<i64, 2>{max(a - s, (i64)0), min(s, a)};
		array<pi64, 2> next;
		for (int j = 0; j < 2; j++)
		{
			i64 x = X[j], y = a - x;
			...
			for (int k = 0; k < 2; k++)
			{
				i64 sum = cur[k].first * x + cur[k].second;
				if (sum < best.second)
					best = { y, sum };
			}
			//cerr << j << endl;
	...
	i64 ans = min(cur[0].first * A[n - 1] + cur[0].second, cur[1].first * A[n - 1] + cur[1].second);
	cout << ans << endl;
}