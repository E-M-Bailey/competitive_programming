...
__int128_t det(__int128_t a, __int128_t b, __int128_t c, __int128_t d)
{
	return a * d - b * c;
}
optional<vec> isect(const seg& l, const seg& r)
{
	__int128_t x1 = l.a.x, x2 = l.b.x, x3 = r.a.x, x4 = r.b.x;
	__int128_t y1 = l.a.y, y2 = l.b.y, y3 = r.a.y, y4 = r.b.y;
	if (x1 == x3 && y1 == y3)
		return vec{ (i64)x1, (i64)y1 };
	if (x1 == x4 && y1 == y4)
		return vec{ (i64)x1, (i64)y1 };
	if (x2 == x3 && y2 == y3)
		return vec{ (i64)x2, (i64)y2 };
	if (x2 == x4 && y2 == y4)
		return vec{ (i64)x2, (i64)y2 };
	if (s(det(x2 - x1, y2 - y1, x3 - x1, y3 - y1)) * s(det(x2 - x1, y2 - y1, x4 - x1, y4 - y1)) == 1)
		return nullopt;
	if (s(det(x4 - x3, y4 - y3, x1 - x3, y1 - y3)) * s(det(x4 - x3, y4 - y3, x2 - x3, y2 - y3)) == 1)
		return nullopt;
	__int128_t n1 = (x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2) * (x3 * y4 - y3 * x4);
	__int128_t n2 = (x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2) * (x3 * y4 - y3 * x4);
	__int128_t d = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
	if (n1 % d || n2 % d) return nullopt;
	return vec{ (i64)(n1 / d), (i64)(n2 / d) };
	//i64 n1 = det(det(x1, y1, x2, y2), det(x1, 1, x2, 1), det(x3, y3, x4, y4), det(x3, 1, x4, 1));
	//i64 n2 = det(det(x1, y1, y))
}

void go()
{
	//seg a{ vec{0,0},vec{1,1} }, b{ vec{0,1},vec{1,0} };
	//seg c{ vec{0,0},vec{2,2} }, d{ vec{0,2},vec{2,0} };
	//auto A = isect(a, b), B = isect(c, d);
	//if (A) cerr << A->x << ' ' << A->y;
	//else cerr << "N/A";
	//cerr << endl;
	//if (B) cerr << B->x << ' ' << B->y;
	...
	for (auto& l : L)
	{
		...
		i64 dx = abs(l.a.x - l.b.x), dy = abs(l.a.y - l.b.y);
		i64 cur = gcd(dx, dy) + 1;
		ans += cur;
		//cerr << dx << ' ' << dy << ' ' << cur << endl;
	...
	unordered_map<i64, map<i64, int>> M;
	rep(i, 0, n) rep(j, i + 1, n)
	{
		auto is = isect(L[i], L[j]);
		if (is) M[is->x][is->y]++;
		//cerr << i << ' ' << j << ' ' << b << endl;
		//if (b) ans--;
	}
	...
	for (const auto& [x, MM] : M) for (auto [y, m] : MM)
		ans -= IT[m];
