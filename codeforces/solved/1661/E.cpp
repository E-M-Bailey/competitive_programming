...
constexpr array<uint16_t, 1 << 9> tp()
{
	array<uint16_t, 1 << 9> ans{};
	for (uint16_t a = 0; a < (1 << 9); a++)
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
				if (a & (1 << (i * 3 + j)))
					ans[a] |= 1 << (j * 3 + i);
...
constexpr array<uint16_t, 1 << 9> TP = tp();

constexpr array<array<uint16_t, 1 << 9>, 1 << 9> ml()
{
	array<array<uint16_t, 1 << 9>, 1 << 9> ans{};
	for (uint16_t a = 0; a < (1 << 9); a++)
		for (uint16_t b = 0; b < (1 << 9); b++)
		{
			uint16_t ta = TP[a];
			for (int i = 0; i < 3; i++)
				for (int j = 0; j < 3; j++)
				{
					if ((ta >> i) & (b >> j) & 0b001'001'001)
						ans[a][b] |= 1 << (i * 3 + j);
				}
		}
	return ans;
...
const array<array<uint16_t, 1 << 9>, 1 << 9> ML = ml();

//constexpr uint16_t T[1 << 18];
...
struct mat
{
	...
	explicit mat(uint8_t x):
		a(S[x])
	{}
	constexpr static uint16_t C[8]
	{
		0b000'000'000,
		0b000'000'001,
		0b000'010'000,
	...
	explicit mat(uint8_t x, uint8_t y):
		a(C[x & y])
	{}

	explicit mat(uint16_t A): a(A) {}
...
struct ent
{
	...
	int in;
	int out;
	explicit ent(uint8_t x): aa(x), ab(aa), bb(aa), l(x), r(x), in(0), out(0)
	{
		if (x == 0b101)
			out = 2;
		else if (x)
	...
	constexpr ent(mat AA, mat AB, mat BB, uint8_t L, uint8_t R, int IN, int OUT):
		aa(AA),
		ab(AB),
		bb(BB),
		l(L),
		...
		out(OUT)
	{}
	constexpr ent(): ent({}, {}, {}, {}, {}, -1, -1) {}
	friend ent operator+(const ent& l, const ent& r)
	{
		if (l.in < 0) return r;
		if (r.in < 0) return l;
		const mat& iab = l.ab, ibc(l.r, r.l), & icd = r.ab;
		const mat& iaa = l.aa, & ibb = l.bb, & icc = r.aa, & idd = r.bb;
		const mat iac = iab * ibc * icc, ica = iac.t();
		const mat ibd = ibb * ibc * icd, idb = ibd.t();
		const mat aa = iaa | iac * ica;
		const mat dd = idd | idb * ibd;
		const mat ad = iab * ibc * icd;
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
			{
				uint16_t bit = 1 << (i * 3 + j);
				if (iaa.a & bit) D.onion(i, j);
				if (ibb.a & bit) D.onion(i + 3, j + 3);
				if (icc.a & bit) D.onion(i + 6, j + 6);
				if (idd.a & bit) D.onion(i + 9, j + 9);
				if (iab.a & bit) D.onion(i, j + 3);
				if (ibc.a & bit) D.onion(i + 3, j + 6);
				if (icd.a & bit) D.onion(i + 6, j + 9);
			}
		uint16_t cmp = 0;
		for (int i = 0; i < 3; i++)
		{
			...
			if ((l.l & bit) && D.P[i] == i)
				cmp |= bit;
			if ((l.r & bit) && D.P[i + 3] == i + 3)
				cmp |= bit << 3;
			if ((r.l & bit) && D.P[i + 6] == i + 6)
				cmp |= bit << 6;
			if ((r.r & bit) && D.P[i + 9] == i + 9)
				cmp |= bit << 9;
		}
		int out = ct + __builtin_popcount(cmp);
		for (int i = 0; i < 3; i++)
		{
			uint16_t bit1 = 1 << D.find(i), bit2 = 1 << D.find(i + 9);
			cmp &= ~bit1 & ~bit2;
		}
		int in = ct + __builtin_popcount(cmp);
...
{
	typedef ent T;
	constexpr static T unit{};
	int n; vector<T> s;
	Tree(const vector<uint8_t>& A): n(rup2((int)A.size())), s(2 * n, unit)
	{
...
void go()
{
	...
	//for (int i = 1; i < 2 * rup2(n); i++)
	//	cerr << i << ' ' << T.s[i].in << ' ' << T.s[i].out << endl;

	int q;
	cin >> q;
	//const ent z((uint8_t)0);
	while (q--)
	{
		int l, r;
		cin >> l >> r;
