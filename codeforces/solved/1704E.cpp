...
#define MINT2_DEF mod_int<i64, MOD2>
MK_TYPES(MINT2_DEF, mint2);
...
int rtsort(int v, vi32& ans, vi32& vis, vbl& ons, const vvi32& G) {
	if (vis[v] >= 0)
		return vis[v];
	ons[v] = true;
	int d = -1;
	for (int w : G[v])
		d = max(rtsort(w, ans, vis, ons, G), d);
	ons[v] = false;
	ans.push_back(v);
	return vis[v] = d + 1;
}

pair<vi32, int> tsort(const vvi32& G) {
	int n = G.size();
	vi32 ans;
	ans.reserve(n);
	vi32 vis(n, -1);
	vbl ons(n);
	for (int v = 0; v < n; v++)
		rtsort(v, ans, vis, ons, G);
	reverse(ALL(ans));
	return make_pair(ans, *max_element(ALL(vis)));
}
...
void go() {
	...
	const auto AB = [&](int v) -> bool {return A[v] || B[v];};

	// simulate directly for d steps
	int t0 = 0;
	vi32 S;
	for (;;) {
		S.clear();
