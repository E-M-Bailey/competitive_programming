...
template<class F>
void inRange(int b, int e, int n, const F& f) {
...
void go() {
	...
	s--;
	int N = 3 * n - 2;
	const auto I = [n](int i) {
		return i < n ? i + n - 1 : i - n;
	};
	const auto O = [n](int i) {
		return i < n ? i + 2 * n - 2 : i - n;
	};
	vector<vector<pii>> G(N);
	for (int i = 1; i < n; i++) {
		G[I(i)].emplace_back(I(i * 2), 0);
		G[I(i)].emplace_back(I(i * 2 + 1), 0);
		G[O(i * 2)].emplace_back(O(i), 0);
		G[O(i * 2 + 1)].emplace_back(O(i), 0);
	}
	while (q--) {
		int t, v, u, l, r, w;
		cin >> t;
		if (t == 1) {
			...
			v--; l--;
			if (t == 2)
				inRange(l, r, n, [&](int i) {G[v].emplace_back(I(i), w);});
			else
				inRange(l, r, n, [&](int i) {G[O(i)].emplace_back(v, w);});
		}
	}
	vi64 D(N, -1);
	const auto cmp = [](pair<int, i64> a, pair<int, i64> b) {return a.second > b.second;};
	priority_queue<pair<int, i64>, vector<pair<int, i64>>, decltype(cmp)> Q(cmp);
	Q.emplace(s, 0);
	while (sz(Q)) {
		auto [v, d] = Q.top();
		Q.pop();
...
void go() {
	...
	int b = 1; // # blocks
	while (b * b < n) b++;
	int a = (n + b - 1) / b; // block size
	vector<vector<pair<int, int>>> G(n + 2 * b);
	for (int i = 0; i < n; i++) {
		G[i].emplace_back(n + i / a, 0);
		G[n + b + i / a].emplace_back(i, 0);
	}
	//vector<
	while (q--) {
		//int t
		...
		if (t == 1) {
			...
			while (i < r) {
				if (i % a || i + a > r) {
					...
					if (t == 2)
						G[v].emplace_back(n + b + i / a, w);
					else
						G[n + i / a].emplace_back(v, w);
					i += a;
				}
			}
		}
