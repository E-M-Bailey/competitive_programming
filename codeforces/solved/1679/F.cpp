...
// then care about relative order of adjacent nodes only

// for each length, frontier set, can add another number and remove its adjacent elements
// to ensure uniqueness, requre it to be the maximum of the new set
...
void go()
{
	...
	int G[10]{ 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff, 0x3ff };
	while (m--)
	{
		int u, v;
		cin >> u >> v;
	...
	rep(i, 0, n)
	{
		rep(mask, 0, 1024)
		{
			...
			{
				int bit = 1 << d;
				int mask2 = (mask & ~G[d]) | bit;
				if (mask2 <= bit * 2 - 1)
					DP[i + 1][mask2] = (DP[i + 1][mask2] + DP[i][mask]) % MOD;
			}
		}
	}
