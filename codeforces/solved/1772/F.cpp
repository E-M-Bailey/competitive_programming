...
struct pict
{
	vvbl d;
	int i;
	friend istream& operator>>(istream& strm, pict& p)
	{
		p.d.resize(n);
		for (vbl& row : p.d)
		{
	...
	friend void diffs(const pict& a, const pict& b, vpi32& ans)
	{
		ans.clear();
		rep(i, 0, n) rep(j, 0, m)
			if (a.d[i][j] != b.d[i][j])
	...
	friend bool operator<(const pict& l, const pict& r)
	{
		static vpi32 d;
		diffs(l, r, d);
		if (d.empty()) return false;
		auto [i, j] = d.front();
		return l.d[i - 1][j] != l.d[i][j];
	}
};
...
void go()
{
	...
	cout << P.front().i + 1 << endl;
	diffs(P.front(), P.back(), d);
	cout << d.size() + k << endl;
	rep(i, 0, k) {
		diffs(P[i], P[i + 1], d);
		for (auto [x, y] : d)
			cout << "1 " << x + 1 << ' ' << y + 1 << endl;
		cout << "2 " << P[i + 1].i + 1 << endl;
	}
}