...
constexpr i64 MOD = 1e9 + 7, I2 = (MOD + 1) / 2;
...
i64 nCr(int n, int r)
{
	static vi64 F{ 1 }, IF{ 1 };
	if (r < 0 || r > n)
		return 0;
	while (F.size() <= n)
	{
		i64 f = F.back() * ssize(F) % MOD;
		F.push_back(f);
		IF.push_back(minv(f));
	}
...
i64 ct(int l, int r, int lb, int rb)
{
	int n = l + r;
	int cl = l - max(0, n - 1 - rb), cr = r - max(0, lb);
	//cerr << ' ' << cl << ' ' << cr << ' ' << lb << ' ' << rb << endl;
	return nCr(cl + cr, cl);
}
...
void go()
{
	...
	// ans += ct(n / 2, (n + 1) / 2, l + k, r - k) for all k > 0
	// l + k <= 0 iff k <= -l, r - k >= n - 1 iff k <= r - n + 1
	int amt = min(-l, r - n + 1);
	ans += amt * ct(n / 2, (n + 1) / 2, 0, n - 1) % MOD;
	if (n % 2)
		(ans *= 2) %= MOD;
	//cerr << ans << endl;
	for (int k = amt + 1;; k++)
	{
		i64 cur = ct(n / 2, (n + 1) / 2, l + k, r - k);
		if (n % 2)
			cur += ct((n + 1) / 2, n / 2, l + k, r - k);
		if (cur == 0)
			break;
		//cerr << cur << ' ' << k << endl;
		(ans += cur) %= MOD;
