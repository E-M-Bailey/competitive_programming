...
constexpr pi32 X(-1, -1);

pi32 min(pi32 a, pi32 b)
{
    return b < a ? b : a;
}
...
void go()
{
	...
	vector<array<pi32, 2>> DP(n);
	DP[0][0] = pi32(A[0] != 0, 1);
	DP[0][1] = pi32(A[0] != 1, 1);
	for (int i = 1; i < n; i++)
	{
		array<pi32, 2> &prev = DP[i - 1], &cur = DP[i];
		array<pi32, 2> c1, c2;
		c1[0].first = prev[0].first + (A[i] != 0);
		c1[0].second = prev[0].second;
		c1[1].first = prev[1].first + (A[i] != 1);
		c1[1].second = prev[1].second;
		c2[0].first = prev[1].first + (A[i] != 0);
        c2[0].second = prev[1].second + 1;
		c2[1].first = prev[0].first + (A[i] != 1);
		c2[1].second = prev[0].second + 1;
		cur = c1;
		if (i % 2 == 0)
        {
            cur[0] = min(cur[0], c2[0]);
            cur[1] = min(cur[1], c2[1]);
        }
	}
	pi32 ans = min(DP.back()[0], DP.back()[1]);
	cout << ans.first;
	if (V == 2)
        cout << ' ' << ans.second;
    cout << endl;
