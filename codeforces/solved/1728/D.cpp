...
char eval(int i, int j, char a, char b) {
    char rec = f(i, j);
	if (rec) return rec;
	if (a < b) return 1;
	if (a == b) return 0;
...
char f(int i, int j) {
	...
	if (dp != 2) return dp;
	if (i == j) return dp = 0;
	char x00 = eval(i + 2, j, S[i], S[i + 1]);
	char x01 = eval(i + 1, j - 1, S[i], S[j - 1]);
	char x10 = eval(i + 1, j - 1, S[j - 1], S[i]);
	char x11 = eval(i, j - 2, S[j - 1], S[j - 2]);
	return dp = max(min(x00, x01), min(x10, x11));
}
