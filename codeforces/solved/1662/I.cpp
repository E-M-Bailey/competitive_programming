...
void go()
{
	...
	rrep(i, 0, n)
	{
		if (200 * i > X.back()) cur += P[i];
		else break;
	}
	ans = max(ans, cur);

	int l = 0, r = 0;
	while (200 * r < X.front()) r++;
	queue<pair<int, i64>> A, R;
	rep(j, 1, m)
	{
		int x = X[j - 1], y = X[j];
		l = r;
		...
		//cerr << "Start " << cur << endl;
		ans = max(ans, cur);
		while (!R.empty() || !A.empty())
		{
			if (A.empty() || (!R.empty() && R.front().first <= A.front().first))
			{
				cur += R.front().second;
				R.pop();
			}
