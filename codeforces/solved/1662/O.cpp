...
constexpr int idx(int r, int a)
{
	return a % 360 + r * 360;
}
constexpr int s = idx(21, 0), t = s + 1, N = t + 1;

template<bool b>
constexpr void bset(array<bitset<N>, N>& A, int u, int v)
{
	A[u][v] = b;
	A[v][u] = b;
}
...
void pre()
{
	rep(r, 0, 21) rep(a, 0, 360)
	{
		int u = idx(r, a), v = idx(r, a + 1);
		G[u].push_back(v);
		G[v].push_back(u);
	}
	rep(r, 0, 20) rep(a, 0, 360)
	{
		int u = idx(r, a), v = idx(r + 1, a);
		G[u].push_back(v);
		G[v].push_back(u);
	}
	rep(a, 0, 360)
	{
		int u = idx(20, a), v = idx(0, a);
		G[u].push_back(s);
		G[s].push_back(u);
		G[t].push_back(v);
		G[v].push_back(t);
