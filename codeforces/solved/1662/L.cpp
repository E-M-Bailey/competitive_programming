...
void go()
{
	...
	ranges::sort(P, {}, [](pi64 a) { return pi64(a.second, a.first); });
	ranges::sort(XICC);
	XICC.resize(unique(ALL(XICC)) - begin(XICC));
	unordered_map<i64, int> XCC;
	int X = size(XICC);
	rep(i, 0, X)
		XCC[XICC[i]] = i;
	...
	{
		bool z = x == 0 && y == 0;
		//cerr << x << ' ' << y << ' ' << (y - x) / 2 << ' ' << (x + y) / (2 * v) << ' ';
		x = XCC[x];
		int val = z ? 0 : t.query(0, x + 1) + 1;
		//cerr << val << endl;
		ans = max(ans, val);
		t.upd(x, val);
	}
