...
int n, q, k, l;
vi32 A, P, S, D, PO, T, PP;
vi64 SP;
vvi32 C, BL;
vector<unordered_map<int, i64>> BP;

void dfs1(int pp, int v)
{
	int t = T[v] = size(PO);
	PO.push_back(v);
	PP[t] = T[pp];
...
void go()
{
	...
	rep(i, 0, n)
		ranges::sort(ALL(C[i]), greater<>{}, [](int v) { return S[v]; });

	SP[0] = (i64)A[0] * A[0];
	rep(i, 1, n)
	{
		SP[i] = SP[P[i]] + (i64)A[i] * A[i];
		D[i] = D[P[i]] + 1;
	}
	...
	while (q--)
	{
		...
		while (x != y)
		{
			...
			int a = i - k + 1, b = j - k + 1, d = D[x];
			if (d % k != k - 1 || PP[a] != PP[i] || PP[b] != PP[j])
			{
				ans += (i64)A[x] * A[y];
				x = P[x];
				y = P[y];
			...
			if (it == BP[a].end())
			{
				i64 bp = 0;
				rep(r, 0, k)
					bp += (i64)A[PO[a + r]] * A[PO[b + r]];
				it = BP[a].emplace(b, bp).first;
			}
			ans += it->second;
			x = P[PO[a]];
			y = P[PO[b]];
