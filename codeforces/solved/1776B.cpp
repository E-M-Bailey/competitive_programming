...
int c(int i, int j)
{
	return min(h, (X[j - 1] - X[i] - 1) / 2);
}

i64 f(int i, int j)
{
	i64& dp = DP[i][j];
	if (dp != MAX<i64>) return dp;
	if (i + 1 == j) return dp = 0;
	//int w = X[i], z = X[j - 1];
	//int best = -1;
	rep(k, i + 1, j)
	{
		//int x = X[k - 1], y = X[k];
		i64 L = f(i, k), R = f(k, j);
		i64 cur = L + R - c(i, k) - c(k, j) + 2 * c(i, j);
		//if (cur < dp)
		//{
		//	dp = cur;
		//	best = k;
...
void go()
{
	...
	X.resize(n);
	cin >> X;
	DP.assign(n, vi64(n + 1, MAX<i64>));
	i64 ans = f(0, n) + h - c(0, n);
	cout << ans;
}