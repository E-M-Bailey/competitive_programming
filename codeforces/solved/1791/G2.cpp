...
void go()
{
	...
	cin >> n >> c;
	vpi32 A(n);
	rep(i, 0, n) A[i] = pair(i, read<int>(cin));
	const auto opt = [n](pi32 a) { return a.second + min(a.first + 1, n - a.first); };
	sort(ALL(A), [n, &opt](pi32 a, pi32 b) { return opt(a) < opt(b); });

	int b = 0;
	while (b * b < n) b++;
	int m = b * b;
	A.resize(m, pair(-1, c + 1));
	vi64 B(b);
	rep(i, 0, m) B[i / b] += opt(A[i]);

	int best = 0;
	rep(i, 0, n)
	{
		int bi = i / b;
		int sum = A[i].second + A[i].first + 1;
		if (sum > c) continue;
		int cur = 1;
		rep(j, 0, b)
		{
