...
void go()
{
	...
	cin >> A;
	vpi64 DP(n);
	DP[0] = pair(A[0], MIN<i64> / 2);
	rep(i, 1, n)
	{
		i64 nn = DP[i - 1].first + A[i],
			ny = DP[i - 1].first - 2 * A[i - 1] - A[i],
			yn = DP[i - 1].second + A[i],
			yy = DP[i - 1].second + 2 * A[i - 1] - A[i];
		DP[i] = pair(max(nn, yn), max(ny, yy));
	}
