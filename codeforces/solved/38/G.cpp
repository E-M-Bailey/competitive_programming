...
struct Treap { // If unsorted remove cmp and bound.
	typedef pii T;
	static constexpr T unit{-1, MIN<int>};
    static uniform_int_distribution<int> distr;
	vi P, S; vector<aii> C; vector<T> V;
	void f(T& m, const T& l, const T& r) { m.second = max({m.first >= 0 ? A[m.first] : MIN<int>, l.second, r.second}); }
	bool cmp(const T& l, const T& r) { return false; }
	int s(int i) { return i < 0 ? 0 : S[i]; }
	const T& v(int i) { return i < 0 ? unit : V[i]; }
	void upd(int i) { if (i >= 0) { f(V[i], v(C[i][0]), v(C[i][1])); S[i] = 1 + s(C[i][0]) + s(C[i][1]); }}
...
void inorder(const Treap& T, int i, vi32& res)
{
    if (i < 0)
        return;
    inorder(T, T.C[i][0], res);
    res.push_back(i);
    inorder(T, T.C[i][1], res);
}
...
void go() {
	...
	for (int i = 0; i < n; i++) {
		int c;
		cin >> A[i] >> c;
		int lo = 0, hi = min(c, T.s(r));
		while (lo < hi) {
			int mid = (lo + hi + 1) / 2;
			//cerr << i << ": query(0," << mid << ") = " << T.query(r, 0, mid).second << endl;
			if (T.query(r, 0, mid).second < A[i])
				lo = mid;
			else
				hi = mid - 1;
		}
		//cerr << "inserting " << i << '(' << A[i] << ") at " << lo << endl;
		r = T.insert(r, lo, T.mkNode({i, A[i]}));
	}
	vi32 ans;
	inorder(T, r, ans);
	reverse(ALL(ans));
