...
void go() {
	...
	while (c0 > c1) {
		...
		pi32 B = C.top();
		C.pop();
		doSwap(A.first, B.first);
		C.emplace(A.first, A.second + B.second);
		c0--;
	}
	while (c0 < c1) {
		pi32 A = C.top();
		...
		if (A.second == 1)
			continue;
		int best = P[A.first], bestD = 1;
		for (int j = P[A.first], d = 1; j != A.first; j = P[j], d++)
			if (j < best) {
				best = j;
				bestD = d;
			}
		doSwap(A.first, best);
		C.emplace(best, bestD);
		C.emplace(A.first, A.second - bestD);
		c0++;
	}
