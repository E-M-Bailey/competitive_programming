...
struct T
{
	...
	i64 num = 0;
	int siz = 0;
	constexpr T(int a, bool o, i64 n, int s): phi(a), all1(o), num(n), siz(s) {}
	constexpr T() : T(0, true, 0, 0) {}
	constexpr explicit T(int a): T(a, a == 1, 0, 1) {}
};
struct Tree
{
	constexpr static T unit = T();
	T f(T L, T R)
	{
		int l = L.phi, r = R.phi;
		//cerr << " calling f " << l << ' ' << r << ' ' << L.siz << ' ' << R.siz << endl;
		if (l == 0)
			return R;
		if (r == 0)
			return L;
		...
		return { l, L.all1 && R.all1, num, L.siz + R.siz };
	}
	bool do_beats(const T& x) const
	{
		return !x.all1;
	}
	void beats1(T& x)
	...
	int n; vector<T> s;
	Tree(const vi32& A): n(rup2(size(A))), s(2 * n, unit)
	{
		rep(i, 0, size(A))
			s[i + n] = T(A[i]);
		rrep(i, 1, n)
	...
	void beats(int i, int l, int r, int nl, int nr)
	{
		l = max(l, nl);
		r = min(r, nr);
		if (l >= r || !do_beats(s[i]))
		...
		beats(i * 2, l, r, nl, nm);
		beats(i * 2 + 1, l, r, nm, nr);
		s[i] = f(s[i * 2], s[i * 2 + 1]);
	}

	void beats(int l, int r)
	{
		beats(1, l, r, 0, n);
	}
