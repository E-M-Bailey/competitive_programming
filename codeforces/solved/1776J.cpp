...
void go()
{
	...
	rep(k, 0, 2 * n) rep(i, 0, 2 * n) rep(j, 0, 2 * n) if (D[i][k] < inf && D[k][j] < inf)
	...
	rep(i, 0, n) rep(j, i, n)
	{
		int d0 = D[i][j], d1 = D[i][j + n];
		rep(b, 0, k + 1)
			ans = max(ans, min(d0 + b, d1 + k - b));
	}
	cout << ans;
}