...
template<uint32_t M> struct mint
{
	...
	constexpr mint(integral auto v): v(v < 0 ? v % M + M : v % M) { if (this->v == M) this->v = 0; }
	...
	[[nodiscard]] constexpr mint pow(pow_t b) { return mpow(*this, b); }
	[[nodiscard]] constexpr mint pow(integral auto b) { return mpow(*this, pow_t(b)); }
...
struct Tree1
{
	int n, m;
	vvi64 s, l;
	Tree1(vi64&& leaves): n(rup2(size(leaves)))
	{
		s.push_back(forward<vi64>(leaves));
		l.emplace_back(n, 1);
		s.back().resize(n, 1);
		while (size(s.back()) > 1)
		{
			vi64 L(size(s.back()) / 2);
			rep(i, 0, size(L))
				L[i] = s.back()[i * 2] * s.back()[i * 2 + 1] % M;
			l.emplace_back(size(L), 1);
			s.push_back(move(L));
		}
		reverse(ALL(s));
		reverse(ALL(l));
	...
	void pd(int i, int j, int nlo, int nhi)
	{
		...
		(s[i][j] *= mpow(l[i][j], nhi - nlo)) %= M;
		l[i][j] = 1;
	}
	i64 query(int i, int j, int lo, int hi, int nlo, int nhi)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (hi <= lo) return 1;
		//cerr << lo << ' ' << nlo << ' ' << hi << ' ' << nhi << endl;
		pd(i, j, nlo, nhi);
		if (lo == nlo && hi == nhi) return s[i][j];
		int mid = midpoint(nlo, nhi);
		return query(i + 1, j * 2, lo, hi, nlo, mid) * query(i + 1, j * 2 + 1, lo, hi, mid, nhi) % M;
	}
	void update(int i, int j, int lo, int hi, int nlo, int nhi, i64 v)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (hi <= lo) return;
		...
		(s[i][j] *= mpow(v, hi - lo)) %= M;
		int mid = midpoint(nlo, nhi);
		update(i + 1, j * 2, lo, hi, nlo, mid, v);
		update(i + 1, j * 2 + 1, lo, hi, mid, nhi, v);
	}
	i64 query(int lo, int hi)
	{
		return query(0, 0, lo, hi, 0, n);
	}
	void update(int lo, int hi, i64 v)
	{
		update(0, 0, lo, hi, 0, n, v);
	}
};
...
struct Tree2
{
	int n, m;
	vvu64 s, l;
	Tree2(vu64&& leaves): n(rup2(size(leaves)))
	{
		s.push_back(forward<vu64>(leaves));
		l.emplace_back(n);
		s.back().resize(n);
		while (size(s.back()) > 1)
		{
			vu64 L(size(s.back()) / 2);
			rep(i, 0, size(L))
				L[i] = s.back()[i * 2] | s.back()[i * 2 + 1];
			l.emplace_back(size(L));
			s.push_back(move(L));
		}
		reverse(ALL(s));
		reverse(ALL(l));
	...
	u64 query(int i, int j, int lo, int hi, int nlo, int nhi)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (hi <= lo) return 0;
		//cerr << lo << ' ' << nlo << ' ' << hi << ' ' << nhi << endl;
		//cerr << lo << ' ' << hi << ' ' << nlo << ' ' << nhi << ' ' << s[i][j] << ' ' << l[i][j] << endl;
		pd(i, j);
		if (lo == nlo && hi == nhi) return s[i][j];
		int mid = midpoint(nlo, nhi);
		return query(i + 1, j * 2, lo, hi, nlo, mid) | query(i + 1, j * 2 + 1, lo, hi, mid, nhi);
	}
	void update(int i, int j, int lo, int hi, int nlo, int nhi, u64 v)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (hi <= lo) return;
		//cerr << lo << ' ' << hi << ' ' << nlo << ' ' << nhi << hex << ' ' << s[i][j] << ' ' << l[i][j] << ' ' << v << dec << endl;
		if (lo == nlo && hi == nhi)
		{
			l[i][j] |= v;
			return;
	...
	u64 query(int lo, int hi)
	{
		return query(0, 0, lo, hi, 0, n);
	}
	void update(int lo, int hi, u64 v)
	{
		update(0, 0, lo, hi, 0, n, v);
	}
};
...
void go()
{
	...
	LP[1] = 0;
	vi64 P;
	rep(i, 2u, 301u) if (LP[i] == i) { P.push_back(i); srep(j, i * 2, 301u, i) LP[j] = min(LP[j], i); }
	vi32 IP(301);
	int k = size(P);
	rep(i, 0, k) IP[P[i]] = i;
	...
	while (q--)
	{
		...
		if (s == "TOTIENT")
		{
			i64 prod = B.query(l, r);
			u64 mask = T.query(l, r);
			//rep(i, 0, n) rep(j, i, n + 1)
			//	cerr << ' ' << i << ' ' << j << ' ' << hex << T.query(i, j) << dec << endl;
			//cerr << "querying " << l << ' ' << r << ' ' << hex << mask << dec << endl;
			while (mask)
			{
		...
		else
		{
			...
			//cerr << "updating " << l << ' ' << r << ' ' << hex << mask << dec << endl;
			T.update(l, r, mask);
		}
	}
}