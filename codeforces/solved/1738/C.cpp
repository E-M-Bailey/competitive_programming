...
vector<vector<array<array<char, 2>, 2>>> DP;
constexpr array<array<char, 2>, 2> DEF{array<char, 2>{-1, -1}, array<char, 2>{-1, -1}};

bool B(int e, int o, bool a);

bool A(int e, int o, bool a) {
    char& dp = DP[e][o][a][0];
    if (dp >= 0)
        return dp;
    else if (o == 0)
...
void go() {
    ...
    DP.assign(e + 1, vector<array<array<char, 2>, 2>>(o + 1, DEF));
    cout << (A(e, o, 0) ? "Bob" : "Alice") << endl;
}
