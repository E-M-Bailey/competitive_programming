...
bool dfs(int v, int d, vbl& V, vi32& S) {
	if (V[v])
		return true;
	V[v] = true;
...
void go() {
	...
	for (int i = 0; i < n; i++)
		for (int j = i + 1; j < n; j++)
			D[i][j] = D[j][i] = abs(X[i][0] - X[j][0]) + abs(X[i][1] - X[j][1]);
	...
	sort(ALL(E), [&](pi32 l, pi32 r) {return D[l.first][l.second] < D[r.first][r.second];});
	...
	for (pi32 e : E) {
		...
		vi32 S;
		vbl V(n, false);
		if (dfs(e.first, D[e.first][e.second], V, S)) {
			for (int s : S)
				VV[s] = true;
			CC.emplace_back(move(S));
		}
	}
