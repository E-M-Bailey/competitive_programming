...
int diam(int u, int v)
{
	...
	{
		if (w == u) continue;
		ans = max(ans, diam(v, w));
		DA[v] = min(DA[v], DA[w] + 1);
		DB[v] = min(DB[v], DB[w] + 1);
		int d = D[w] + 1;
		if (d > D[v])
		{
			d2 = D[v];
...
void go()
{
	...
	int d = diam(-1, 0);
	i64 dab = MAX<int>;
	rep(v, 0, n) dab = min(dab, DA[v] + DB[v]);
	//cerr << d << ' ' << dab << endl;
	if (d >= da * 2 + 1 && db >= 2 * da + 1 && dab > da)
		cout << "Bob\n";
	else
		cout << "Alice\n";
}