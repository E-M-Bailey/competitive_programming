...
struct mint
{
	...
	constexpr friend bool operator>(mint a, mint b) { return a.v > b.v; }
	constexpr friend bool operator>=(mint a, mint b) { return a.v >= b.v; }
	explicit constexpr operator u32() const { return v; }
...
MK_TYPES(mint<MOD>, mnt);

void solve_tridiag(vmnt& A, vmnt& B, vmnt& C, vmnt& D)
{
	int n = size(A);
	C[0] /= B[0];
	D[0] /= B[0];
