...
pair<int, int> query(int i, int j, vector<string> const &G)
{
	cout << "? " << i + 1 << ' ' << j + 1 << '\n';
...
void go()
{
	...
	while (b - t > 1)
	{
		int mid = (t + b) / 2;
		bool odd = mid % 2 != t % 2;
		rep(i, 0, t) fill(ALL(G[i]), '^');
		rep(i, mid, n) fill(ALL(G[i]), 'v');
		rep(i, t, mid)
		{
			if (i % 2 != t % 2)
			{
		...
		if (odd)
			...
			G[mid - 1].front() = '<';
		if (query(t, 0, G) != make_pair(mid - 1, odd ? n : -1))
		{
			b = mid;
			continue;
		}

		reverse(begin(G) + t, begin(G) + mid);
		rep(i, t, mid) rep(j, 0, n) if (G[i][j] == 'v') G[i][j] = '^';
		if (odd) rep(i, t, mid)
		{
			reverse(ALL(G[i]));
			rep(j, 0, n) if (G[i][j] != '^') G[i][j] ^= 2;
		}
		if (query(mid - 1, odd ? n - 1 : 0, G) != make_pair(t, -1))
			b = mid;
		else
			t = mid;
	}
	...
	rep(i, 0, t) fill(ALL(G[i]), '^');
	rep(i, t, n) fill(ALL(G[i]), 'v');
	fill(ALL(G[t]), '>');
	auto [ir, jr] = query(t, 0, G);
	if (ir == -1) ans(t, jr, '^');
	if (ir == n) ans(t, jr, 'v');
