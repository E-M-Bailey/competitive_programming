...
tuple<int, int, int> eval(vi32 &P, int a, int b)
{
	swap(P[a], P[b]);
	static vi32 Q;
	Q.resize(n);
	...
	int ct = 0, lo = n - 1, hi = 0;
	rep(i, 0, n)
	{
		lo = min(lo, Q[i]);
		hi = max(hi, Q[i]);
		// cerr << "  " << a << ' ' << b << ' ' << i << ' ' << lo << ' ' << hi << ' ' << ct << endl;
		ct += hi - lo == i;
	}
	swap(P[a], P[b]);
	return make_tuple(ct, a, b);
}
...
void go()
{
	...
	int i0 = find(ALL(P), 0) - begin(P), i1 = find(ALL(P), 1) - begin(P);
	tuple<int, int, int> ans = min({ eval(P, 0, 0), eval(P, 0, i0), eval(P, 0, i1), eval(P, i0, n - 1), eval(P, i1, n - 1) });
