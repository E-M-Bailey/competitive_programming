...
int S[10 * MAXN - 6];
int *const A = S + (MAXN - 1), *const B = S + (4 * MAXN - 3), *const C = S + (5 * MAXN - 3), *const D = S + (8 * MAXN - 5);
...
void rec(int l, int r)
{
	...
	for (int j = mid; j < r; j++)
		A[j] = B[j - M[j]] = C[j + m[j]] = D[j + m[j] - M[j]] = 0;
	for (int i = l; i < mid; i++)
		D[i]++;
	int i = l, I = l;
	for (int j = r - 1; j >= mid; j--)
	{
		...
		//cerr << '[' << l << ' ' << mid << ' ' << r << "] [" << i << ' ' << I << "] " << A[j] << ' ' << B[j - M[j]] << ' ' << C[j + m[j]] << ' ' << D[j + m[j] - M[j]] << endl;
		//cerr << A[j] + B[j - M[j]] + C[j + m[j]] + D[j + m[j] - M[j]] << endl;
		
		ans += A[j] + B[j - M[j]] + C[j + m[j]] + D[j + m[j] - M[j]];
	}
	rec(l, mid);
	rec(mid, r);
}
