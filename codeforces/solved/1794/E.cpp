...
template<uint32_t M> struct mint
{
	...
	[[nodiscard]] constexpr friend mint operator*(mint a, mint b) { return a *= b; }
	[[nodiscard]] constexpr friend mint mpows(mint a, pow_t b)
	{
		mint p = 1;
		for (; b.v; b.v /= 2, a *= a) if (b.v % 2) p *= a;
		return p;
	}
	[[nodiscard]] constexpr mint mpow(pow_t b) const { return mpows(*this, b); }
	[[nodiscard]] constexpr mint inv() const requires PRIME
	{
		dassert(*this);
		return mpow(INV_POW);
	}
	[[nodiscard]] constexpr friend mint inv(mint a) { static_assert(PRIME); return a.inv(); }
...
TDEFS2(mint<1000000009>, m1);
TDEFS2(mint<1000000021>, m2);
constexpr m1 a1 = 200003;
constexpr m2 a2 = 200017;
array<m1, 200001> P1;
array<m2, 200001> P2;
...
void dfs2(int v)
{
	for (int w : G[v])
	{
		H1[w] += a1 * (H1[v] - a1 * H1[w]);
		H2[w] += a2 * (H2[v] - a2 * H2[w]);
		dfs2(w);
	}
}
...
void go()
{
	...
	unordered_map<u32, usu32> GH;
	for (int i = 0; i < n; i++) {
		//cerr << a1 + P1[i] << endl;
		GH[a1 + P1[i]].insert(a2 + P2[i]);
	}
