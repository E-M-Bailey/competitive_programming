...
array<u32, 1000001> LP{};
array<mnt, 6000> FC;
void pre()
{
	iota(ALL(LP), 0);
	rep(i, 2u, 1000001u) if (LP[i] == i) srep(j, i * 2, 1000001u, i) LP[j] = min(LP[j], i);
	LP[1] = 0;
	FC[0] = 1;
	rep(i, 1, 6000)
		FC[i] = FC[i - 1] * mnt(i);
...
mnt dp(int i, int e)
{
	if (i == 0) return e == 0 ? 1 : 0;
	if (e < 0) return 0;
	if (I[i][e]) return DP[i][e];
	auto [p, m] = FF[i - 1];
	mnt ans = dp(i - 1, e - m) * ncr(e, m);
	if (LP[p] == p)
		ans += dp(i - 1, e - m + 1) * ncr(e, m - 1);
	DP[i][e] = ans;
	I[i][e] = true;
	//cerr << i << ' ' << e << ' ' << ans << endl;
	return ans;
...
void go()
{
	...
	//for (auto [p, m] : FF)
	//	cerr << p << ' ' << m << endl;
	//cerr << pct << ' ' << ncr(pct, n) << endl;
	DP.assign(k + 1, vmnt(n + 1));
	I.assign(k + 1, vbl(n + 1, false));

	//mnt ans = ncr(pct, n) * FC[n];
	cout << dp(k, n) << endl;
}