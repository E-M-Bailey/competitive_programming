...
i64 area(int i, int j, int k)
{
	auto [ax, ay] = X[I[(n + i) % n]];
	auto [bx, by] = X[I[(n + j) % n]];
	auto [cx, cy] = X[I[(n + k) % n]];
	i64 a = bx - ax, b = by - ay, c = cx - bx, d = cy - by;
	return a * d - b * c;
}
...
int choose()
{
	rep(i, 0, n)
	{
		i64 a = area(i - 1, i, i + 1);
		i64 b = min(area(i - 2, i - 1, i + 1), area(i - 1, i + 1, i + 2));
		rep(j, 0, n)
			if (i != j && (i + 1) % n != j && i != (j + 1) % n)
				b = min(b, area(j - 1, j, j + 1));
		if (a <= b) return I[i];
	}
	//assert(false);
}
