...
struct seg
{
	...
	friend bool compat(seg l, seg r)
	{
		if (r < l)
			swap(l, r);
		return r.l > l.r || r.r <= l.r;
	}
};

int lp(const vvpi32& G, int l, int r)
{
	vi32 D(r - l + 1, 0);
	for (int i = l; i <= r; i++)
		for (const pi32& E : G[i])
			if (E.first <= r)
				D[E.first - l] = max(D[E.first - l], D[i - l] + E.second);
    //cerr << "lp(" << l << ',' << r << ")=" << D[r - l] << endl;
	return D[r - l];
}
...
void go()
{
	...
	for (seg s : S)
		G[s.l].emplace_back(s.r + 1, lp(G, s.l, s.r + 1) + 1);



    cout << lp(G, 0, m - 1) << endl;
	//cerr << endl;
}
