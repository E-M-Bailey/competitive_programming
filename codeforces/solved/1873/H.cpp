...
int dfs2(int u, int v, int x)
{
	...
	for (int w : G[v])
		if (u != w && !IC[w])
			ans = min(ans, dfs2(v, w, x) + 1);
	return ans;
}

pair<int, int> cdist(int x)
{
	int bi = -1, bd = INT_MAX;
	rep(i, 0, m)
	{
		int d = dfs2(C[i], C[i], x);
		if (d < bd)
		{
			bi = i;
			bd = d;
...
void go()
{
	...
	if (db >= da + min((ca + m - cb) % m, (cb + m - ca) % m))
		cout << "NO\n";
	else
		cout << "YES\n";
}
