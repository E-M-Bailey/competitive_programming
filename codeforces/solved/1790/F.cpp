...
struct rmq
{
	...
	int row(int d) const
	{
		return 31 - __builtin_clz(max(d - 1, 1));
	}
	...
	{
		data.reserve(m);
		data.push_back(forward<vector<T>>(A));
		int s = 1;
		while (sz(data) < m)
		{
			vector<T>& P = data.back();
			int k = sz(P) - s;
			assert(k > 0);
			vector<T> C(k);
			rep(i, 0, k)
				C[i] = min(P[i], P[i + s]);
			data.push_back(move(C));
			s *= 2;
		}
	}

	T rmin(int l, int r) const
	{
		int d = r - l;
		if (d == 1)
			return data[0][l];
		int k = row(d), s = 1 << k;
		//cerr << l << ' ' << r << ' ' << d << ' ' << k << ' ' << s << ' ' << n << ' ' << m << endl;
		//cerr << ' ' << k << ' ' << data.size() << endl;
		//cerr << ' ' << l << ' ' << r - s << ' ' << data[k].size() << endl;
		auto L = data[k][l];
		auto R = data[k][r - s];
		auto ans = min(L, R);
		return ans;
...
private:
	void lca_dfs(const vvi32& G, vpi32& ET, int u, int v, int d)
	{
		FO[v] = sz(ET);
		ET.emplace_back(d, v);
		for (int w : G[v]) if (w != u)
		{
			lca_dfs(G, ET, v, w, d + 1);
			ET.emplace_back(d, v);
		}
	}

	rmq<pi32> mk_rmq(const vvi32& G)
	{
		vpi32 ET;
		ET.reserve(2 * n + 1);
		lca_dfs(G, ET, -1, 0, 0);
		return rmq<pi32>(move(ET));
	}
...
public:
	...
	int get(int u, int v) const
	{
		...
		if (l > r)
			swap(l, r);
		return RMQ.rmin(l, r + 1).second;
	}

	int depth(int v) const
	{
		return RMQ.data[0][FO[v]].first;
	}
...
void go()
{
	...
	lca LCA(G);
	centroid_tree T(move(G));

	vi32 D(n, MAX<int> / 2);

	int ans = MAX<int>;

	const auto mark = [&LCA, &T, &D](int v)
	{
		for (int u : T.anc(v))
			D[u] = min(D[u], LCA.dist(u, v));
	};
	const auto get = [&LCA, &T, &D, &ans](int v)
	{
		for (int u : T.anc(v))
			ans = min(ans, LCA.dist(u, v) + D[u]);
	};
