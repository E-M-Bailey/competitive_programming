...
const unordered_map<string, int> I{ {"www", 0}, {"iww", 1}, {"nww", 2}, {"iiw", 3}, {"inw", 4}, {"nnw", 5}, {"iii", 6}, {"iin", 7}, {"inn", 8}, {"nnn", 9} };
constexpr array<pi32, 6> C{ pi32{0, 2}, pi32{0, 1}, pi32{6, 7}, pi32{3, 6}, pi32{8, 9}, pi32{5, 9} };
constexpr array<pc8, 6> N{ pc8{'w', 'i'}, pc8{'w', 'n'}, pc8{'i', 'w'}, pc8{'i', 'n'}, pc8{'n', 'w'}, pc8{'n', 'i'} };
constexpr array<pi32, 3> P{ pi32{0, 2}, pi32{1, 4}, pi32{3, 5} };
constexpr array<tuple<int, int, int>, 2> T{ make_tuple(0, 3, 4), make_tuple(1, 5, 2) };
...
void go()
{
	...
	rep(i, 0, n)
	{
		string& a = A[i];
		sort(ALL(a));
		dassert(I.find(a) != I.end());
		FA[I.find(a)->second].push_back(i);
	}
	array<vi32, 6> FE;
	rep(i, 0, 6)
	{
		for (int j : FA[C[i].first])
			FE[i].push_back(j);
		for (int j : FA[C[i].second])
			FE[i].push_back(j);
	}
	vector<tuple<int, char, int, char>> ANS;
	for (auto [a, b] : P)
	{
		while (!FE[a].empty() && !FE[b].empty())
		{
			int i = FE[a].back(), j = FE[b].back();
			FE[a].pop_back();
			FE[b].pop_back();
			ANS.push_back(make_tuple(i, N[a].first, j, N[b].first));
		}
	}
	for (auto [a, b, c] : T)
	{
		while (!FE[a].empty() && !FE[b].empty() && !FE[c].empty())
		{
			int i = FE[a].back(), j = FE[b].back(), k = FE[c].back();
			FE[a].pop_back();
			FE[b].pop_back();
			FE[c].pop_back();
			ANS.push_back(make_tuple(i, N[a].first, j, N[b].first));
			ANS.push_back(make_tuple(j, N[a].first, k, N[c].first));
		}
	}
	cout << size(ANS) << endl;
	for (auto [i, a, j, b] : ANS)
		cout << i + 1 << ' ' << a << ' ' << j + 1 << ' ' << b << endl;

	//rep(i, )

		//array<vi32, 6> FE{ FA[0] + FA[2], FA[0] + FA[1], FA[6] + FA[7], FA[3] + FA[6], FA[8] + FA[9], FA[5] + FA[9] };
