...
vector<array<vpi32, 3>> P;
unordered_map<int, unordered_map<int, int>> EI;
...
void dfs(int u, int v)
{
	...
	for (int w : G[v])
		if (w != u)
		{
			dfs(v, w);
			rep(i, 0, 3)
				(DP[w][i] ? T : F)[i].push_back(w);
		}

	DP[v][1] = F[0].empty();
	P[v][1].clear();
	for (auto w : T[0])
		P[v][1].emplace_back(w, 0);

	DP[v][2] = false;
	if (F[0].empty() && !T[1].empty())
	{
		DP[v][2] = true;
		P[v][2].clear();
		for (int w : T[0])
			P[v][2].emplace_back(w, w == T[1].front() ? 1 : 0);
	}
	else if (F[0].size() == 1 && DP[F[0].front()][1])
	{
		DP[v][2] = true;
		P[v][2].clear();
		for (int w : T[0])
			P[v][2].emplace_back(w, 0);
		P[v][2].emplace_back(F[0].front(), 1);
	}

	DP[v][0] = false;
	if (F[0].empty() && !T[2].empty())
	{
		DP[v][0] = true;
		P[v][0].clear();
		for (int w : T[0])
			P[v][0].emplace_back(w, w == T[2][0] ? 2 : 0);
	}
	else if (F[0].size() == 1 && DP[F[0][0]][2])
	{
		DP[v][0] = true;
		P[v][0].clear();
		for (int w : T[0])
			P[v][0].emplace_back(w, 0);
		P[v][0].emplace_back(F[0][0], 2);
	}
	else if (F[0].empty() && T[1].size() >= 2)
	{
		DP[v][0] = true;
		P[v][0].clear();
		for (int w : T[0])
			P[v][0].emplace_back(w, w == T[1][0] || w == T[1][1] ? 1 : 0);
	}
	else if (F[0].size() == 1 && DP[F[0][0]][1] && T[1].size() >= 2)
	{
		DP[v][0] = true;
		P[v][0].clear();
		for (int w : T[0])
			P[v][0].emplace_back(w, w == T[1][T[1][0] == F[0][0]] ? 1 : 0);
		P[v][0].emplace_back(F[0][0], 1);
	}
	else if (F[0].size() == 2 && DP[F[0][0]][1] && DP[F[0][1]][1])
	{
		DP[v][0] = true;
		P[v][0].clear();
		for (int w : T[0])
			P[v][0].emplace_back(w, 0);
		P[v][0].emplace_back(F[0][0], 1);
		P[v][0].emplace_back(F[0][1], 1);
	}
}

vi32 E;

void dfs2(int u, int v, int c)
{
	cerr << u << ' ' << v << ' ' << c << endl;
	if (u != v && c == 0)
		E.push_back(EI[u][v]);
