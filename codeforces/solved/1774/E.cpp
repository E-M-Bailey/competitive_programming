...
void dfs1(const vvi32& G1, int u, int v)
{
	G[v].p = u;
	G[v].C.reserve(G1[v].size() - (u >= 0));
	G[v].da = G[v].na ? 0 : MIN<int>;
	G[v].db = G[v].nb ? 0 : MIN<int>;
	G[v].c = 0;
	for (int w : G1[v])
		if (w != u)
		{
			G[v].C.push_back(w);
			dfs1(G1, v, w);
			G[v].da = max(G[v].da, G[w].da + 1);
			G[v].db = max(G[v].db, G[w].db + 1);
			bool a = G[w].da >= 0 || G[w].db >= d;
			bool b = G[w].da >= d || G[w].db >= 0;
			if (a || b)
				G[v].c += G[w].c;
			if (a)
				G[v].c += 2;
