...
void doSort(int n, vi32& P, const vi64& H) {
    if (P.empty())
        return;
	int pvt = *max_element(ALL(P), [&](int l, int r) {return H[l] < H[r];});
	vbl R(n);
	for (int p : P)
		if (p != pvt) {
			R[p] = cross(n - 2, pvt, p) == -1;
			if (H[p] == H[pvt])
				R[pvt] = !R[p];
		}
    //cerr << pvt + 1 << endl;
...
void go() {
	...
	vi32 PY, NY;
	for (int i = 0; i < n - 2; i++) {
		H[i] = height(n - 2, n - 1, i);
		(cross(n - 2, n - 1, i) > 0 ? PY : NY).push_back(i);
	}
