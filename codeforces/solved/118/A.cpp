...
void go()
{
	string s;
	cin >> s;
	unordered_set V{ 'a', 'e', 'i', 'o', 'u', 'y' };
	cout << (s
		| views::transform(bind_front(bit_or<char>(), 32))
		| views::filter([&](char c) { return V.find(c) == V.end(); })
		| views::transform([](char c) { return make_pair('.', c); }));
}//bind_front(make_pair<char, char>, '.'))