...
void go()
{
	...
	usi32 P;
	rep(i, 0, n)
		if (!P.insert(A[i]).second) P.erase(A[i]);
	if (size(P) > 1)
	{
		cout << 0 << endl;
		return;
	...
	const auto process = [&]()
	{
		...
		const auto good = [&]()
		{
			if (!M.empty() && (i > M.front() || j <= M.back()) && (i > (n - 1 - M.back()) || j <= (n - 1 - M.front())))
				return false;
			//cerr << ' ' << sum << ' ' << neg << ' ' << odd << endl;
			return sum >= 0 && neg == 0 && odd <= 1;
		};
		F.clear();
		for (; i < n / 2; i++)
		{
