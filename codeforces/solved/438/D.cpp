...
struct Node {
	...
	int m;      // max
	int l = INT_MAX;
	Node(int val, ll p) : p(p), c{}, v(val), s(val), m(val) {}
	Node(int val) : Node(val, rnd()) {}
	//~Node() { rep(i, 0, 2) delete c[i]; } // breaks persistence
	void mod(int x) { if (x <= m) {l = x; v %= l; push(this);} } // custom
	void set(int x) { push(this); s = v = m = x; }
};
...
void fix(Node* n) { // custom
	n->n = 1; n->s = n->v; n->m = n->v; // reset sum
	rep(i, 0, 2) if (n->c[i]) {
		n->n += n->c[i]->n;
		n->m = max(n->m, n->c[i]->m);
...
Node* push(Node* n) { // custom; recurse for ST beats
	cp(n); if (n && n->l != INT_MAX) { // if (has lazy value)
		rep(i, 0, 2) if (cp(n->c[i]), n->c[i]) {
			n->c[i]->mod(n->l);
		} n->l = INT_MAX; fix(n); // reset lazy value
	} return n;
}
...
void go() {
	...
	while (m--) {
		...
		if (t == 1) {
			cin >> l >> r;
			tie(a, c) = split(a, impl(r));
			tie(a, b) = split(a, impl(l - 1));
			cout << b->s << endl;
			a = join(a, b);
			a = join(a, c);
		}
			...
			cin >> k >> x;
			tie(a, c) = split(a, impl(k));
			tie(a, b) = split(a, impl(k - 1));
			b->set(x);
			a = join(a, b);
			a = join(a, c);
		}
