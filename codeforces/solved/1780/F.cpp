...
struct trie
{
	vector<node> N = vector<node>(1);
	ent& get(const vi32& V, int mask)
	{
		int c = 0;
		for (int i = 0; i < size(V); i++)
			if (mask & 1 << i) {
				auto [it, ins] = N[c].C.try_emplace(V[i], size(N));
				if (ins)
					N.emplace_back();
				c = it->second;
			}
...
void go()
{
	array<u32, 300001> LP = to<300001>(views::iota(0u, 300001u));
	rep(i, 2, 300001u) if (LP[i] == i) srep(j, i * 2, 300001u, i) LP[j] = min(LP[j], i);
	...
	for (int i = 0; i < n; i++)
	{
		...
		ranges::sort(PF);
		int k;
		PF.resize(k = unique(ALL(PF)) - begin(PF));
		if (k > 6) exit(-1);
		rep(mask, 0u, 1u << k)
		{
			ent& e = T.get(PF, mask);
