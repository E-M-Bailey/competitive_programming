...
void go() {
	...
	const auto uv = [m](int i, int j) {
		return pi32(i + j, i - j);
	};
	//const auto ij = [m](fld u, fld v) {
	//	return pi32(round((u + v) / 2), round((u - v) / 2));
	//};

	//fld um = INF<fld>, uM = -INF<fld>, vm = INF<fld>, vM = -INF<fld>;
	int um = MAX<int>, uM = MIN<int>, vm = MAX<int>, vM = MIN<int>;

	vvbl G(n, vbl(m, false));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < m; j++)
			if (read<char>() == 'B') {
				G[i][j] = true;
	...
	fld uu = um + uM, vv = vm + vM;
	int im = (uu + vv) / 4, iM = (uu + vv + 3) / 4, jm = (uu - vv) / 4, jM = (uu - vv + 3) / 4;
	vpi32 C{pi32(im, jm), pi32(im, jM), pi32(iM, jm), pi32(iM, jM)};
	pi32 ans = *min_element(ALL(C), [&](pi32 l, pi32 r) {return cmp(l.first, l.second) < cmp(r.first, r.second);});
	//auto [ii, jj] = ij(uu, vv);
