...
void go()
{
	...
	while (m--)
	{
		...
		G[u].emplace_back(v + n * w, 0);
		G[v].emplace_back(u + n * w, 0);
		for (int w2 = 1; w2 <= 50; w2++)
		{
			int d = w + w2;
			d *= d;

			G[u + n * w2].emplace_back(v, d);
			G[v + n * w2].emplace_back(u, d);
		}
	}
	...
	do
	{
		int v = Q.top().second, dv = D[v];
		Q.pop();
		if (V[v])
			continue;
		V[v] = true;
		for (auto [w, wt] : G[v])
			if (int dw = dv + wt; !V[w] && dw < D[w])
			{
				D[w] = dw;
				Q.emplace(-dw, w);
			}
