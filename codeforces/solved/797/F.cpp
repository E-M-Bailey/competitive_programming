...
void go()
{
	...
	cin >> n >> m;
	vi64 X(n);
	vector<pair<i64, int>> PC(m);
	cin >> X >> PC;
	sort(ALL(X));
	sort(ALL(PC));
	vi64 A(n + 1, MAX<i64>), B(n + 1);
	A[0] = 0;
	...
	for (auto [p, c] : PC)
	{
		...
		rep(i, 0, n + 1)
		{
			A[i] = A[i] == MAX<i64> ? MAX<i64> : A[i] - sum;
			sum += abs(p - X[i]);
		}
		sum = 0;
		R.assign(ALL(A));
		...
		{
			int l = max(0, i - c);
			i64 b = l == i + 1 ? MAX<i64> : R.query(l, i + 1);
			B[i] = b == MAX<i64> ? MAX<i64> : sum + b;
			sum += abs(p - X[i]);
		}
		A.swap(B);
	}
	cout << (A.back() == MAX<i64> ? -1 : A.back());
}
