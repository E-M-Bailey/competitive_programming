...
void go()
{
	...
	string s;
	cin >> s;
	if (count(ALL(s), 'A') != a + c + d || count(ALL(s), 'B') != b + c + d)
	{
		cout << "NO" << endl;
		return;
	}
	...
	for (int i = 1; i < n; i++)
	{
		if (s[i] == s[i - 1])
			continue;
		if (R.empty() || R.back().back() < i - 2)
			R.emplace_back();
		R.back().push_back(i - 1);
	}

	int flex = 0, ab = 0, ba = 0;
	vi32 AB, BA;
	for (vi32& RR : R)
	{
		int m = RR.size() + 1;
	...
	//cerr << a << ' ' << b <<' ' << c << ' ' << d << ' ' << R.size() << ' ' << flex << ' ' << ab << ' ' << ba << endl;
	//for (vi32 RR : R)
    //    cerr << "    " << RR.size() << endl;

	if (ab < c)
	{
		int amt = min(flex, c - ab);
		flex -= amt;
		ab += amt;
	}
	if (ba < d)
	{
		int amt = min(flex, d - ba);
		flex -= amt;
		ba += amt;
	}

	while (!BA.empty() && ab < c && ba > d)
	{
		int cur = BA.back();
		BA.pop_back();
		ba -= cur;
