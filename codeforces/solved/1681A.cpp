...
vi32 A, B;
vvpi32 DP;
vstr8 NAMES{"Alice", "Bob"};

int rec(int i, int j, bool bob)
{
	int& dp = bob ? DP[i][j].first : DP[i][j].second;
	if (dp >= 0)
		return dp;

	int aLim = i == 0 ? 0 : A[i - 1], bLim = j == 0 ? 0 : B[j - 1];
	int lim = max(aLim, bLim);

	if (bob)
	{
		bool win = false;
		for (int k = m; !win && k > 0 && B[k - 1] > lim; k--)
			win |= rec(i, k, false) == 1;
		return dp = win ? 1 : 0;
	}
	else
	{
		bool win = false;
		for (int k = n; !win && k > 0 && A[k - 1] > lim; k--)
			win |= rec(k, j, true) == 0;
		return dp = win ? 0 : 1;
	}
}
...
void go()
{
	...
	DP.assign(n + 1, vpi32(m + 1, make_pair(-1, -1)));
    cout << NAMES[rec(0, 0, false)] << endl << NAMES[rec(0, 0, true)] << endl;
}
