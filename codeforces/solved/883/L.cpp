...
struct event
{
	...
	TP tp;
	int a, b, i;
	event(i64 t, TP tp, int a, int b, int i) : t(t), tp(tp), a(a), b(b), i(i) {}
	auto tup() const
	{
		return make_tuple(t, tp, a, b, i);
	}
...
struct taxi
{
	i64 t;
	int i;
	taxi(i64 t, int i): t(t), i(i) {}
	tuple<i64, int> tup() const
	{
		return { t, i };
	}
	friend bool operator>(const taxi& a, const taxi& b)
	{
		return a.tup() > b.tup();
	}
};
...
void go()
{
	...
	for (int i = 0; i < m; i++)
	{
		...
		a--;
		b--;
		Q.emplace(t, RQ, a, b, i);
	}
	qreq R;
	vector<pair<i64, int>> Z(m);

	while (!Q.empty())
	{
		//cerr << "loop\n";
		//cerr << "loop " << size(Q) << ' ' << size(R) << ' ' << endl;
		i64 tt = Q.top().t;
		while (!Q.empty() && Q.top().t == tt)
		{
			auto [t, tp, a, b, i] = Q.top();
			//cerr << NM[tp] << ' ' << t << ' ' << a << ' ' << b << ' ' << i << endl;
			Q.pop();
			if (tp == RQ)
			{
				R.emplace(t, a, b, i);
			...
			{
				//cerr << "--" << t << ' ' << t + abs(a - b) << endl;
				Q.emplace(t + abs(a - b), TB, a, b, i);
			}
			else if (tp == TB)
			{
				X.insert(b);
				//cerr << " pushing " << i << " to T[" << t << ']' << endl;
				T[b].emplace(t, i);
			}
		}
		while (size(X) > 2 && !R.empty())
		{
			auto [t, a, b, i] = R.front();
			R.pop();
			auto it = X.upper_bound(a);
			int l = *prev(it);
			int r = *it;
			int ld = a - l, rd = r - a;
			bool gl = ld < rd || (ld == rd && T[r].top() > T[l].top());
			int x = gl ? l : r;
			if (x < 0 || x >= n)
				exit(-1);
			//cerr << ' ' << x << ' ' << l << ' ' << r << endl;
			...
			if (T[x].empty())
				X.erase(x);
			tt = max(t, tt);
			//cerr << ' ' << tt + abs(x - a) << endl;
			Z[i] = { tt + abs(x - a) - t, j + 1 };
			Q.emplace(tt + abs(x - a), TA, a, b, j);
		}
	}
...
void go()
{
	...
	//i64 tac = MAX<i64>;
	bool pac = false, ac = true;
	while (!Q.empty() || !R.empty())
	{
		if (Q.empty() && R.front().t > tac)
			tac = R.front().t;
		//cerr << tac << endl;
		if (R.front().t <= tac && (Q.empty() || Q.top().t > tac) && size(X) > 2 && !R.empty())
		{
			//cerr << " req ";
			//if (Q.empty())
			//	dassert(tac < MAX<i64>);
			...
			//cerr << ' ' << tt + abs(x - a) << endl;
			//cerr << tac << ' ' << t << ' ' << j << endl;
			Z[i] = { tac + abs(x - a) - t, j + 1 };
			Q.emplace(tac + abs(x - a), TA, a, b, j);
			tac = size(X) == 2 ? MAX<i64> : tac + abs(x - a);
		}
		//cerr << "loop\n";
		//cerr << "loop " << size(Q) << ' ' << size(R) << ' ' << ac << endl;
		while (!Q.empty())
		{
			auto [t, tp, a, b, i] = Q.top();
			//cerr << " queue " << t << ' ' << tp << ' ' << a << ' ' << b << ' ' << i << endl;
			//cerr << NM[tp] << ' ' << t << ' ' << a << ' ' << b << ' ' << i << endl;
			Q.pop();
			if (tp == RQ)
			{
