...
void go()
{
	int n, m;
	cin >> n >> m;
	li32 L = to<li32>(views::iota(0, n));
	unordered_map<int, li32::iterator> M;
	...
	rep(i, 0, m)
	{
		...
		if (mit == end(M))
		{
			if (L.back() < n && T[L.back()] == -1) T[L.back()] = i + 1;
			M.erase(L.back());
			L.pop_back();
		}
		else
		...
		L.push_front(p);
		M[p] = begin(L);
		//cerr << (T | views::transform([](int t) { return make_pair(t, ' '); })) << endl;
	}
	//cerr << endl;
	cout << (T | views::transform([](int t) { return make_pair(t, ' '); })) << endl;
}