...
struct Tree {
	typedef int T;
	static constexpr T unit = MAX<int>;
...
void go() {
	...
	const auto check = [&](int k) {
		auto it = G.upper_bound(k);
		if (it == G.end())
			return it;
		return ST.query(k, *it + 1) == A[k] ? it : G.end();
	...
	for (int i = 0; i < m; i++) {
		...
		A[k] -= d;
		ST.update(k, A[k]);
		if (k > 0 && ST.query(0, k) > A[k])
			G.insert(k);
		si32::iterator it;
		while ((it = check(k)) != G.end())
			G.erase(it);
        cout << G.size() + 1 << ' ';
	}
	cout << endl;
