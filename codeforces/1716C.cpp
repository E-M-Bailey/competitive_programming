...
void go() {
	...
	int d = 1;
	for (int i = n - 1; i >= 0; i--) {
		P[i][0] = max({P[i + 1][0] - 1, A[i][0], A[i][1] - d, 0});
		P[i][1] = max({P[i + 1][1] - 1, A[i][1], A[i][0] - d, 0});
		d += 2;
	}
/*
	for (auto p : P)
	...
	int ans = MAX<int>, p = 0;
	for (int i = 0; i <= n; i++) {
		int cur = max({p, P[i][i % 2] - 2 * i, 0});
		if (cur + 2 * n - 1 == 18)
			cerr << i << endl;
		ans = min(ans, max({p, P[i][i % 2] - 2 * i, 0}));
		if (i < n)
			p = max({p, A[i][i % 2] - i * 2, A[i][1 - i % 2] - (i * 2 + 1)});
	}
	cout << ans + 2 * n - 1 << endl;
}
