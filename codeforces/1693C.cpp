...
ostream& operator<<(ostream& strm, Entry e) {
    return strm << e.d;
}

struct Treap { // If unsorted remove cmp and bound.
	typedef Entry T;
	static constexpr T unit{MAX<int> / 2, MAX<int> / 2};
	static uniform_int_distribution<int> distr;
	vi P, S; vector<aii> C; vector<T> V;
	void f(T& m, const T& l, const T& r, int sl, int sr) { m.val = min({l.val, m.d + sl, r.val + sl + 1}); }
	bool cmp(const T& l, const T& r) { return l.d > r.d; }
	int s(int i) { return i < 0 ? 0 : S[i]; }
	const T& v(int i) { return i < 0 ? unit : V[i]; }
	void upd(int i) { if (i >= 0) { f(V[i], v(C[i][0]), v(C[i][1]), s(C[i][0]), s(C[i][1])); S[i] = 1 + s(C[i][0]) + s(C[i][1]); }}
	int mkNode(const T& val) { P.push_back(distr(randy)); S.push_back(1); C.push_back({-1, -1}); V.push_back(val); return sz(P) - 1; }
	int mkTree(const vector<T>& D) {
		vi I{-1};
		for (const T& d : D) {
			int i = mkNode(d);
			while (I.back() >= 0 && P[I.back()] < P.back()) { C[i][0] = I.back(); I.pop_back(); }
			if (I.back() >= 0) C[I.back()][1] = i;
			upd(i); upd(I.back()); I.push_back(i);
		} return sz(D) ? I[2] : -1;
	}
	aii split(int i, int pos) {
		if (i < 0) return {-1, -1};
		int d = pos > s(C[i][0]); aii c = split(C[i][d], d ? pos - s(C[i][0]) - 1 : pos);
		C[i][d] = c[!d]; c[!d] = i; upd(i); return c;
	}
	int join(aii c) {
		if (min(c[0], c[1]) < 0) return max(c[0], c[1]);
		int d = P[c[0]] >= P[c[1]], i = c[!d];
		c[!d] = C[i][d]; C[i][d] = join(c); upd(i); return i;
	}
	int succ(int i, const T& val, bool strict = 0) {
		int res = s(i), d;
		for (; i >= 0; i = C[i][!d]) if (d = strict ? cmp(val, v(i)) : !cmp(v(i), val)) res = i;
		return res;
	}
	int insert(int i, int pos, int w) { aii c = split(i, pos); return join({join({c[0], w}), c[1]}); }
	int erase(int i, int pos) { aii c = split(i, pos); return join({c[0], split(c[1], 1)[1]}); }
	T query(int i, int s, int e) { aii c = split(i, s), d = split(c[1], e); T val = v(d[0]); join(d); join(c); return val; }
	void dbg(ostream& strm, int i) {
        if (i < 0) return;
        dbg(strm, C[i][0]);
        strm << V[i] << ' ';
        dbg(strm, C[i][1]);
...
uniform_int_distribution<int> Treap::distr(MIN<int>, MAX<int>);
...
void go() {
	...
	while (!byD.empty()) {
		...
		for (int w : H[v]) {
			...
			Entry e{d + 1, d + 1};
			int nd = T.mkNode(e);
			R[w] = T.insert(R[w], T.succ(R[w], e), nd);
			D[w] = T.v(R[w]).val + G[w].size() - T.s(R[w]);
			cerr << "Setting " << w << ' ' << D[w] << ' ' << T.v(R[w]).val << ' ' << G[w].size() << ' ' << T.s(R[w]) << endl;
			byD[D[w]].insert(w);
		}
		//while ((it = byD.begin())->second.empty())
        //    byD.
	}
	...
	/*for (int i = 0; i < T.P.size(); i++)
        cerr << T.C[i][0] << ' ' << T.C[i][1] << ' ' << T.V[i] << endl;
    cerr << endl;*/
