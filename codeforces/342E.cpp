...
void sSize(int u, int v)
{
	S[v] = 1;
	for (int w : G[v])
	{
		if (CL[w] >= 0 || w == u) continue;
		sSize(v, w);
		S[v] += S[w];
	}
}

int centroid(int s, int u, int v)
{
	int maxst = s - S[v];
	for (int w : G[v])
	{
		if (CL[w] >= 0 || w == u)
			continue;
		int c = centroid(s, v, w);
		if (c >= 0)
			return c;
		maxst = max(maxst, S[w]);
	}
	return maxst * 2 <= s ? v : -1;
}

void cDepth(int d, int u, int v)
{
	CD[v].push_back(d);
	for (int w : G[v])
	{
...
void cDecomp(int t, int u, int v)
{
	sSize(-1, v);
	int s = S[v];
	int c = centroid(s, -1, v);
	//cerr << "cDecomp(" << t << ',' << u << ',' << v << "): " << c << endl;
	CP[c] = u;
	cDepth(0, -1, c);
	CL[c] = u < 0 ? 0 : CL[u] + 1;
	for (int w : G[c])
...
void mark(int v)
{
	for (int c = v, i = CD[v].size() - 1; c >= 0; c = CP[c], i--)
		//cerr << i << endl;
		MD[c] = min(MD[c], CD[v][i]);
}
...
int query(int v)
{
	int ans = MAX<int> / 2;
	for (int c = v, i = CD[v].size() - 1; c >= 0; c = CP[c], i--)
		ans = min(ans, CD[v][i] + MD[c]);
	return ans == MAX<int> / 2 ? -1 : ans;
}
