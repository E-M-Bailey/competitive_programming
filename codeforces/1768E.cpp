...
{
	i64 p = 1, b = M - 2;
	for (; b; b /= 2, a = a * a % M) if (b % 2) p = p * a % M;
	return p;
}
...
void go()
{
	int n;
	cin >> n >> M;
	vi64 F(3 * n + 1, 1), I(3 * n + 1, 1);
	rep(i, 1, 3 * n + 1)
		I[i] = inv(F[i] = F[i - 1] * i % M);
	i64 a1 = F[2 * n];
	i64 b1 = F[n];
	i64 a2 = F[2 * n] * F[2 * n] % M * I[n] % M;
	i64 b2 = 0;
	rep(i, 0, n + 1)
		b2 = (b2 + F[n] * F[n] % M * F[n] % M * F[2 * n - i] % M * I[i] % M * I[n - i] % M * I[n - i] % M) % M;
	i64 c0 = 1;
	i64 c1 = (2 * a1 + M - b1) % M;
	i64 c2 = (2 * a2 + M - b2) % M;
	i64 c3 = F[3 * n];
	...
	i64 ans = (c1 + M - c0) + 2 * (c2 + M - c1) + 3 * (c3 + M - c2);
	cout << ans % M;
}
