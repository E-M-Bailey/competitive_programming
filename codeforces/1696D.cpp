...
void go() {
	...
	const auto argmin = [&](int i, int j) {
		return A[i] <= A[j] ? i : j;
	};
	const auto argmax = [&](int i, int j) {
		return A[i] >= A[j] ? i : j;
	};
	...
	P[0] = make_pair(0, 0);
	for (int i = 1; i < n; i++)
		P[i] = make_pair(argmin(P[i - 1].first, i), argmax(P[i - 1].second, i));
	S[n - 1] = make_pair(n - 1, n - 1);
	for (int i = n - 2; i >= 0; i--)
		S[i] = make_pair(argmin(S[i + 1].first, i), argmax(S[i + 1].second, i));

	int mid = P[n - 1].first;
	int ans = 0;
	bool mx = false;
	for (int k = mid; k > 0; mx = !mx, k = mx ? P[k - 1].second : P[k - 1].first)
		ans++;
	mx = false;
	for (int k = mid; k < n - 1; mx = !mx, k = mx ? S[k + 1].second : S[k + 1].first)
		ans++;
