...
struct LPTreap {
	...
	int mk(N n) {
		int r = sz(T);
		T.push_back(n); f(T[r]); /*a*/ return r;
	}
	...
	void dbg(int r, int d = 0) {
		if (r < 0) return;
		dbg(T[r].c[0], d + 1);
		cerr << string(d, ' ') << r << ' ' << char(T[r].v + 'a') << ' ' << n(r) << ' ' << T[r].l << endl;
		dbg(T[r].c[1], d + 1);
	}
	void pt(int r) {
		io<1>(r, [&](int i) {cerr << char(T[i].v + 'a');});
		cerr << endl;
	}
};
...
void go() {
	...
	while (q--) {
		int i, j, k;
		cin >> i >> j >> k;
		tie(r, t) = T.spti<1>(r, j);
		tie(r, s) = T.spti<1>(r, i - 1);
		T.T[s].l = k ? 1 : -1;
		r = T.jn(T.jn(r, s), t);
	}
	str.clear();
	T.io<1>(r, [&](int i) {str.push_back(T.T[i].v + 'a');});
	cout << str;
}