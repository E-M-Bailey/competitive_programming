...
Res askRes(int a, int b) {
    return {a, b, query(a, b)};
}

Res hSolve(int lo1, int hi1, int lo2, int hi2) {
    int n = hi1 - lo1;
    if (n == 1)
        return mkRes(lo1, lo2);
    switch (query())
}
...
Res solve(int lo, int hi) {
    ...
    if (n == 1)
        return lo;
    if (n == 2)
        return mkRes(lo, lo + 1);
    int a1 = (3 * lo + hi) / 4, a2 = (lo + hi) / 2, a3 = (lo + 3 * hi) / 4;
    int a = solve(lo, a1), b = solve(a2, a3);
    sw
    if (n == 4)
        switch (query(lo, lo + 2)) {
            case 1:
                return askRes(lo, lo + 3);
            case 0:
                return askRes(lo + 1, lo + 3);
            case 2:
                return askRes(lo + 1, lo + 2);
        }
    int mid = (lo + hi) / 2;
    int a = solve(lo, mid), b = solve(mid, hi);
    return argquery(a, b);
}
