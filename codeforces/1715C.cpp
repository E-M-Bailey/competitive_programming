...
void go() {
	...
	map<int, int> I;
	I.emplace(0, n);
	const auto merge(map<int, int>::iterator l) {
	    map<int, int>::iterator r = next(l);
	    if (l == I.end())
	        return;
	...
	const auto query = [&](int i, int x) {
	    if (x == A[i])
	        return;
	    auto it = prev(upper_bound(i));
	    auto [lo, hi] = *it;
	    I.erase(it);
	    if (i > lo)
	        I.emplace(lo, i);
