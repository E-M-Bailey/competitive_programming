...
void go() {
	...
	map<int, vi32> TZ;
	for (int i = 0; i < n; i++)
		TZ[__builtin_ctz(A[i])].push_back(i);

	if (dsu.k == 2 && !TZ[0].empty()) {
		ans++;
		A[TZ.rbegin()->second.front()]++;
		pt();
	}

	int maxtz = TZ.rbegin()->first;
	int cx = dsu.find(TZ[maxtz].front());
	const usi32& E = dsu.E[cx];
	bool allSame = true;
	for (int e : E)
        if (__builtin_ctz(A[e]) != maxtz)
            allSame = false;
