...
template<u32 M> requires mint<M>::PRIME struct ntt
{
	TDEFS2(mint<M>, mnt);
	static_assert(mnt::PRIME);

};

// C_t(k) = [k > 0] nCr(2^(t+1) - k, 2^t)

// dp_(t+1)(i) = (2^t)! * sum_[j + k = i](dp_t(j) * C_t(k))
//TDEFS2(mint<998244353>, mnt);

void go()
{
	for (auto p : sieve<23>::PRIMES)
		cerr << p << ' ';
	cerr << endl;
