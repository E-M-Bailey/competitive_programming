...
void go()
{
	...
	int ans = (S[0] + 1) / 2 + (S[1] + 1) / 2;
	for (int i = 0; i < n - 1; i++)
		ans = min(ans, cost1(A[i], A[i + 1]));
	for (int i = 0; i < n - 2; i++)
	...
	/*
	const auto op = [](int a, int b)
	{
		return min(a, b);
	};
	segtree<int, decltype(op)> ST(move(A), op, MAX<int>);

	const auto incr = [&](int i, int amt)
	{
	    if (i >= 0 && i < n)
	           ST.update(i, ST[i] + amt);
	};
	...
	for (int i = 0; i < n; i++)
	   {
	       ...
	       incr(i, -2 * cur);
	       incr(i + 1, -cur);
	       ans = min(ans, cur + max(0, min(ST.query(0, i), ST.query(i + 1, n))));
	       incr(i - 1, cur);
	       incr(i, 2 * cur);
	       incr(i + 1, cur);
	   }
