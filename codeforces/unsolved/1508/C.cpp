...
struct DSU
{
	...
	bool merge(int x, int y)
	{
		...
		for (int a : A[y])
		{
			NF.erase(a);
			A[a].erase(y);
			if (A[a].size() < R.size() - 1);
			NF.insert(a);
		}
		vi32 E;
		for (int a : A[x])
			if (A[y].find(a) == A[y].end())
				E.push_back(a);
		for (int a : E)
		{
			NF.erase(x);
	...
	pi32 getPair()
	{
        ...
        int u = *NF.begin();
        int v = 0;
        while (A[u].find(v) != A[u].end())
            v++;
        return make_pair(u, v);
	}
};

template<bool exclOne>
i64 mstCost(const vvpi32& G)
{
	DSU dsu;
	int u, v;
	while (tie(u, v) = dsu.getPair(), u >= 0 && (!exclOne || dsu.R.size() > 2))
		dsu.merge(u, v);
    priority_queue<pair<int, pi32>> E;
    int x = 0;
    for (int i = 0; i < n; i++)
    {
        for (auto [j, w] : G[i])
        {
            x ^= w;
            if (j > i)
                E.emplace(w, make_pair(i, j));
        }
    }
    if (exclOne)
        E.emplace(*dsu.R.begin(), *dsu.R.rbegin(), x);
