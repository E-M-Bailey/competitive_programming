...
int k, H;
int64_t a;
vector<gp_hash_table<int, ent>> DP;
...
void go()
{
	...
	for (int i = 1; i < k; i++)
	{
		...
		int64_t offset = 0;
		for (int amt = (m + 1) / 2, rem = m, j = 0; rem > 0; rem -= amt, amt = (amt + 1) / 2, j++)
			offset += amt * P[j];
		offset = offset * m % MOD;

		int64_t dw = MOD + P[i] - P[i - 1];

		for (auto [hl, el] : DP[i - 1])
		{
			for (auto [hr, er] : DP[i - 1])
			{
				int l = el.w, r = er.w + m;
				int64_t ha = (hl + hr + offset + dw * (l + 1)) % MOD;
				int64_t hb = (hl + hr + offset + dw * (r + 1)) % MOD;
				DP[i][(int)ha] = ent{ l, hl, hr };
				DP[i][(int)hb] = ent{ r, hl, hr };
			}
		}
	}

	{
		...
		for (int amt = (m + 1) / 2, rem = m, j = 0; rem > 0; rem -= amt, amt = (amt + 1) / 2, j++)
			offset += amt * P[j];
		offset = offset * m % MOD;
		int64_t dw = MOD + P[k] - P[k - 1];

		for (auto [h, e] : DP[k - 1])
		{
			int l = e.w, r = e.w + m;
			int64_t ha = (h + offset + dw * (l + 1)) % MOD;
			int64_t hb = (h + offset + dw * (r + 1)) % MOD;
			int64_t ra = (MOD + H - ha) % MOD;
			int64_t rb = (MOD + H - hb) % MOD;
			if (DP[k - 1].find((int)ra) != end(DP[k - 1]))
				DP[k][H] = ent{ l, h, (int)ra };
			else if (DP[k - 1].find((int)rb) != end(DP[k - 1]))
				DP[k][H] = ent{ r, (int)rb, h };
			if (!DP[k].empty())
				break;
		}
	}
	...
	const auto cmp_ans = [&](auto const &self, int st, int i, int hash) -> void
	{
		auto [w, hl, hr] = DP[i][hash];
		if (i > 0)
		{
			int m = 1 << (i - 1);
			self(self, st, i - 1, hl);
			self(self, st + m, i - 1, hr);
		}
		ans[st + w] = L[i];
	};
	cmp_ans(cmp_ans, 0, k, H);
