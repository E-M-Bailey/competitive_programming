...
*/
	...
	for (int i = 1; i < m; i++) {
		...
		while (lo > j || hi < j) {
			...
			if (idx != j) {
				ans = ans * ++ct % MOD;
                //cerr << idx << ' ' << P[idx] << ' ' << ct << endl;
			}
            if (P[idx] > mx)
                tc++;
            //cerr << P[idx] << ' ' << ct << ' ' << tc << endl;
		}
		/*
		while (lo > j) {
			S.insert(P[--lo]);
	...
	/*
	const auto nCr = [&F](int n, int r) -> mod {
		if (r < 0 || r > n)
			return 0;
	...
	for (int i = 1; i < m; i++) {
		mod cur = 0;
		for (int j = 0; j <= min(tc, D[i]); j++)
			cur += nCr(tc, j) * nCr(D[i], j);
		ans *= cur;
		tc += C[i];
	}*/
	cout << ans << endl;
