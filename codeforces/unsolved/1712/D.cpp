...
/*
int cmp(const vi32& A, int i, int k) {
    if (k < 0)
        return 1e9;
    const auto cmp = [&A](int i, int j) {return A[i] > A[j];};
    PQi32<decltype(cmp)> Q(cmp);
    for (int j = 0; j < n; j++)
        if (j != i && j != i + 1)
            Q.push(j);
    vi32 B = A;
    ...
    int ans = 1e9;
    for (int j = 0; j < n; j++)
        ans = min(ans, j == i || j == i + 1 ? B[i] : 2 * B[i]);
    return ans;
}*/

vi32 kp(int n, int k, const vi32& A) {
    vi32 P(n + 1);
    if (k < 0)
        return P;
    map<int, int> S;
    ...
    for (int i = 0; i < n; i++) {
        ...
        P[i + 1] = ct == k + 1 ? prev(S.end())->first : 1e9;
    }
    return P;
}

pair<vi32, vi32> kps(int n, int k, const vi32& A) {
    vi32 B(RALL(A));
    vi32 rans = kp(n, k, B);
    return {kp(n, k, A), vi32(RALL(rans))};
}
...
void go() {
    ...
    for (int& a : A)
        cin >> a;
    auto [P0, S0] = kps(n, k, A);
    auto [P1, S1] = kps(n, k - 1, A);
    auto [P2, S2] = kps(n, k - 2, A);
