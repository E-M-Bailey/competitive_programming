...
struct Entry {
	int lmss, rmss, mss, sum, lmssi, rmssi, mssi, mssj;
};

Entry mss(const vi32& A, int lo, int hi, bool dbg) {
	dbg = false;
	if (lo == hi) {
		if (A[lo] >= 0)
			return Entry{A[lo], A[lo], A[lo], A[lo], lo, lo, lo, lo};
		else
			return Entry{0, 0, 0, A[lo], lo - 1, lo + 1, lo, lo - 1};
	}

	int mid = (lo + hi) / 2;
	Entry l = mss(A, lo, mid, dbg), r = mss(A, mid + 1, hi, dbg);
	Entry ans;

	tie(ans.lmss, ans.lmssi) = max(make_pair(l.lmss, l.lmssi), make_pair(l.sum + r.lmss, r.lmssi));
	tie(ans.rmss, ans.rmssi) = max(make_pair(r.rmss, r.rmssi), make_pair(r.sum + l.rmss, l.rmssi));
	ans.sum = l.sum + r.sum;
	tie(ans.mss, ans.mssi, ans.mssj) = max({make_tuple(l.mss, l.mssi, l.mssj), make_tuple(r.mss, r.mssi, r.mssj), make_tuple(l.rmss + r.lmss, l.rmssi, r.lmssi)});

	if (dbg)
		cerr << "dbg " << lo << ' ' << hi << ' ' << ans.lmss << ' ' << ans.lmssi << ' ' << ans.rmss << ' ' << ans.rmssi << ' ' << ans.mss << ' ' << ans.mssi << ' '  << ans.mssj << ' ' << ans.sum << endl;

	return ans;

	//return Entry{, , max({l.mss, r.mss, l.rmss + r.lmss}, l.sum + r.sum};
}
...
void go() {
	...
	int best = 0, besta = 0, bestl = 0, bestr = 0;
	for (const auto& [x, II] : I) {
		vi32 A;
		A.push_back(1);
		for (int i = 1; i < II.size(); i++) {
			A.push_back(II[i - 1] + 1 - II[i]);
			A.push_back(1);
		}
		if (x == 4 && false) {
			for (int a : A)
		...
		Entry e = mss(A, 0, A.size() - 1, x == 4);
		int cur = e.mss, l = II[e.mssi / 2], r = II[e.mssj / 2];
		//cerr << cur << ' ' << x << ' ' << l << ' ' << r << endl;
		if (cur > best)
			tie(best, besta, bestl, bestr) = make_tuple(cur, x, l, r);
	}
	//cerr << endl;

	cout << besta << ' ' << bestl + 1 << ' ' << bestr + 1 << endl;
