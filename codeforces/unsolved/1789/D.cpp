...
vi32 cmp(int n, vbl A, const vbl& B) {
	
	vi32 ans;
	const auto mv = [&](int a, int b)
	{
		dassert(a != b);
		int d = a - b;
		ans.push_back(d);
	...
	{
		int x = 0;
		while (x < n && B[x] == 0 && A[x] == 0) x++;
		int y = x;
		while (y < n && B[y] == 0 && A[y] == 1) y++;
		int d = x;
		while (d < n && B[d] == A[d]) d++;
		if (d == n) break;
		//cerr << A << ' ' << B << ' ' << x << ' ' << y << ' ' << d << endl;
...
void go()
{
	...
	int a = accumulate(ALL(A), 0), b = accumulate(ALL(B), 0);
	if (a == 0 || b == 0)
	{
		cout << -1 << endl;
		return;
	...
	vi32 a2 = cmp(n, A, B);
	for (int& a : a2) a *= -1;
	const vi32& ans = (size(a2) < size(a1) ? a2 : a1);
	cout << size(ans) << endl;
