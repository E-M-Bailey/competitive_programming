...
struct Node {
	...
	Node(int lo, int hi, T d) :
		lo(lo),
		hi(hi),
		d(d)
	{}
	Node* mkCh(bool d, T def) {
		Node*& c = ch[d];
		if (!c) {
			int mid = (lo + hi) / 2;
			c = new Node(d ? mid : lo, d ? hi : mid, def);
		}
		return c;
	}
	void setCh(bool d, T val) {
		Node*& c = ch[d];
		if (!c) {
			int mid = (lo + hi) / 2;
			c = new Node(d ? mid : lo, d ? hi : mid, val);
		} else
			c->d = val;
...
struct Tree {
	typedef i64 U;
	constexpr T T_UNIT{0, 0};
	constexpr U U_UNIT = 0;
	T tt(T l, T r) {return {l.s + r.s, max(l.m, l.s + r.m)};}
	T ut(U l, T r, int len) {return {r.l * len + r.s, l + r.m};}
	U uu(U l, U r) {return l + r;}
	array<T, 2> split(T l, int llen, int rlen) {

		return {l, l};
	}
	array<U, 2> split(U l, int ls) {return {l, l};}

	struct E {T t; U u;};
	constexpr E UNIT{T_UNIT, U_UNIT};
	typedef Node<E> N;
	N* root;
	int n;
	Tree(int n) :
		n(n),
		root(new Node(0, n, UNIT) {}
	~Tree() {
		delete root;
	}
	void push(N* v) {
		v->d.t = ut(v->d.u, v->d.t);
		if (v->len() > 1 && v->ch[0]) {
			c->ch[0]->d
		}
		if (i < m - 1) {
			u[i + 1][j * 2] = uu(u[i][j], u[i + 1][j * 2]);
			u[i + 1][j * 2 + 1] = uu(u[i][j], u[i + 1][j * 2 + 1]);
		}
		u[i][j] = U_UNIT;
	}
	void mkCh(N* v) {
		if (v->ch[0])
			return;
		push(v);
		auto [l, r] = split(v->d.t);
		v->mkCh(0, l);
		v->mkCh(1, r);
	}
	void upd(int i, int j) {
		if (i < m - 1)
			t[i][j] = tt(eval(i + 1, j * 2), eval(i + 1, j * 2 + 1));
	}
	void update(U d, int lo, int hi, int nlo, int nhi, int i, int j) {
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (lo >= hi)
			return;
		...
		push(i, j);
		int mid = (nlo + nhi) / 2;
		update(d, lo, hi, nlo, mid, i + 1, j * 2);
		update(d, lo, hi, mid, nhi, i + 1, j * 2 + 1);
	}
	void update(U d, int lo, int hi) {
		update(d, lo, hi, 0, n, 0, 0);
	}
	T query(int lo, int hi, int nlo, int nhi, int i, int j) {
		...
		if (lo == nlo && hi == nhi)
			return t[i][j];
		int mid = (nlo + nhi) / 2;
		T a = query(d, lo, hi, nlo, mid, i + 1, j * 2);
		T b = query(d, lo, hi, mid, nhi, i + 1, j * 2 + 1);
		return tt(a, b);
	}
	T query(int lo, int hi) {
		return query(lo, hi, 0, n, 0, 0);
	}
};
