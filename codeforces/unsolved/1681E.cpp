...
struct cell
{
	...
	friend i64 direct(cell a, cell b)
	{
		return abs(a.lu() - b.lu());
	}

	friend istream& operator>>(istream& istrm, cell& c)
	{
		istrm >> c.x >> c.y;
		c.x--;
		c.y--;
...
typedef array<cell, 2> pcell;
typedef vector<layer> vpcell;

struct layer
{
	i64 ac, ad, bc, bd;

	layer(i64 ac, ad, bc, bd) :
	...
	friend layer operator+(const layer& A, const layer& B)
	{
		layer C;
		C.ac = min(A.ac + B.ac, A.ad + B.bc);
		C.ad = min(A.ac + B.ad, A.ad + B.bd);
		C.bc = min(A.bc + B.ac, A.bd + B.bc);
		C.bd = min(A.bc + B.ad, A.bd + B.bd);
		return C;
	}
};
...
void go()
{
	...
	for (int i = 0; i < n - 1; i++)
	{
		cell a = D[i][0], b = D[i][1], c = D[i + 1][0], d = D[i + 1][1];
		i64 ab = direct(a, b);
		L[i].ac = 1 + direct(a.u(), c);
		L[i].ad = 1 + direct(a.u(), d);
		L[i].bc = 1 + direct(b.r(), c);
		L[i].bd = 1 + direct(b.r(), d);
		if (!d.diag())
			L[i].ad = min(L[i].ad, ab + L[i].bd);
		if (!c.diag())
			L[i].bc = min(L[i].bc, ab + L[i].ac);
	}

	LD.resize(n);
	LD[0] = direct(D[0][0], D[0][1]);
	for (int i = 1; i < n; i++)
    {
        const layer& l = L[i - 1];
        i64 ab = direct(D[i - 1][0], D[i - 1][1]);
        i64 cd = direct(D[i][0], D[i][1]);
        i64 cad = l.ac + direct(D[i - 1])
        LD[i] = min({, })
    }

	segtree<layer> ST(ALL(L));
}
