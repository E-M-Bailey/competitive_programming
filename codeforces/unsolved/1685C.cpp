...
{
	randy.seed(duration_cast<nanoseconds>(high_resolution_clock::now().time_since_epoch()).count());
	if (argc > 1)
	{
...
struct entry
{
	...
	entry(int val, int size, int sum) :
		val(val),
		size(size),
		sum(sum)
	{}
...
struct node
{
	...
	friend int size(node* n)
	{
		return n ? n->data.size : 0;
	}

	friend pair<node*, node*> split(node* n, int idx)
	{
		if (!n)
		...
		if (idx <= ls)
		{
            auto [l, r] = split(n->ch[0], idx);
            n->ch[0] = r;
            return make_pair(l, n);
		}
		else
        {
			auto [l, r] = split(n->ch[1], idx - ls - 1);
			n->ch[1] = l;
	...
	friend node* join(node* l, node* r)
	{
        ...
        if (l->p < r->p)
        {
            node* n = join(l, r->ch[0]);
            r->ch[0] = n;
            return r;
        }
        else
        {
            node* n = join(l->ch[1], r);
            l->ch[1] = n;
            return l;
        }
	}
	...
	int sum(int l, int r)
	{
		...
		l = max(0, l);
		r = min(data.size - 1, r);
		if (l == 0 && r == data.size - 1)
			return data.sum;
		int ls = ch[0] ? ch[0]->data.size : 0;
		int ans = 0;
		if (l < ls)
			ans += ch[0]->sum(l, r);
		if (l <= ls && r > ls)
			ans += data.val;
		if (r > ls + 1)
			ans += ch[1]->sum(l - ls - 1, r - ls - 1);
		return ans;
	}

	friend node* reverse(node* n, int l, int r)
	{
	    auto [L, C] = n->split(l);
	    auto [C, R] = n->split(r);
	    c->rev ^= 1;
	    return join(join(L, C), R);
	}
};

node* make_treap(const vector<entry>& data)
{
...
void go()
{
	...
	for (int i = 0; i < n; i++)
    {
        data[i] = entry(s[i] == '(' ? 1 : -1);
    }

    node* T = make_treap(data);
