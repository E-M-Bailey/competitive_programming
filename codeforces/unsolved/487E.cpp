...
{
	size_t deg = 0;
	rng::subrange<edge*> adj, inUp, inDown, outUp, outDown;
};
...
struct graph
{
	...
	graph(int n, int m):
		n(n),
		m(m),
		V(rng::subrange_n(new vert[n], n)),
		E(rng::subrange_n(new edge[m], m))
	{
		for (edge& e : E)
		{
			int a, b;
		...
		for (vert& v : V)
			v.adj = rng::subrange_n(new edge[v.deg], v.deg);
		for (edge& : E);
	}
...
void go()
{
	...
	for (vert* v = V; v < V + n; v++)
		v->edges = new edge * [v->deg];
