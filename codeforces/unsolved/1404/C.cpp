...
constexpr struct val
{
	int m, i;
	bool e;
} NIL = { MAX<int> / 2, -1, false };
const val& operator+(const val& l, const val& r)
{
	return l.e && l.m < r.m ? l : r.e ? r : NIL;
}
TDEFS1(val);
...
struct Tree
{
	...
	vvval s;
	vvi32 l;
	Tree(const vi32& A): n(rup2(size(A)))
	{
		s.emplace_back(n, NIL);
		rep(i, 0, size(A))
		{
			s.back()[i].m = A[i];
			s.back()[i].i = i;
	...
	val query(int lo, int hi, int nlo, int nhi, int i, int j)
	{
		lo = max(lo, nlo);
		hi = min(hi, nhi);
		if (hi <= lo)
			return NIL;
		dassert(0 <= i && i < size(s));
		dassert(0 <= j && j < size(s[i]));
		pd(i, j);
		if (lo == nlo && hi == nhi) return s[i][j];
		int mid = midpoint(nlo, nhi);
		return query(lo, hi, nlo, mid, i + 1, j * 2) + query(lo, hi, mid, nhi, i + 1, j * 2 + 1);
	}
	val query(int lo, int hi) { return query(lo, hi, 0, n, 0, 0); }
	void update(int lo, int hi, int nlo, int nhi, int i, int j, int d)
	{
		...
		pd(i, j);
		int mid = midpoint(nlo, nhi);
		update(lo, hi, nlo, mid, i + 1, j * 2, d);
		update(lo, hi, mid, nhi, i + 1, j * 2 + 1, d);
		fix(i, j);
	}
	void update(int lo, int hi, int d) { update(lo, hi, 0, n, 0, 0, d); }
	void sete(int i, bool e)
	{
		s.back()[i].e = e;
		for (int j = i / 2, k = m - 2; k >= 0; k--, j /= 2) fix(k, j);
	}
	void dbg()
	{
		rep(i, 0, m)
		{
			rep(j, 0, size(s[i]))
			{
				cerr << s[i][j].m << '_' << s[i][j].i << '_' << s[i][j].e << ' ';
			}
			cerr << endl;
		}
	}
