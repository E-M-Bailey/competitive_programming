...
{
	typedef int T;
	static constexpr T unit = MAX<int> / 2;
...
void go()
{
	...
	/*
		...
		const auto pidx = [=](int i) { return i; };
		const auto bidx = [=](int i) { return n + i; };
		const auto aidx = [=](int i) { return n + b + i; };
		const int s = aidx(a), t = pidx(b), N = aidx(n);

		vector<vector<pair<int, bool>>> G(N);
		rep(i, 0, n)
		{
			G[aidx(i)].emplace_back(pidx(i), 1);
			G[pidx(i)].emplace_back(bidx(i / b), 0);
			int l = max(i - P[i], 0), r = min(i + P[i] + 1, n);
			while (l % b && l < r)
				G[pidx(l++)].emplace_back(aidx(i), 0);
			while (r % b && l < r)
				G[pidx(--r)].emplace_back(aidx(i), 0);
		}
		*/
}