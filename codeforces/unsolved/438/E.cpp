...
TDEFS2(mint<998244353>, mnt);
TDEFS2(mnt::pow_t, pnt);
mnt operator ""_m(unsigned long long x) { return mnt(x); }
pnt operator ""_p(unsigned long long x) { return pnt(x); }
constexpr u32 LIM = isqrtf(pnt::MOD) + 1;

struct pp
{
	u32 p, c, pw;
	pp(u32 p, u32 c, u32 pw): p(p), c(c), pw(pw) {}
};
TDEFS1(pp);
...
const struct sieve
{
	vu32 lp;
	vu32 pr;
	sieve(): lp(to<vu32>(views::iota(0u, LIM)))
	{
		lp[1] = 0;
		rep(i, 2u, LIM)
		{
	...
	void sfac(u32 x, vpp& pf) const
	{
		...
		for (u32 p : pr)
		{
			...
			if (c)
				pf.emplace_back(p, c, pw);
		}
		if (x > 1)
			pf.emplace_back(x, 1, x);
	}
	void ffac(u32 x, vpp& pf) const
	{
		dassert(x < LIM);
		pf.clear();
		while (x > 1)
...
const struct fac
{
	...
	{
		dassert(y <= pnt::MOD);
		return *div.lower_bound(y);
	}
	mnt proot()
	{
		rep(r, 2_m, LIM)
			if (ranges::all_of(S.pr, [r](pnt p) { return r.pow(pnt::MOD / p) != 1; }))
				return r;
		dassert(false);
		return 0;
	}
...
const struct ntt
{
	ntt()
	{
		rep(r, 2_m, ISQRTC<(u64)pnt::MOD> +1) {}
	}
} N;
