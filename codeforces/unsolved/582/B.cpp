...
{
	int a = size(A), b = size(B), c = B[0].size();
	vvi64 C(a, vi64(c, MIN<int>));
	rep(i, 0, a) rep(j, 0, c) rep(k, 0, b) C[i][j] = max(C[i][j], A[i][k] + B[k][j]);
	return C;
}

vvi64 pw(const vvi64& A, int p)
{
	int n = size(A);
	if (p <= 1)
	{
		if (p == 1) return A;
		vvi64 I(n, vi64(n, MIN<int>));
		rep(i, 0, n) I[i][i] = 0;
		return I;
	}
	vvi64 ans = pw(A, p / 2);
	ans = mul(ans, ans);
	return p % 2 ? ans : mul(ans, A);
}
...
void go()
{
	...
	rep(i, 0, n)
	{
		rep(j, 0, i) if (A[j] <= A[i]) X[i][0] = max(X[i][0], X[j][0] + 1);
	}
	
	/*rep(p, 0, 50)
	{
		auto PW = pw(M, p);
		rep(i, 0, n) { rep(j, 0, n) cerr << PW[i][j] << ' '; cerr << endl; }
		auto Z = mul(pw(M, p), X);
		rep(i, 0, n)
		{
			cerr << Z[i][0];
	...
	i64 a = 1;
	rep(i, 0, n) a = max(a, ans[i][0]);
	cout << a << endl;
	//rep(i, 0, n)
	//{
	//	cerr << X[i][0] << ' ' << ans[i][0] << endl;
	//}
	/*
		vi32 P(n), S(n);
		rep(i, 0, n) rep(j, 0, )
