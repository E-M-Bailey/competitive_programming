...
void go() {
    ...
    for (int i = 0; i < k; i++)
        for (int j = i + 1; j < k; j++)
            P.emplace_back(i, j);
    const auto f = [k](int i, int j) {return make_pair(, (i + j) % k);}
    sort(ALL(P), [&](pi32 a, pi32 b) {return f(a.first, a.second) > f(b.first, b.second);});
    ...
    */
	...
	for (int i = 0; i < k; i++) {
		...
		vi32 P(k);
		iota(ALL(P), 0);
		sort(ALL(P), [i, k](int a, int b) {return (i + a) % k > (i + b) % k;});
		while (F[i])
        {
            cerr << i << ' ' << P.back() << endl;
            while (F[P.back()] == 0)
                P.pop_back();
            int j = P.back();
            while (F[i] && F[j] && (i != j || F[i] > 1))
            {
                cerr << i << ' ' << F[i] << ' ' << j << ' ' << F[j] << endl;
                F[i]--;
                F[j]--;
                ans += (i + j) % k;
            }
	...
	*/
	...
	cout << (accumulate(ALL(A), 0ll) - ans) / k << endl;
}
