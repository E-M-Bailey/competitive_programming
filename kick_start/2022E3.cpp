...
template<class T>
bezout<T> eEuclid(const T& l, const T& r) {
    T a = l < 0 ? -l : l, b = r < 0 ? -r : r;
	vector<pair<T, T>> S;
	while (b != 0) {
		S.emplace_back(a, b);
		tie(a, b) = make_pair(b, a % b);
...
void go() {
	...
	cin >> n >> s;
	s += s;
	state<1000000007> s1(s);
	state<998244353> s2(s);
	for (int m = 0;;) {
		s1.adv(); s2.adv();
		m++;
		if (m > n && s1.done() && s2.done()) {
			cout << s.substr(n, m - n) << endl;
			return;
		}
	}
}
