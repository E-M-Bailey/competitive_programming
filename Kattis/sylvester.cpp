...
i64 n0, x_0, y_0, w0, h0;

void pt(i64 n, i64 x1, i64 y1, i64 x2, i64 y2, i64 nx, i64 ny, int sgn) {
    x1 = max(x1, nx);
    y1 = max(y1, ny);
    x2 = min(x2, nx + n - 1);
    y2 = min(y2, ny + n - 1);
    if (x1 > x2 || y1 > y2)
        return;
    if (n == 1) {
        ANS[ny - y_0].push_back(sgn);
        return;
    }
    pt(n / 2, x1, y1, x2, y2, nx, ny, sgn);
    pt(n / 2, x1, y1, x2, y2, nx + n / 2, ny, sgn);
    pt(n / 2, x1, y1, x2, y2, nx, ny + n / 2, sgn);
    pt(n / 2, x1, y1, x2, y2, nx + n / 2, ny + n / 2, -sgn);
}

void go() {
    cin >> n0 >> x_0 >> y_0 >> w0 >> h0;
    ANS.assign(h0, vi32());
    pt(n0, x_0, y_0, x_0 + w0 - 1, y_0 + h0 - 1, 0, 0, 1);
    stringstream strm;
    for (const vi32& row : ANS) {
        for (int a : row)
            strm << a << ' ';
        strm << endl;
    }
