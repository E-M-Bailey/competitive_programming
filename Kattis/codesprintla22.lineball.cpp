...
bool find(int j, const vvi32& G, vi32& btoa, vi32& vis)
{
	if (btoa[j] == -1)
        return 1;
	vis[j] = true;
...
int dfsMatching(const vvi32& G, vi32& btoa)
{
	vi32 vis;
	...
	return btoa.size() - count(ALL(btoa), -1);
}

vi32 cover(const vvi32& G, int n, int m)
{
	vi32 match(m, -1);
	int res = dfsMatching(G, match);
	vbl lfound(n, true), seen(m);
	for (int it : match)
		if (it != -1)
			lfound[it] = false;
	vi32 q, cover;
