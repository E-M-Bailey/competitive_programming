...
struct Pt {
	T x, y;
	friend istream& operator>>(istream& istrm, Pt& pt) {
		istrm >> pt.x >> pt.y;
		//cerr << "reading " << pt;
		pt.x /= l;
		pt.y /= l;
	...
	friend ostream& operator<<(ostream& ostrm, Pt pt) {
        return ostrm << "(" << pt.x << ", " << pt.y << ")";
	}
	friend bool operator==(Pt l, Pt r) { return tie(l.x, l.y) == tie(r.x, r.y); }
	friend Pt operator-(Pt l, Pt r) { return {l.x - r.x, l.y - r.y}; }
	friend Pt operator+(Pt l, Pt r) { return {l.x + r.x, l.y + r.y}; }
	friend T operator^(Pt l, Pt r) { return l.x * r.y - l.y * r.x; }
	T arg() {
		T ans = atan2(y, x);
		return ans >= 0 ? ans : ans + 2 * PI;
	}
};

vector<Pt> pts;

long double sqArea(Pt a, Pt b) {
	bool comp = false;
	if (a.x > b.x) {
		a.x = 1 - a.x;
		b.x = 1 - b.x;
	...
	if (a.x == b.x)
		return comp ? a.x : 1 - a.x;
	T dy = b.y - a.y, dx = b.x - a.x, sl = dy / dx, yi = a.y - sl * a.x;
	Pt p0 = yi >= 0 ? Pt{0, yi} : Pt{-yi / sl, 0};
	Pt p1 = sl + yi <= 1 ? Pt{1, sl + yi} : Pt{(1 - yi) / sl, 1};
	T ans = (p0.y + p1.y) * (p1.x - p0.x) / 2 + 1 - p1.x;
	return comp ? 1 - ans : ans;
}
/*
template<typename U>
void eraseMarked(vector<U>& v, vector<bool> m) {
...
struct Event {
	T angle;
	int dCt, idx;
	Event(T angle, int dCt, int idx) :
		angle(angle),
		dCt(dCt),
		idx(idx)
	{}
	friend bool operator<(Event l, Event r) {
        if (abs(l.angle - r.angle) > EPS)
...
void go() {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < n; j++) {
			...
			Pt b = pts[j];
			T angle = (b - a).arg();
			E.emplace_back(angle, 1, j);
			E.emplace_back(angle >= PI ? angle - PI : angle + PI, -1, j);
			ct += b.y < a.y || b.y == a.y && b.x < a.x;
		}
		bool r = a.x * 2 > 1, u = a.y * 2 > 1;
		Pt c = {r, u};
		Pt d = c - a;
		(r ^ u ? d.y : d.x) *= -1;
		E.emplace_back(d.arg(), 0, n);
		pts.push_back(d + a);

		//E.emplace_back(-1, init);
		sort(ALL(E));
		T invN = (T)1 / n;
		ans = max(ans, ct * invN - a.y);
		for (auto [ag, dCt, j] : E) {
            //cerr << i << a << ' ' << j << pts[j] << ": c = " << ct << ", += " << dCt << ' ' << ag << endl;
            ct += dCt;
            T pCt = ct * invN;
            T pAr = sqArea(a, pts[j]);
            if (dCt < 0)
                pAr = 1 - pAr;
            T cur = pCt - pAr;
            //cerr << "  " << ct * invN << '-' << sqArea(a, pts[j]) << '=' << cur; if (cur >= ans) cerr << '*'; cerr << endl;
            ans = max(ans, cur);
		}
		pts.pop_back();
	}
