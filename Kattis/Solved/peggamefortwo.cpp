...
int f(int state) {
    ...
    for (array<int, 3> mv : MV) {
        if (!(state & 1 << mv[0]) || !(state & 1 << mv[1]) || (state & 1 << mv[2]))
            continue;
        found = true;
        int next = state ^ (1 << mv[0] | 1 << mv[1] | 1 << mv[2]);
        dp += f(next);
        //dp = max(dp, G[])
    }
    return dp;
...
void go() {
	...
	/*int ans = 0;
	for (int i = 0; i < 15; i++) {
		cerr << i << ' ' << f((1 << 15) - 1 - (1 << i)) << endl;
		ans = max(ans, f((1 << 15) - 1 - (1 << i)));
	}
	cerr << ans << endl << MV.size() << endl;*/
	//cerr << ans << endl;
