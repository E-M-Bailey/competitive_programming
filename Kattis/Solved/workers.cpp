...
struct result
{
	...
	result(int n, int k, const vvpi32& A, const vvpi32& B) :
		n(n),
		k(k),
		c(0)
	{
		vvf64 WA(n, vf64(n)), WB(n, vf64(n));
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
			{
		...
		IA = hungarian<f64>(WA, 1e-13);
		IB = perm_inv(hungarian<f64>(WB, 1e-13));
		for (int i = 0; i < n; i++)
			{
                c += WA[i][IA[i]];
                c += WB[IB[i]][i];
	...
	friend ostream& operator<<(ostream& strm, const result& r)
	{
        ...
        for (int i = 0; i < r.n; i++)
        {
            int j = r.IA[i];
            int k = r.IB[j];
            buf << i + 1 << ' ' << j + 1 << (j < r.k ? 'A' : 'B') << ' ' << k + 1 << endl;
        }
        strm << buf.str();
        return strm;
	}
...
void go()
{
	int n;
	cin >> n;
	vvpi32 A(n, vpi32(n)), B(n, vpi32(n));
	for (vpi32& AA : A)
		for (pi32& a : AA)
