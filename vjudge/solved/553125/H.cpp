...
{
	//cerr << "   Intersecting " << a << " with " << b << endl;
	const auto [A, C] = a.v;
	const auto [B, D] = -b.v;
	const auto d = (a.p - b.p) / (a.v ^ b.v);
	//cerr << "   " << A << ' ' << B << ' ' << C << ' ' << D << endl;
	//cerr << "   " << d << endl;
	//cerr << "   " << v2{D, -B} *d << ' ' << v2{-C, A} *d << endl;
...
void cmp_hits(int ib)
{
	H[ib].clear();
	const ray& r = R[ib];
	//cerr << " Ball " << ib << " path: " << r.v << " starting " << r.st << '(' << r.v(r.st) << ')' << endl;
	for (int is = 0; is < k; is++)
	{
		const seg& s = S[is];
		if (abs(r.v.v ^ s.v.v) <= E)
		{
			//cerr << " Ball " << ib << " parallel to segment " << is << '(' << S[is] << ')' << "." << endl;
			continue;
		}
		auto [tr, ts] = isect(r.v, s.v);
		if (tr - r.st <= E || ts <= E || s.l - ts <= E)
		{
			//cerr << " Ball " << ib << " misses segment " << is << '(' << S[is] << ')' << " at time " << tr << "." << endl;
			//cerr << "  " << tr << ' ' << ts << endl;
			//cerr << "  " << tr - r.st << ' ' << ts << ' ' << s.l - ts << endl;
			continue;
		}
		//cerr << " Ball " << ib << " hits segment " << is << '(' << S[is] << ')' << " at time " << tr << "." << endl;
...
void proc()
{
	...
	{
		//cerr << "Ball " << ib << " passing through region " << ir << " at time " << t << endl;
		//cerr << " nxt: " << ht.nxt() << endl;
		hits.pop_back();
	}
	else
	{
		//cerr << "Ball " << ib << " hitting region " << ir << " at time " << t << endl;
		//cerr << " nxt: " << ht.nxt() << endl;
		ry = ht.nxt();
		cmp_hits(ib);
	}
	//cerr << ' ' << ssize(que) << ' ' << c << endl;
	if (is != 0)
		upd(ib);
	//else
	//	cerr << "Ball " << ib << " done" << endl;
	//for (int jb : O)
	//	cerr << ' ' << jb;
	//cerr << endl;
}
...
int main(int argc, const char* argv[])
{
	...
	isect(tvec{ v2{0, -1}, v2{0, 1} }, tvec{ v2{-1, 0}, v2{1, 0} });
