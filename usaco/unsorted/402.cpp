...
struct LPTreap {
	...
	void f(N& n) { n.s = n.v + s(pd(n.c[0])) + s(pd(n.c[1])); } //a
	...
	template<bool Pd/*x goes right*/> void rk(int r, K x) { //k
		while (pd(r) >= 0) {
			bool d = Pd ? cmp(T[pd(r)].k, x) : !cmp(x, T[pd(r)].k);
			push(r, d); r = T[r].c[d];
		}
	}
	...
	pii spt() { // split
		aii a{ -1, -1 }; while (sz(R)) {
			B b = pop(); T[b.r].c[b.d] = a[!b.d]; upd(a[!b.d] = b.r);
		} return { a[0], a[1] };
	}
