...
void go()
{
	...
	vi32 G(n + 1), E(n + 1), L(n + 1);
	rep(i, 0, n)
	{
		G[i + 1] = G[i] + (A[i].first > A[i].second);
		E[i + 1] = E[i] + (A[i].first == A[i].second);
		L[i + 1] = L[i] + (A[i].first < A[i].second);
	}

	auto counts = [&](int l, int r, int k) -> tuple<int, int, int>
		{
			l += 2 * k;
			r += 2 * k;
			if (r <= 0)
			...
			if (l >= r)
				return { 0, 0, 0 };
			return { G[r] - G[l], E[r] - E[l], L[r] - L[l] };
		};

	rep(k, 0, n * 2)
	{
		auto [g1, e1, l1] = counts(-5 * n, -3 * n - 1, k);
		auto [g3, e3, l3] = counts(-3 * n, -1 * n - 1, k);
		auto [g5, e5, l5] = counts(-1 * n, +1 * n - 1, k);
		auto [g7, e7, l7] = counts(+1 * n, +3 * n - 1, k);

		auto [g2, e2, l2] = counts(-3 * n - 1, -3 * n, k);
		auto [g4, e4, l4] = counts(-1 * n - 1, -1 * n, k);
		auto [g6, e6, l6] = counts(+1 * n - 1, +1 * n, k);
	...
	/*
	int ans = 0;
	while (ans < n && (A.front().first >= A.front().second || A.back().first <= A.back().second))
	{
		auto [a, b] = A.front();
		A.pop_front();
		A.emplace_back(b, a);
