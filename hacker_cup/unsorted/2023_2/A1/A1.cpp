...
struct dso
{
	...
	vpi32 X;
	dso(vvpi32 const &Y) :
		n((int)(size(Y))), P(size(Y)), S(size(Y), 1), X(size(Y))
	{
		iota(ALL(P), 0);
		rep(i, 0, size(P))
		{
	...
	bool onion(int x, int y)
	{
		...
		P[y] = x;
		S[x] += S[y];
		//cerr << "merge " << x << ' ' << y << ' ' << X[x].first << ' ' << X[y].first << endl;
		if (X[x].first == -2 || X[y].first == -2 || (X[x].first >= 0 && X[y].first >= 0 && X[x] != X[y]))
			X[x].first = -2;
		else
			X[x] = max(X[x], X[y]);
		return true;
...
void go()
{
	...
	rep(k, 0, n)
	{
		...
		{
			int i2 = i + di, j2 = j + dj;
			if (i2 < 0 || i2 >= r || j2 < 0 || j2 >= c || G[i2][j2] != '.')
				continue;
			Y[k].emplace_back(i2, j2);
		}
	}
	...
	rep(k, 0, n)
	{
		...
		{
			int i2 = i + di, j2 = j + dj;
			if (i2 < 0 || i2 >= r || j2 < 0 || j2 >= c || G[i2][j2] != 'W')
				continue;
			int k2 = B[i2][j2];
			D.onion(k, k2);
		}
	...
	//{
	//	auto [i, j] = A[k];
	//	cerr << i << ' ' << j << ' ' << k << ' ' << Y[k].size() << ' ' << D.P[k] << ' ' << D.S[k] << ' ' << D.X[k].first << endl;
	//}
	vvi32 C(r, vi32(c));
	int ans = 0;
	rep(k, 0, n)
	{
		if (D.P[k] != k || D.X[k].first < 0)
			continue;
		auto [i, j] = D.X[k];
		//cerr << k << ": " << D.S[k] << ' ' << i << ' ' << j << ' ' << ans << endl;
		ans = max(ans, C[i][j] += D.S[k]);
	}
	//cerr << endl;
	cout << (ans > 0 ? "YES" : "NO") << '\n';
}
