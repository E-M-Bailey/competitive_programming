...
// a <- isect(a, b), b <- unspecified
void isect(usi32 &A, usi32 &B)
{
	if (A.size() < B.size())
		A.swap(B);
	usi32 ans;
...
// a <- union(a, b), b <- unspecified
void onion(usi32 &A, usi32 &B)
{
	if (A.size() < B.size())
		A.swap(B);
	for (int b : B)
...
void dfs(int v)
{
	...
	L[v] = C[v].empty() || (C[v].size() == 1 && L[C[v][0]]);
	...
	else
	{
		...
		// B:
		{
			...
			for (int w : Y)
			{
				...
				usi32 B2;
				for (int b : B[w])
					if (FL[b] == nl && FR[b] == nr && A[h].contains(b))
						B2.insert(b);
				onion(B[v], B2);
				for (int a : A[w])
					FL[a]++;
			...
			for (auto [b, f] : FL)
			{
				if (f == nl && FR[b] == nr - 1 && A[h].contains(b))
					B[v].insert(b);
				else if (f == nl && FR[b] == nr && (L[h] || B[h].contains(b)))
					B[v].insert(b);
			}
			for (auto [b, f] : FR)
			{
				if (FL[b] == nl && f == nr && (L[h] || B[h].contains(b)))
					B[v].insert(b);
			}
...
void go()
{
	...
	rep(i, 0, n)
	{
		...
		{
			cin >> tok;
			X[i][j] = D.emplace(move(tok), (int)D.size()).first->second;
		}
	}
