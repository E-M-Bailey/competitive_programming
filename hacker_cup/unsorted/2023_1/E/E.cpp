...
template<class T>
void add(unordered_map<T, int> &M, T key, int del)
{
	auto it = M.try_emplace(key, 0).first;
	if ((it->second += del) == 0)
		M.erase(it);
}

// Based on https://github.com/kth-competitive-programming/kactl/blob/main/content/data-structures/SegmentTree.h
struct Tree
{
	vector<unordered_map<i64, int>> F;
	vector<unordered_map<int, int>> FF;
	int n;

	Tree(int n = 0) : F(2 * n), FF(2 * n), n(n) {}

	void update(int pos, i64 val, int del)
	{
		pos += n;
		add(FF[pos], F[pos][val], -1);
		add(F[pos], val, del);
		add(FF[pos], F[pos][val], 1);
	...
	unordered_map<int, int> query(int b, int e)
	{ // query [b, e)
		unordered_map<int, int> ans;
		for (b += n, e += n; b < e; b /= 2, e /= 2)
		{
			if (b % 2) for (auto [f, ff] : FF[b++]) ans[f] += ff;
			if (e % 2) for (auto [f, ff] : FF[--e]) ans[f] += ff;
		}
		return ans;
	}
};

i64 constexpr MOD = 4611686018427387847, A = 4611686018427387817;
...
void go()
{
	...
	rep(k, 1, lim + 1)
	{
		for (int i : IS)
		{
			if (W[i].size() >= k)
			{
				H[i] = ((i128)H[i] * A + W[i][W[i].size() - k] - 'a') % MOD
			}
		}
	}
}
