...
struct Node
{
	struct T { int m, am, M, aM; }; // data type
	typedef bool L;
	const T tneut{ MOD, -1, 0, -1 };     // neutral elements
	const L lneut = false;
	T f(T a, T b)
	{
		if (b.m < a.m)
	...
	{
		if (b)
			return { MOD - a.M, a.aM, MOD - a.m, a.am };
		else
			return a;
	} // Apply lazy
	L comb(L a, L b)
...
void go()
{
	...
	auto root = make_unique<Node>(A, 0, n);
