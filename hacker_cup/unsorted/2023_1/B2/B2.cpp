...
// len -> sum -> prod -> prev
vector<vector<unordered_map<int, int>>> A(42, vector<unordered_map<int, int>>(42));
void pre()
{
	A[0][0].emplace(1, -1);
	rep(i, 0, 41)
		...
		rep(s, 0, 41)
		{
			for (auto [p, pr] : A[i][s])
			{
				rep(j, 1, 42)
				{
					i64 p2 = (i64)p * j;
					int s2 = s + j;
					if (s2 > 41 || p2 > 1'000'000'000)
						break;
					A[i + 1][s + j].try_emplace((int)p2, j);
				}
			}
		}
	}
...
void go()
{
	...
	int l = -1, s = -1, c = INT_MAX;
	rep(i, 0, 42)
		rep(j, 0, 42)
	{
		if ((it = A[i][j].find(p)) == A[i][j].end())
			continue;
		int cur = i + 41 - j;
		//cerr << i << ' ' << 
		if (cur < c)
	...
	while (it->second >= 0)
	{
		...
		it = A[--l][s -= it->second].find(p /= it->second);
	}
	cout << ans.size();
	for (int a : ans)
		cout << ' ' << a;
